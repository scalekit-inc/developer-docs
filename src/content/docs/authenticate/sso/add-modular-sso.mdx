...
<Aside type="note" title="Using SaaSKit?">
  SaaSKit includes SSO functionality by default. Skip this guide and follow [Enterprise SSO with SaaSKit](/authenticate/auth-methods/enterprise-sso/) instead. This Modular SSO guide is for developers who need to add SSO to an existing authentication system without replacing it.
</Aside>
