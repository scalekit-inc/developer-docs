...
3. ### Verify with the three webhook headers
   ...
   | `webhook-timestamp` | Unix seconds. The SDK rejects timestamps more than 5 minutes in the past or future |
   ...
   <Aside type="caution" title="Verify before you trust the payload">
   A forged POST can create or delete users in your app. Verify first. Sign the exact bytes Scalekit posted. `JSON.parse` / `request.get_json()` / `req.json()` change whitespace and fail the HMAC. Use `express.raw`, `request.get_data()`, or `io.ReadAll`.
   </Aside>
