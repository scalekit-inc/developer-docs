...
<Aside type='note' title='Webhook retry behavior'>
  Scalekit will retry failed webhook deliveries up to 3 times with exponential backoff if:

  - Your endpoint returns a status code other than 201
