...
Unlike [JWT-based M2M authentication](/authenticate/m2m/api-auth-quickstart/), API keys do not carry embedded claims and cannot be validated on the client side. Validate API keys server-side through Scalekit, then use the returned claims only when the key is valid. Revocation takes effect immediately with no expiry windows or propagation delays. Each key is scoped to an organization and optionally to a specific user, making it easy to build personal access tokens, per-user rate limiting, and audit trails.
...
<CardGrid>
  <LinkCard
    title="OAuth 2.0 for M2M APIs"
    description="Use short-lived JWTs instead of long-lived keys for machine-to-machine authentication."
    href="/authenticate/m2m/api-auth-quickstart/"
  />
  <LinkCard
    title="M2M API clients"
    description="Register and manage API clients for your customers."
    href="/guides/m2m/api-auth-m2m-clients/"
  />
</CardGrid>