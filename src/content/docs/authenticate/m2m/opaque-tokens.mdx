...
sidebar:
  label: "Opaque tokens"
tags: [api-tokens, opaque-tokens, m2m, authentication, bearer-tokens]
head:
  - tag: style
    content: |
...
if err != nil {
  log.Fatalf("Failed to create user token: %v", err)
}
```
...
You now have the building blocks to issue, validate, and manage opaque API tokens in your application. For short-lived, scoped authentication between services, see [M2M authentication with client credentials](/authenticate/m2m/api-auth-quickstart/). To control what each token can access, configure [scopes for your M2M clients](/guides/m2m/scopes/).
