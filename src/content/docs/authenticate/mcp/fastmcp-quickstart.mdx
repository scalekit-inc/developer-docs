...
description: 'FastMCP todo server with OAuth scope validation and CRUD operations.'
tags: [mcp-auth, quickstart, fastmcp, python, oauth2, scope-validation, ai-agents]
sidebar:
  order: 1
  label: 'Add auth to FastMCP server'
...
seeAlso:
  ...
  items:
    - title: 'Browse code sample'
      icon: 'github'
      url: 'https://github.com/scalekit-inc/mcp-demo/tree/main/todo-fastmcp'
---
...
5. ## Provide runtime secrets
   ...
   | ----------------------------- | ----------------------------------------------------------------------------------------- |
   | `SCALEKIT_ENVIRONMENT_URL`    | Your Scalekit environment URL, labeled **Environment URL** in **Dashboard > Settings > API Credentials** |
   | `SCALEKIT_CLIENT_ID`          | Client ID from **Dashboard > Settings > API Credentials**                                 |
   | `SCALEKIT_RESOURCE_ID`        | The resource identifier assigned to your MCP server (starts with `res_`). Find it on your MCP server's detail page in the dashboard. |
...
7. ## Connect with an MCP client
   ...
   <Aside>
   Leave the Inspector's Authentication fields empty. This quickstart uses dynamic client registration (DCR)
   </Aside>

   <Aside type="note" title="Testing scope enforcement">
     Try calling <code>create_todo</code> with a token that only has <code>todo:read</code>. The server will reject the request with an insufficient permissions error.
   </Aside>
