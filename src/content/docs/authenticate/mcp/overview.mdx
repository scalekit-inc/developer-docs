...
- **Client-Initiated Metadata (CIMD)**: Lets authorization servers fetch client metadata directly from a client-hosted document for authorization
...
<Steps>
4. **Client** registers with authorization server (if using DCR)
5. **Authorization server** issues client credentials (if using DCR)
