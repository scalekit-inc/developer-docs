...
MCP Client -> Your MCP Server: POST /mcp/session\n(Initialize session)
Your MCP Server ->  MCP Client: Session capabilities
MCP Client -> Your MCP Server: GET /capabilities\n(Fetch server capabilities/scopes/permissions/metadata)
Your MCP Server ->  MCP Client: Returns available API capabilities/authentication_scopes,\nauthorization_server_metadata with authorization_scopes
MCP Client -> Your MCP Server: GET /auth/options\n(Request available auth configuration options/auth_protocol)
Your MCP Server -> Scalekit Auth Server: GET /.well-known/oauth-authorization-server
Scalekit Auth Server -> Your MCP Server: Scalekit Auth Server metadata\n(endpoints, OAuth support)
Your MCP Server -> MCP Client: Returns OAuth server metadata
```
...
```d2 pad=36
title: "MCP Login Authorization Phase" {
  near: top-center
  shape: text
  style.font-size: 18
}
...
MCP Client -> Your MCP Server: Initiate OAuth flow (PKCE)
MCP Client -> Your MCP Server: GET /api/authorize
Your MCP Server -> Scalekit Auth Server: Redirect to /oauth/authorize with PKCE challenge
Scalekit Auth Server -> Your Customer's User: Display login/consent prompt
Your Customer's User -> Scalekit Auth Server: Authenticate & grant consent
Scalekit Auth Server -> Your MCP Server: Redirect with authorization code & state
Your MCP Server -> MCP Client: Return authorization code
MCP Client -> Scalekit Auth Server: POST /oauth/token with PKCE verifier
Scalekit Auth Server -> MCP Client: Issue access token (PKCE verified)
MCP Client -> Your MCP Server: Store token
Your MCP Server -> MCP Client: Success response
```
