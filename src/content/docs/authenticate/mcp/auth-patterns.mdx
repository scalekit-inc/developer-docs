...
description: 'Learn the three authentication patterns for MCP servers: human users via OAuth Authorization Code flow, autonomous agents via Client Credentials flow, and downstream integrations using API keys, OAuth, or token cascading'
sidebar:
  label: 'Auth patterns'
tableOfContents:
  maxHeadingLevel: 2
...
prev:
  label: 'Introduction to MCP authentication'
  link: '/mcp/intro-to-mcp-auth/'
next:
  label: 'Add OAuth 2.1 to your MCP server'
  link: '/mcp/quickstart/'
browseCentral:
...
seeAlso:
  ...
  expanded: false
  items:
    - title: 'MCP OAuth 2.1 quickstart'
      icon: 'book'
      url: '/mcp/quickstart/'
    - title: 'FastMCP implementation'
      icon: 'book'
      url: '/authenticate/mcp/fastmcp-quickstart/'
    - title: 'FastAPI + FastMCP implementation'
      icon: 'book'
      url: '/authenticate/mcp/fastapi-fastmcp-quickstart/'
    - title: 'Express.js implementation'
      icon: 'book'
      url: '/authenticate/mcp/expressjs-quickstart/'
---
...
To implement this pattern, you'll need to:

- Register your MCP server in the Scalekit Dashboard
- Implement the `.well-known/oauth-protected-resource` metadata endpoint
- Configure scopes that match your server's capabilities
- Set up token validation middleware to verify incoming requests
- Test the flow end-to-end with an MCP client

For complete implementation guidance, see the [MCP OAuth 2.1 quickstart](/mcp/quickstart/) or framework-specific guides for [FastMCP](/authenticate/mcp/fastmcp-quickstart/), [FastAPI + FastMCP](/authenticate/mcp/fastapi-fastmcp-quickstart/), and [Express.js](/authenticate/mcp/expressjs-quickstart/).
...
1. Navigate to **[Dashboard](https://app.scalekit.com) > MCP Servers** and select your MCP Server. Go to the **Clients** tab.
...
4. Optionally, set scopes (e.g., `todo:read`, `todo:write`) that correspond to the permissions configured for your MCP Server. Click **Save**.
...
To implement this pattern in your autonomous agent:

- Create an M2M client in the Scalekit Dashboard for your target MCP server
- Securely store the client credentials (use environment variables or a secrets manager)
- Request access tokens using the Client Credentials flow before making MCP calls
- Cache tokens and refresh them before expiration
- Attach tokens as Bearer tokens in the `Authorization` header when calling MCP tools
...
- Follow the [MCP OAuth 2.1 quickstart](/mcp/quickstart/) to implement Pattern 1 or Pattern 2
