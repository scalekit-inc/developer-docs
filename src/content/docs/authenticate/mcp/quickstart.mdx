...
2. ## Add MCP server to get drop-in OAuth2.1 authorization server
   ...
   ![Add MCP server](@/assets/docs/quickstart/mcp-create.webp)
   ...
   2. Enter the **Server URL** where your MCP server is hosted. Include the trailing slash (for example, `https://mcp.yourapp.com/`). Scalekit includes this URL as the `aud` claim in issued tokens, indicating that the token was intended for your specific MCP server. You can optionally validate this claim before allowing tool access or execution.
   3. Enable **dynamic client registration** for MCP hosts. This allows MCP hosts to automatically register with Scalekit (and your authorization server), eliminating the need for manual registration and making it easier for users to adopt your MCP server secur.
...
3. ## Let MCP clients discover your OAuth2.1 authorization server
   ...
   ![MCP server setup](@/assets/docs/quickstart/mcp-server-setup.webp)
...
4. ## Validate all MCP client requests have a valid access token
   ...
   Validate the token against your configured resource audience to ensure it was issued for your specific MCP server. The resource identifier must match the Server URL you registered in step 2.
...
6. ## Enable additional authentication methods
   ...
   **[Enterprise SSO](/authenticate/auth-methods/enterprise-sso/)**
