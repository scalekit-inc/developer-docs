...
2. ## Add MCP server to get drop-in OAuth2.1 authorization server
   ...
   Note: If your MCP server is intended for use by public MCP Clients, it’s recommended to keep both DCR and CIMD enabled.
    ...
    <Aside type="caution" title="Toggling DCR or CIMD?">
      If you enable or disable DCR or CIMD, be sure to restart your MCP server. Certain MCP frameworks cache authorization server details, and a restart ensures the updated configuration is correctly applied.
    </Aside>
