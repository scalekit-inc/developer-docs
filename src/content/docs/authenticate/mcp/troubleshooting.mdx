...
### A client gets an `invalid_client_metadata_url` error when it connects
...
The `invalid_client_metadata_url` error belongs to the CIMD path: Scalekit couldn't resolve or validate the metadata URL the client sent. A client that completes DCR correctly doesn't trigger this error. Follow the resolution steps in the section above.

---

### How do I view authentication logs to debug a failed connection?
...
### ChatGPT connector completes consent but requests stay pending

When you add your MCP server as a ChatGPT connector, the OAuth consent screen completes, but ChatGPT then shows an error and every request stays in a `pending` state.

**Root cause:** ChatGPT requires `private_key_jwt` token-endpoint authentication and uses it even when your authorization server advertises other supported methods. If the connector registered before your server accommodated `private_key_jwt`, ChatGPT holds onto the earlier registration and its token requests never succeed.

**Resolution:**

1. Confirm your Scalekit environment accepts `private_key_jwt` for token-endpoint authentication. Contact Scalekit support if you are unsure whether it is enabled for your MCP server.
2. In ChatGPT, disconnect the connector for your MCP server.
3. Reconnect the connector. Reconnecting forces a fresh Dynamic Client Registration, so ChatGPT registers with the correct token-endpoint authentication method.
4. Complete the consent flow again and confirm requests move out of `pending`.

<Aside type="note" title="Reconnect to force fresh registration">
Disconnecting and reconnecting is required because ChatGPT reuses its cached client registration. Without a fresh registration, the connector keeps sending token requests that use the earlier, incompatible configuration.
</Aside>
