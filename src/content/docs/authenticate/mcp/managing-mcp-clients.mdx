...
- **Shorten the access token lifetime.** Lower the **Access token lifetime** for the MCP server in the dashboard (set it when you [add an MCP server](/authenticate/mcp/quickstart/); the recommended range is 300–3600 seconds). You can also set it per client. A shorter lifetime means a revoked token stops working sooner.
