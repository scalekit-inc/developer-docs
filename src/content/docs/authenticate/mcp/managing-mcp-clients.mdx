...
tags: [mcp-auth, clients, oauth2, consent, management, revoke-access, authorization]
tableOfContents:
  maxHeadingLevel: 3
sidebar:
...
3.  **Choosing the right OAuth flow:**
    ...
    **Cross-App Access (XAA):** XAA doesn't require any explicit setting - your static clients by default support cross-app access under enterprise plan.
...
To audit the same consents from your own admin screens, list them with the Node SDK. See [Resource consents](/saaskit/sdks/node/resources/).
...
To revoke a consent programmatically, call `revokeUserConsent` in the Node SDK. See [Resource consents](/saaskit/sdks/node/resources/).
...
### How revocation affects active access tokens

Revoking consent or ending a session stops Scalekit from issuing **new** access tokens for that user, but it does not cancel an access token that was already issued. The existing token keeps working until it expires.

This is expected behavior, not a vulnerability. Scalekit issues access tokens as self-contained JWTs. Your MCP server validates each token locally against Scalekit's public keys and does not call back to the authorization server on every request, so Scalekit cannot reach out and cancel a token that is already in circulation. Every OAuth server that issues self-contained JWTs behaves this way. The exposure is bounded by the time left on the token — its `exp` claim.

To shrink or close that window, use either approach:

- **Shorten the access token lifetime.** Lower the **Access token lifetime** for the MCP server at **Dashboard > MCP servers > your server > Access token lifetime** (you can also set it when you [add an MCP server](/authenticate/mcp/quickstart/); the recommended range is 300–3600 seconds). You can also set it per client. A shorter lifetime means a revoked token stops working sooner. This MCP server setting is separate from the access token lifetime configured for a Web Application — changing the Web Application setting does not affect tokens minted for this MCP server.
- **Maintain a token denylist.** Validate the token with the Scalekit SDK as you do today, then check its `jti` claim against a denylist before serving the request, and reject the request when the `jti` is present. Store each revoked `jti` with a time-to-live equal to the token's remaining lifetime — for example, a Redis key that expires at the token's `exp`. This gives near-immediate revocation at the cost of one lookup per request.
