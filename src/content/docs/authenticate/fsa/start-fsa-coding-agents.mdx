...
4. ## Generate authentication implementation
   ...
   ```
    ...
    - Loads the Full Stack Auth skill from the marketplace
    - Analyzes your existing application structure
    - Generates Scalekit client initialization with environment credentials
    - Creates the login redirect handler
    - Implements the OAuth callback to exchange the authorization code for tokens
    - Adds secure session storage and a logout endpoint
