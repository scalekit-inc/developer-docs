...
tags: [authentication, login, code-exchange, oauth2, tokens, jwt, callback]
...
If you already added session middleware, `/callback` is already registered. See [Manage user sessions](/authenticate/fsa/manage-session/) for how session middleware completes login.
...
3. ## Decoding token claims
      ...
      <details>
      <summary>Validate ID tokens without an SDK</summary>
      ...
      <Aside type="note" title="Audience may be a string or array">
      ...
      ```ruby title="Ruby on Rails example"
      require "jwt"
      require "net/http"
      require "json"
...
<Aside type="caution" title="Include state in production code">
