---
title: Quickstart
description: "Hosted auth pages, managed sessions, secure logout. Purpose built. Simple where it counts"
sidebar:
...
<Card title="Get moving, instantly, with your go-to AI assistant" icon={IconRiAiGenerate2}>
  <div class="llm-prompt-card">
    <div class="llm-prompt-content">
      <p>Input this prompt in your IDE to analyze your existing code base and generate complete auth implementation code accordingly.</p>
...
2. ## Set up redirect URLs for your application

   Redirect URLs are endpoints in your application that Scalekit redirects users to during the authentication flow. Pre-registering these URLs with Scalekit is required to prevent malicious or unauthorized redirects. 
   To configure them, go to **Scalekit dashboard** → **Authentication** → **Redirect URLs**.

   - **Allowed callback URL**: Where Scalekit sends the user after they successfully authenticate.  
     Your app uses this endpoint to exchange the authorization code for session tokens and fetch the user profile (example: `https://your-app.com/auth/callback`). Make sure this matches your app URL exactly (scheme, domain, port, and path). [Learn more](/guides/dashboard/redirects/#allowed-callback-urls)

   - **Initiate-login URL**: Required to handle scenarios where authentication is not initiated from your app—for example, when a user accepts an organization invitation or starts sign-in directly from their identity provider (IdP-initiated SSO). Scalekit redirects the user to this endpoint, and your app should immediately redirect them back to Scalekit’s `/authorize` endpoint to continue the authentication flow. [Learn more](/guides/dashboard/redirects/#initiate-login-url)
...
3. ## Redirect users to sign up (or) login
   ...
   // Must match the allowed callback URL you registered in step 2
...
5. ## Create and manage user sessions
