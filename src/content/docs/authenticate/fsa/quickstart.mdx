...
2. ## Start login
   ...
   See [Manage user sessions](/authenticate/fsa/manage-session/) for how `sk_session` is stored and refreshed.
...
3. ## Get user details from the callback
   ...
   Session middleware already owns `/callback` for Node.js and Python. See [Manage user sessions](/authenticate/fsa/manage-session/). Go and Java exchange the code below.

   <Tabs syncKey="tech-stack">
   <TabItem value="node" label="Node.js">

   `/callback` is already registered. The helper writes `sk_session`. See [Manage user sessions](/authenticate/fsa/manage-session/) for how `sk_session` is stored and refreshed.
...
4. ## Create and manage user sessions
      ...
      Node.js and Python session middleware write `sk_session` and refresh it. See [Manage user sessions](/authenticate/fsa/manage-session/). Go and Java set cookies below.

      <Tabs syncKey="tech-stack">
        <TabItem value="node" label="Node.js">

        Session middleware already wrote `sk_session`. How to protect routes and refresh: [Manage user sessions](/authenticate/fsa/manage-session/).
      ...
      Session middleware already checks `sk_session` on Node.js and Python. See [Manage user sessions](/authenticate/fsa/manage-session/). Go and Java verify tokens below.

      <Tabs syncKey="tech-stack">
        <TabItem value="node" label="Node.js">

        Use `auth.requiresAuth` or `auth.withAuth`. Full steps: [Manage user sessions](/authenticate/fsa/manage-session/).

        </TabItem>
        <TabItem value="python" label="Python">

        Use `@auth.requires_auth` or `Depends(auth.requires_auth)`. Full steps: [Manage user sessions](/authenticate/fsa/manage-session/).
...
5. ## Log out the user
      ...
      Node.js and Python session middleware already registered `GET /logout`. See [Manage user sessions](/authenticate/fsa/manage-session/) and [Implement logout](/authenticate/fsa/logout/).

      <Tabs syncKey="tech-stack">
        <TabItem value="node" label="Node.js">

        Send the browser to `/logout`. Full logout steps: [Implement logout](/authenticate/fsa/logout/).
