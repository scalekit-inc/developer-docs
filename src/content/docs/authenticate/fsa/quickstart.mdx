...
2. ## Set up redirect URLs for your application
   ...
   - **Initiate-login URL**: Required to handle scenarios where authentication is not initiated from your app. 
     Scalekit redirects the user to this endpoint, and your app should immediately redirect them back to Scalekit’s `/authorize` endpoint to continue the flow. [Learn more](/guides/dashboard/redirects/#initiate-login-url)
...
3. ## Redirect users to sign up (or) login
   ...
   // Must match the allowed callback URL you registered in step 1
