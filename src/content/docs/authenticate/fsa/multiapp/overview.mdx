...
head:
  ...
  - tag: script
    ...
    content: |-
      {"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{"@type":"Question","name":"Why am I getting a redirect URI mismatch error?","acceptedAnswer":{"@type":"Answer","text":"<p>The exact URI (including trailing slashes and query parameters) must match what's configured in <strong>Dashboard > Developers > Applications > [Your App] > Redirects</strong>. Common mismatches include:</p><ul><li>http vs https</li><li>Missing or extra trailing slash</li><li>Different port numbers in development</li></ul>"}},{"@type":"Question","name":"Why aren't my apps sharing authentication state?","acceptedAnswer":{"@type":"Answer","text":"<p>Verify all applications are registered in the same Scalekit environment. Apps in different environments maintain separate session pools and cannot share authentication state.</p>"}},{"@type":"Question","name":"Why are users prompted to login on every app?","acceptedAnswer":{"@type":"Answer","text":"<p>Check the following:</p><ul><li>All apps use the same Scalekit environment URL</li><li>The browser allows third-party cookies (required for session detection)</li><li>The user is using the same browser across apps</li></ul>"}},{"@type":"Question","name":"Why is the refresh token being rejected?","acceptedAnswer":{"@type":"Answer","text":"<p>The Scalekit session may have been revoked from another application, or the refresh token has expired. Redirect the user to log in again to establish a new session.</p>"}}]}
seeAlso:
  items:
