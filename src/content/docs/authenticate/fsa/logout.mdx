...
tags: [authentication, logout, sessions, security, token-revocation, sign-out]
sidebar:
...
Clear the app session and the Scalekit session in one logout.

Node.js and Python session middleware already register `GET /logout`. That handler clears `sk_session` and ends the Scalekit session. See [Manage user sessions](/authenticate/fsa/manage-session/) for how `sk_session` is stored and refreshed.

![Logout flow showing three session layers](@/assets/docs/fsa/logout/1.png)

<Steps>

1. ## Send the browser to logout

   Session middleware already owns `/logout`. Send the browser to `/logout`. Do not call `/oidc/logout` from `fetch`.

   Go and Java still build the logout URL. See those tabs.
   ...
   `auth.router` already registered `GET /logout`. Link to it. See [Manage user sessions](/authenticate/fsa/manage-session/) for how `sk_session` is stored and refreshed.
   ...
   Export the logout Route Handler. See [Manage user sessions](/authenticate/fsa/manage-session/) for how `sk_session` is stored and refreshed.
   ...
   `ScalekitAuth` already registered `GET /logout`. See [Manage user sessions](/authenticate/fsa/manage-session/) for how `sk_session` is stored and refreshed.
   ...
   `auth.install(app)` already registered `GET /logout`. See [Manage user sessions](/authenticate/fsa/manage-session/) for how `sk_session` is stored and refreshed.
   ...
   Session middleware clears `sk_session` and ends the Scalekit session. If you build logout yourself, extract the ID token before you clear cookies.
    ...
    <details>
    <summary>Build it yourself — getLogoutUrl and clear cookies</summary>
