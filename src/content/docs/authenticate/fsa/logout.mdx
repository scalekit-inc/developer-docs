...
Use `/oidc/logout` for user logout functionality. This endpoint requires a browser redirect and clears the user's session server-side.

The `/oauth/revoke` endpoint is different—it's an authenticated API call for programmatic token invalidation and requires client authentication (client_id + client_secret).

| Scenario | Endpoint | Method |
|----------|----------|--------|
| **User logout** (terminate session) | `/oidc/logout` | Browser redirect |
| **Revoke specific tokens** | `/oauth/revoke` | Authenticated API call |
...
The `/oidc/logout` endpoint must be accessed via **browser redirect**, not an API call. When you redirect the browser to Scalekit's logout URL, the browser automatically includes the session cookie in the request, allowing Scalekit to identify and terminate the correct session.
...
2. **Confirm you're using `/oidc/logout`** - The `/oauth/revoke` endpoint is for token invalidation, not user logout.

3. **Check post-logout redirect URI** - Ensure it's registered in **Dashboard > Authentication > Redirects > Post Logout URL**.
