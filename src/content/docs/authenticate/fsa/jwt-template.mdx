...
Review the default claims already present on every access token:
...
These claims are also listed in your Scalekit dashboard. Navigate to **Dashboard > Customize > Token Claims** to see them.
...
For a true end-to-end check, sign in through your OIDC flow and decode the access token you receive. Decoding only shows you what a token contains, it doesn't validate it: your API must still verify the signature and check `iss`, `aud`, and the expiry before trusting any claim on it. A custom claim also only reflects the data at the time the token was issued, so an existing token keeps its old value until the user logs in again or the token refreshes.
