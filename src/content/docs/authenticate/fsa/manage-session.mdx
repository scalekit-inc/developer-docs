...
3. ## Configure session security and duration
    ...
    <Aside type="note" title="Per-organization overrides">
    Organizations can override the absolute and idle session timeouts above with their own custom values. Scalekit always applies the stricter of the two at session creation. Access token lifetime is not configurable at the org level. See [Organization session policy](/authenticate/manage-organizations/organization-session-policy/).
    </Aside>
