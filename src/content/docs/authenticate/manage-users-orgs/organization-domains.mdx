...
2. ### Generate an Admin Portal link and embed in your app
   ...
   Generate a admin portal link and embed in your application:
...
Firstly, turn on the `domain_verification` feature for the organization.Redirect the end users to [Hosted widgets](/authenticate/manage-users-orgs/hosted-widgets/) directly to manage their organization's domains. The domain verification flow inside the widget is identical to the Admin Portal: the user enters a domain, the widget displays the TXT record to publish, and Scalekit verifies in the background.
