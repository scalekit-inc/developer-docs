...
JIT provisioning only works for users whose email domain matches one of the organization's registered [Organization domains](/authenticate/auth-methods/enterprise-sso/). This ensures that only verified members of the organization can be automatically provisioned.
...
- **Solution**: Register the user's domain in [Organization domains](/authenticate/auth-methods/enterprise-sso/) or [manually invite](/fsa/guides/user-invitations/) the user
...
- [Organization domains](/authenticate/auth-methods/enterprise-sso/) - Register email domains for JIT provisioning
