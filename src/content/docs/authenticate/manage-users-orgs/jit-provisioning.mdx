...
4. ### Using self-service Admin Portal for organization admins
    ...
    Your customers (organization admins) can also manage JIT provisioning settings through the Admin Portal. Generate and share Admin Portal with your end customers to set up SSO for their organization. Learn more about generating [Admin Portal](/guides/admin-portal/) for an organization.
...
<details>
<summary>Why is JIT provisioning creating duplicate users?</summary>

This typically happens when you have multiple SSO connections with different user identifier mappings. Scalekit may not recognize the same user across different connections.

**Solution**: Ensure consistent user identifier mapping (email, userPrincipalName, etc.) across all SSO connections for the same organization.
</details>

<details>
<summary>Why are user roles not assigned correctly during JIT provisioning?</summary>

Default roles are assigned based on the SSO connection configuration. If roles aren't assigned correctly, the default role configuration may be missing or incorrect.

**Solution**: Review SSO connection settings for default role assignments in **Dashboard > Organizations > [Organization] > Roles**.
</details>
