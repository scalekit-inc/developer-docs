...
5. ### Verify organization domains
    ...
    Your customers can add and verify organization domains directly from the **Domain Verification** widget. They enter a domain, publish the displayed DNS TXT record at their DNS provider, and click **Verify** — Scalekit polls automatically and updates the status to **DNS Verified** once the record is found.

    <Aside>
    Domain Verification widget visibility depends on the organization's feature entitlements. It appears only if the `domain_verification` feature is enabled for the organization. Enable it in the Scalekit dashboard at **Dashboard > Organizations > [Org] > Settings > Features**, or programmatically via [`updateOrganization`](/authenticate/manage-users-orgs/organization-domains/#enable-self-serve-dns-verification-for-an-organization).
    </Aside>

    For the verification status lifecycle and webhook events, see [Organization domains](/authenticate/manage-users-orgs/organization-domains/).
...
6. ### Manage session policy
    ...
    <Aside>
    Session policy widget visibility depends on the organization's feature entitlements. It appears only if the `session_policy` feature is enabled for the organization. Enable it in the Scalekit dashboard or programmatically via [`updateOrganizationSettings`](/authenticate/manage-organizations/organization-session-policy/#prerequisites).
    </Aside>

    For the full configuration options and API examples, see [Organization session policy](/authenticate/manage-organizations/organization-session-policy/).
...
<Aside>
Scalekit creates **Admin** and **Member** roles for every environment by default. Scalekit permissions are mapped to these two roles by default. The Admin role has all Scalekit permissions — including `sk_org_session_policy_manage` — and can access all Hosted Widgets. The Member role has limited access to organization widgets and can only view organization settings and organization members. Both roles have access to user widgets.
