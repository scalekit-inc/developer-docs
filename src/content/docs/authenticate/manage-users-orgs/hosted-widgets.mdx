...
head:
  - tag: script
    ...
    content: |-
      {"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{"@type":"Question","name":"What is the shared session between my app and Hosted UI widgets?","acceptedAnswer":{"@type":"Answer","text":"<p>It is the same Scalekit session issued to your application for that user. After your <a href=\"/authenticate/fsa/quickstart/\">authentication flow</a>, the browser holds Scalekit cookies for your environment or custom domain. Hosted UI widgets read those same cookies, so users do not sign in again when you redirect them to /ui/.</p>"}},{"@type":"Question","name":"What happens if a user does not have a session?","acceptedAnswer":{"@type":"Answer","text":"<p>Scalekit redirects the user to that Scalekit environment's login page for their account. Send users to /ui/ only after they complete your <a href=\"/authenticate/fsa/quickstart/\">authentication flow</a> so a Scalekit session cookie is available.</p>"}},{"@type":"Question","name":"What happens when a user logs out from Hosted UI widgets?","acceptedAnswer":{"@type":"Answer","text":"<p>Scalekit ends the Scalekit session and redirects the user to that Scalekit environment's login page for their account (the hosted login screen on your environment or <a href=\"/guides/custom-domain/\">custom domain</a>). Because Hosted UI widgets and your app share that session via Scalekit cookies, the user must sign in again before returning to /ui/ or to app routes that rely on the same browser session.</p><p>If you keep a separate application session in your own backend, clear it when Scalekit logs the user out. Use <a href=\"/guides/dashboard/redirects/#back-channel-logout-url\">back-channel logout</a> or the <a href=\"/apis/#webhook/userlogout\">user logout webhook</a> so your backend stays in sync. See <a href=\"/authenticate/fsa/manage-session/\">manage sessions</a> for guidance.</p>"}},{"@type":"Question","name":"How does /ui/ choose the organization for multi-org users?","acceptedAnswer":{"@type":"Answer","text":"<p>The portal uses the user's <strong>current active organization</strong> from their Scalekit session. Set or switch the active organization in your application before redirecting to /ui/. You do not pass an organization ID as a query parameter on the portal URL.</p>"}},{"@type":"Question","name":"Do Hosted UI widgets require webhooks to load data?","acceptedAnswer":{"@type":"Answer","text":"<p>No. Scalekit hosts the pages and loads the data they need. Use webhooks for your application workflows, for example logout propagation or provisioning events, not to supply data to Hosted UI widgets.</p>"}},{"@type":"Question","name":"Can I embed Hosted UI widgets in an iframe?","acceptedAnswer":{"@type":"Answer","text":"<p>No. Use the <a href=\"/guides/admin-portal/\">Admin portal</a> for iframe embeds and shareable links. The Admin portal supports only the SSO, SCIM, and organization domain verification modules. For every other module, such as member management, session policy, and user profile, use Hosted UI widgets, which are full-page experiences only; redirect users to /ui/.</p>"}}]}
---
...
![Organization settings widget showing the organization name field and a list of allowed email domains](@/assets/docs/hosted-widgets/org_settings.png)
...
![Member management widget listing organization users with their roles and active status, plus an Invite User button](@/assets/docs/hosted-widgets/org_member.png)
...
![Single Sign-On widget prompting the user to choose an identity provider such as Okta, OneLogin, Google Workspace, or Entra ID](@/assets/docs/hosted-widgets/org_sso.png)
...
![SCIM provisioning widget prompting the user to select a directory provider such as Okta, OneLogin, Google Workspace, or Entra ID](@/assets/docs/hosted-widgets/org_scim.png)
...
![Session policy widget with Application and Custom options selected, showing absolute and idle session timeout settings](@/assets/docs/hosted-widgets/2026-05-21-17-12-14.png)
...
![My Account widget showing editable first name and last name fields and a verified email address](@/assets/docs/hosted-widgets/user_profile.png)
...
![User security widget showing a registered passkey and a list of active sessions with options to revoke them](@/assets/docs/hosted-widgets/user_security.png)
