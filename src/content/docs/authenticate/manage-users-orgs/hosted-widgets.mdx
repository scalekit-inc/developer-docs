...
2. ### Verify organization domains

    Your customers can add and verify the domains they own, enabling [Home Realm Discovery (SSO) and SCIM provisioning](/authenticate/manage-users-orgs/organization-domains/) for their organization.
    ...
    <Aside>
    Domain verification widget visibility depends on the `domain_verification` feature being enabled for the organization. You can enable it in the Scalekit dashboard or via the [organization settings API](/authenticate/manage-users-orgs/organization-domains/).

    Access is controlled by Scalekit permissions: `sk_org_settings_read` to view domains, `sk_org_settings_write` to add, verify, or delete domains.
    </Aside>
...
5. ### Configure SCIM for the organization
