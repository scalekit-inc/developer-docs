...
To integrate Hosted UI widgets, redirect your organization members to the portal URL:
...
If you need a **shareable link** or an **iframe embed** so enterprise IT can configure SSO and SCIM, use the [Admin portal](/guides/admin-portal/) instead. Hosted UI widgets are the better fit when signed-in users in your SaaSKit app open self-serve settings (members, profile, security, domains, session policy, and enterprise setup) from inside your product.
...
If no session exists, Scalekit redirects the user to your application's login endpoint so your app can start the authentication flow again. Send users to `/ui/` only after they complete your [SaaSKit authentication flow](/authenticate/fsa/quickstart/) so a Scalekit session is available.
...
When a user logs out from Hosted UI widgets, they are redirected to your configured post-logout destination. This can leave your app session and the Scalekit session out of sync.

If you manage sessions in your own backend instead of using Scalekit's default session management, handle logout propagation yourself. See the [SaaSKit quickstart](/authenticate/fsa/quickstart/) and [manage sessions](/authenticate/fsa/manage-session/) guides, and use one of the following approaches:

- Implement [back-channel logout](/guides/dashboard/redirects/#back-channel-logout-url) so Scalekit can notify your app about session termination.
- Listen for the [user logout webhook](/apis/#webhook/userlogout) to get notified about session termination.
...
Redirect users to `/ui/` for the Hosted UI widgets portal. For a shareable link or an iframe embed focused on enterprise SSO and SCIM setup, use the [Admin portal](/guides/admin-portal/).
