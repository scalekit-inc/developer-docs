...
4. ## Group-based role assignment
   ...
   - Roles are revoked when users are removed from those groups, including when the group itself is deleted in the directory
   - If a user belongs to multiple mapped groups, deleting one group recalculates the user's role from their remaining group memberships instead of leaving the deleted group's role in place
