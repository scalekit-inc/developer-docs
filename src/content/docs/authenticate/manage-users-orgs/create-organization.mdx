...
[Scalekit integration](/authenticate/fsa/quickstart/) that's set up to autheticate users and complete login flow automatically creates an organization for them. Both the ID token and access token will contain the organization ID that the user belongs to.
...
## User creates organizations themselves <Badge text="API" variant="tip" />

Applications often provide options for users to create their own organizations. For example, show an option for users such "Create new workspace" within their app.

You can use the Scalekit SDK to power such options in the app's UI.
...
When a user belongs to multiple organizations, subsequent login flows automatically present a organization selection interface, allowing users to choose their desired organization.
