...
Organization-specific roles are custom roles that apply only within a single organization. Organization-specific roles are separate from standard roles your application defines for all customers.

For example, define a "Squad Lead" role with permissions to close tasks. Your application receives the permissions context through the Scalekit access token.
