...
sidebar:
  label: Using auth interceptors
tableOfContents:
  minHeadingLevel: 2
  maxHeadingLevel: 3
browseCentral:
  label: "Custom auth workflows with interceptors"
  filterType: ["tutorial"]
  category: ["Customize"]
  icon: book
prev:
...
5. ### Access custom claims in your application
   ...
   After authentication, the custom claims are included in both the access token and ID token. You can access them by decoding the tokens:
