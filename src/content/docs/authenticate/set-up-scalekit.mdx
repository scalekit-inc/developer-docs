...
tags: [setup, sdk, installation, environment, getting-started, authentication]
...
![Windsurf editor with the Cascade chat panel answering a question using indexed Scalekit documentation](@/assets/docs/dev-kit/ai-assisted-setup/windsurf.png)
...
<ProductAgentBlock
  ...
  syncKey="setup-scalekit-cli"
/>

Change the skill name if you are adding a different product:

| Product | Example prompt |
| ------- | -------------- |
| AgentKit | `Use integrating-agentkit to connect this agent to GitHub.` |
| MCP authentication | `Use adding-mcp-oauth to add OAuth 2.1 to this MCP server.` |
| Single sign-on (SSO) | `Use implementing-modular-sso to add SAML and OIDC SSO.` |
| SCIM | `Use implementing-scim-provisioning to add user provisioning.` |

See the [Build with AI guide](/dev-kit/build-with-ai/) for the full setup.
