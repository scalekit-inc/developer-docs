---
title: Configure Magic Link & OTP
description: Enable Magic Link & OTP sign-in with email verification codes or magic links. Configure security settings and expiry periods.
sidebar:
...
After completing the [quickstart guide](/authenticate/fsa/quickstart/), configure Magic Link & OTP to let users sign in without passwords. Users verify their identity using email verification codes or magic links.

Switch between Magic Link & OTP methods seamlessly without modifying any code:
...
## Regenerate Magic Link & OTP credentials on resend

When a user requests a new Magic Link & OTP credential, the system generates a fresh code or link while automatically invalidating the previous one. This approach is recommended for all applications as a critical security measure to prevent potential misuse of compromised credentials.
