...
4. ## Identify and enforce SSO for organization users
    ...
    **Organization domains** automatically route users to the correct SSO connection based on their email address. When a user signs in with an email domain that matches a registered organization domain, Scalekit redirects them to that organization's SSO provider and enforces SSO login.
    ...
    For example, if an organization registers `megacorp.org`, any user signing in with an `@megacorp.org` email address is redirected to Megacorp's SSO provider.
    ...
    <Aside>
    When implementing [domain-based discovery mechanisms like `domain_hint`](/authenticate/fsa/implement-login/#add-state-parameter:~:text=Optional-,login_hint,-Pre%2Dpopulate%20the), light weight validation step to check if the [domain has an active SSO connection](https://docs.scalekit.com/apis/#tag/connections/GET/api/v1/connections) to prevent unnecessary redirect loops
    </Aside>
...
## Keep user data current with automatic attribute sync

Scalekit automatically syncs user profile attributes from your identity provider each time a user authenticates. User information stays current across both systems without manual updates.

![](@/assets/docs/enterprise-sso/user-profile-current.png)

The organization administrators can enable this feature in the admin portal from Admin Portal > Single Sign On > Settings > Sync user attributes during login