...
from scalekit.v1.users import CreateUser
...
user_response = scalekit_client.user.create_user_and_membership(
  org_id,
  CreateUser(email="john+sktest@acmecorp.com", send_invitation_email=False),
)
```
  </TabItem>
  <TabItem label="Go">
```go title="Create test user in an organization"
import usersv1 "github.com/scalekit-inc/scalekit-sdk-go/pkg/grpc/scalekit/v1/users"
