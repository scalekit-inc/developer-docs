...
<details>
<summary>Why did my test user not get the invitation email?</summary>

Test Users only skip OTP and magic-link emails. Invitation emails still go to the address you enter.

If that address has no inbox, the invite never arrives and the user stays **Pending Invite**.

Uncheck **Send invitation email** when you create the user in **Dashboard > Organizations > Users > Create user**. Or pass `sendInvitationEmail: false` in the API, as shown in [Test the login flow](#test-the-login-flow).
