...
Each organization in your app will have multiple users. Many times it's the collaboration between users maybe the most important part of your project. For instance, if Slack is your app, you can have multiple users in an organization and they can communicate with each other.
...
Your [integration](/authenticate/fsa/quickstart) with Scalekit enables two additional user provisioning methods:
...
You must first implement and register the [initiate login endpoint](/guides/dashboard/intitate-login-endpoint/) in your application and Scalekit dashboard.
</Aside>