...
- A new organization exists with the test user as a member. Verify in **Dashboard > Organizations > \{org\} > Users**.
...
Use this flow to verify login for a user that already exists in an organization. The test user must be created before the test runs. Add them manually in **Dashboard > Organizations > \{org\} > Users > Create user**, or create them programmatically with the Scalekit API or SDK:
...
Every allowlisted email must contain `+sktest`, which makes test identities easy to spot in logs and audit records.
</Aside>
