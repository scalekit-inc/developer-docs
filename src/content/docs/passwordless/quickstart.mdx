---
title: Magic link & OTP email headless API
...
browseCentral:
  ...
  icon: book
sidebar:
  label: 'Build Magic link & OTP auth'
badge:
   text: 'Headless'
   variant: 'default'
banner:
...
This guide explains how you can implement Magic link & OTP authentication using Scalekit's headless APIs to send either verification codes (OTP) or magic links to your user's email address and verify their identity. Magic link & OTP provides two email-based authentication methods—clickable links or one-time passcodes—allowing users to authenticate without passwords. You have full control over all UI components and user flows in your application, while Scalekit provides the backend authentication infrastructure.
...
<Card title="Get moving, instantly, with your go-to AI assistant" icon={IconRiAiGenerate2}>
  <div class="llm-prompt-card">
    <div class="llm-prompt-content">
      <p>Input this prompt in your IDE to analyze your existing code base and generate Magic link & OTP implementation code accordingly.</p>

      <span class="llm-prompt-subtitle">
        *Compatible with Cursor, Windsurf, VS Code, and any AI-powered tools
      </span>
    </div>
    <div class="llm-prompt-action">
      <CopyPrompt
        promptType="headlessPasswordlessQuickstartPrompt"
            label="Copy llm prompt"
        className="sk-secondary"
        variant="secondary"
        title="Generate Magic link & OTP integration code"
      />
    </div>
  </div>
</Card>
...
2. ## Configure Magic link & OTP settings
...
3. ## Send verification email
    ...
    The first step in the Magic link & OTP flow is to send a verification email to the user's email. This verification email contains either a **one-time passcode (OTP) or a magic link or both** based on your selection in the Scalekit dashboard (earlier step).

    Follow these steps to implement the verification email flow:

    1. Create a form to collect the user's email address
    2. Call the Magic link & OTP API when the form is submitted
   ...
   <TabItem label="cURL" value="curl">
    ...
    <details>
      ...
      | `passwordless_type` | The type of Magic link & OTP authentication to use. Currently supports `OTP`, `LINK` and `LINK_OTP` <Badge variant="note" text="string" /> |
    </details>
   ...
   <TabItem label="Node.js" value="node">
    ...
    <details>
      ...
      | Parameters | Description |
      |-----------|-------------|
      | `authRequestId` | Unique identifier for the Magic link & OTP authentication request <Badge variant="note" text="string" /> |
      ...
      | `passwordlessType` | Type of Magic link & OTP authentication (`OTP`, `LINK` or `LINK_OTP`) <Badge variant="note" text="string" /> |
...
3. ### Resend a verification email
   ...
   ```
    ...
    <Aside type="caution" title="Rate limits">
      Scalekit enforces a rate limit of 2 Magic link & OTP emails per minute per email address. This limit includes both initial sends and resends.
    </Aside>
       ...
       <Tabs syncKey='tech-stack'>
         ...
         <TabItem label="Node.js" value="node">
           ...
           <details>
             ...
             | `state` | The state parameter that was passed in the original request <Badge variant="note" text="string" /> |
             | `template` | The template that was used for the verification code <Badge variant="note" text="string" /> |
             | `passwordlessType` | The type of Magic link & OTP authentication to use. Currently supports `OTP`, `LINK` and `LINK_OTP` <Badge variant="note" text="string" /> |
           </details>
         </TabItem>
         <TabItem label="Python" value="python">
           ```python
