---
title: Headless email API for magic link and OTP
description: Implement email OTP or magic link using direct API calls with full control over UX
tags: [passwordless, quickstart, magic-link, email-otp, headless-api, authentication, users]
browseCentral:
  label: "API to send magic link or OTP verifications"
  filterType: ["tutorial"]
  category: ["User authentication"]
  icon: book
sidebar:
  label: 'Headless magic link & email OTP'
badge:
   text: 'Headless'
   variant: 'default'
head:
...
import { Aside, Badge, LinkCard, Steps, TabItem } from '@astrojs/starlight/components';
...
2. ## Configure magic link and OTP settings
   ...
   - **Issue new credentials on resend**: Each resend generates a fresh code or link and invalidates the previous one.
    ...
    ![Scalekit dashboard Magic link & OTP settings showing the login method selection and expiry period fields](@/assets/docs/fsa/auth-methods/1.png)
