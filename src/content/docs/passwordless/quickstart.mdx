...
import { Aside, Badge, Card, LinkCard, Steps, TabItem, Tabs } from '@astrojs/starlight/components';
...
2. ### Send verification email
   ...
   **Example implementation**
   ...
   <Tabs syncKey='tech-stack'>
    ...
    resp, err := client.Passwordless().SendPasswordlessEmail(
      ctx,
      "john.doe@example.com",
...
3. ### Resend a verification email
   ...
   <Tabs syncKey='tech-stack'>
    ...
    resendResp, err := client.Passwordless().ResendPasswordlessEmail(
       ...
       <Tabs syncKey='tech-stack'>
         ...
         <TabItem label="Node.js">
           ...
           <TabItem label="Go">
             ...
             verifyResp, err := client.Passwordless().VerifyPasswordlessEmail(
               ctx, // context.Context
               &scalekit.VerifyPasswordlessOptions{
                 Code:          "123456",     // OTP code entered by the user
                 AuthRequestId: authRequestId, // from send response
               },
             )

             if err != nil {
               // handle invalid/expired code or other errors
               // ...
             }
     ...
     <TabItem label="Magic Link Verification">
       ...
       **Example implementation**
       ...
       <Tabs syncKey='tech-stack'>
         ...
         <TabItem label="Go">
           ```go
           // Inside your HTTP handler: capture link_token from query, then verify
           linkToken := linkTokenFromQuery // e.g., r.URL.Query().Get("link_token")

           verifyResp, err := client.Passwordless().VerifyPasswordlessEmail(
             ctx, // context.Context
             &scalekit.VerifyPasswordlessOptions{
               LinkToken: linkToken, // Magic link token from the URL
               // Optionally include AuthRequestId if same-origin is enforced
               // AuthRequestId: authRequestId,
             },
           )

           if err != nil {
             // handle invalid/expired link or other errors
             // ...
           }

           // verifyResp contains the verified user's info (email, state, template, passwordless type)
           ```
           ...
           <details>
             ...
             |-----------|----------|-------------|
             | `options.LinkToken` | Yes | The link token received by the user <Badge variant="note" text="string" /> |
             | `options.AuthRequestId` | No | The unique identifier for the authentication request that was sent earlier. Required if same-browser-origin enforcement is enabled <Badge variant="note" text="string" /> |
           </details>
