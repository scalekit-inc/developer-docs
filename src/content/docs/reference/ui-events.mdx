...
Fires when the admin portal session is created and loaded successfully. Use this event to display the portal iframe and confirm readiness to users.

```json title="PORTAL_LOAD_SUCCESS"
{
  "event_type": "PORTAL_LOAD_SUCCESS",
  "object": "session",
  "message": "The admin portal loaded successfully",
...
Fires when the admin portal session failed to load. Use this to prompt users that the session has failed to load.

```json title="PORTAL_LOAD_FAILURE"
{
  "event_type": "PORTAL_LOAD_FAILURE",
  "object": "session",
  "message": "The admin portal failed to load",
  "data": {
    "error_code": "SESSION_EXPIRED" // error code indicating why the session load failed
  }
}
```
...
| `object` | string | The object type associated with the event |
| `message` | string | Human-readable message describing the event |
| `data.error_code` | string | Error code indicating why the session load failed |
...
function initAdminPortalEventHandling(scalekitEnvironmentUrl) {
  window.addEventListener('message', (event) => {
    ...
    switch (event_type) {
      ...
      case 'PORTAL_LOAD_SUCCESS':
        handlePortalLoadSuccess(data.expiry);
        break;

      case 'PORTAL_LOAD_FAILURE':
        handlePortalLoadFailure(data.error_code);
        break;
...
function handlePortalLoadSuccess(expiryTime) {
  const expiryDate = new Date(expiryTime);
  console.log('[Admin Portal] Loaded successfully, session expires at', expiryDate);

  // Update UI to show the portal is ready
  document.getElementById('admin-portal-iframe').style.display = 'block';
}

function handlePortalLoadFailure(errorCode) {
  console.error('[Admin Portal] Failed to load, error code:', errorCode);

  // Hide the iframe and show an error message to the user
  document.getElementById('admin-portal-iframe').style.display = 'none';

  showModal({
    title: 'Portal failed to load',
    message: errorCode === 'SESSION_EXPIRED'
      ? 'Your session has expired. Please refresh to continue.'
      : `The admin portal could not be loaded (${errorCode}). Please try again.`,
    action: {
      label: 'Refresh Page',
