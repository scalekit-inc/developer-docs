...
sidebar:
  label: "Understand user profiles"
head:
  - tag: style
    content: |
...
When a user signs in using any authentication method—whether it's enterprise SSO, social login, or passwordless—Scalekit normalizes the profile information into a standard, consistent structure. This simplifies development by ensuring you always receive user data in a predictable format, regardless of the underlying identity provider.

This unified approach means you only need to write your identity-related logic once.
