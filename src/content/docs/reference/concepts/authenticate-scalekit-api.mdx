...
import { Aside, Steps, LinkButton, Tab, TabItem } from '@astrojs/starlight/components';
...
Scalekit's token endpoint lets you generate an access token for your application.

```url "/oauth/token" title="Token Endpoint" showLineNumbers=false
https://<your-subdomain>.scalekit.dev/oauth/token
```

A successful request format is essential for your application to get authenticated API access token.
