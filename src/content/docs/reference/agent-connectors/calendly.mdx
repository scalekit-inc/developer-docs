...
List activity log entries for an organization with filtering by actor, action type, time range, and search term. NOTE: Requires Calendly Enterprise plan. Returns audit log events including login activity, resource creation, updates, and deletions.
