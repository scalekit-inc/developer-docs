...
import { SetupMiroSection, UsageMiroSection } from '@components/templates'
...
<Aside type="note" title="Enterprise-only tools">
  Several Miro tools require an Enterprise plan: `miro_audit_logs_get`, `miro_board_export_*`, `miro_data_classification_*`, `miro_org_*`, `miro_project_*`, `miro_team_*`, and `miro_teams_list`. These will return a `403 Forbidden` error if your Miro account is not on the Enterprise plan.
</Aside>

<Aside type="note" title="Experimental API">
  The mind map tools (`miro_mindmap_*`) use Miro's experimental API and may change or be removed without notice.
</Aside>
