...
head:
  ...
  - tag: script
    content: |
      ...
      (function() {
        if (window.__pipedriveHeadingCopyInitialized) return;
        window.__pipedriveHeadingCopyInitialized = true;
          ...
          headings.forEach(heading => {
            ...
            heading.addEventListener('click', async () => {
              ...
              try {
                ...
                // Visual feedback — use CSS class to avoid mutating textContent
                // (mutating textContent removes Starlight's anchor link children)
                heading.classList.add('copy-feedback');

                setTimeout(() => {
                  heading.classList.remove('copy-feedback');
                }, 2000);
              } catch (err) {
