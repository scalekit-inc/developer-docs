...
- `roles`: Specifies the user's role within that organization
...
This feature is currently in private beta. [Contact us](/support/contact-us) to enable it for your account.
</Aside>
...
If you already have a membership table that stores the relationship between users and organizations, you can add the Scalekit `user_id` to that table. User records are shared across organizations at your Scalekt environment, so when you update a user record, the changes apply across all organizations.
