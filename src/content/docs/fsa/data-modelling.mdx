...
<Aside type="note" title="How this all connects">
Every user has a `memberships` array. Each membership record represents that user's relationship
to a specific organization—their roles, status, and metadata in that context. This structure
enables the flexible 1-to-many model that powers modern B2B applications.
</Aside>
