...
4. #### Create a session and log in the user
      ...
      This sets browser cookies with the session tokens. Every request to your backend needs to verify the `accessToken` to ensure the user is authenticated. If expired, use the `refreshToken` (store it secure database) to get a new access token.

      <Tabs syncKey="tech-stack">
        <TabItem value="node" label="Node.js">
        ```javascript collapse={14-25, 40-55}
        ...
        const verifyToken = async (req, res, next) => {
          try {
            ...
            const isValid = await scalekit.validateToken(accessToken);

            if (isValid) {
              // Token is valid, decode and attach user info to request
              const decoded = jwt.decode(accessToken);
              req.user = decoded;
              return next();
            }

            // You would validate the refresh token against your database
              const storedRefreshToken = await db.refreshTokens.findOne({ token: refreshToken });
              if (!storedRefreshToken || storedRefreshToken.isRevoked) {
                return res.status(401).json({ message: 'Invalid refresh token' });
              }

            // Use Scalekit SDK to refresh the token using refreshToken (store in secure database)
            const newTokens = await scalekit.refreshToken(refreshToken);

            // Set new access token in cookie
            res.cookie('accessToken', newTokens.access_token, {
              maxAge: (newTokens.expires_in - 60) * 1000, // Subtract 60 seconds for safety margin
            ...
            await db.refreshTokens.updateOne(
              { token: refreshToken },
              { $set: { token: newTokens.refresh_token, updatedAt: new Date() } }
            );
            }

            // Attach user info from new token to request
            const decoded = jwt.decode(newTokens.access_token);
            req.user = decoded;
            next();
          } catch (error) {
