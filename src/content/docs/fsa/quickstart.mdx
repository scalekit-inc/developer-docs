...
3. ## Handle the authentication callback
   ...
   <TabItem value="node" label="Node.js">
      ...
      - `user` - Complete user profile with email, name, and verification status
      - `idToken` - JWT containing verified user identity claims
      ...
      <CheckItem>
        Validate incoming requests by checking for errors before processing the authorization code
      </CheckItem>
      <CheckItem>
        Store tokens securely and never expose them to frontend code or URLs
      </CheckItem>
      <CheckItem>
        Use the [token explorer tool](https://authplay.io/pg/token-explorer) to inspect and debug token contents during development
      </CheckItem>
