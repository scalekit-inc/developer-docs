...
Your application continuously validates the access token for each incoming request. When the token is valid, the user's session remains active. If the access token expires, your app attempts to refresh it using the stored refresh token. However, if token refresh fails, the user is automatically logged out, effectively terminating their session.