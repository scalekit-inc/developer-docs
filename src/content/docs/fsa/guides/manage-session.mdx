...
<Steps>
1. ## Keep users signed in: include _offline_access_

    Scalekit provides session management capabilities out of the box. Your application receives an `access_token` and `refresh_token` in the authentication response.

    Include `offline_access` in the list of scopes sent during the authorization
    request to Scalekit to receive a `refresh_token`. This token allows you to get new access tokens without requiring users to sign in again.

    Create an authorization URL with offline_access scope:

    <Tabs syncKey="tech-stack">
    <TabItem value="node" label="Node.js">
    ```javascript wrap title="Express.js" {2,6-8,10} "offline_access"
    // Must match the callback URL you registered in your Scalekit dashboard
    ...
    ```python wrap title="Flask" {1,4,8-10,12} "offline_access"
    ...
    ```go wrap title="Gin" {2,6-8,10} "offline_access"
    // Must match the callback URL you registered in your Scalekit dashboard
    ...
    ```java wrap title="Spring" {1-3,6,10-11,13} "offline_access"
    ...
    This redirects users to Scalekit's managed sign-in page where they can authenticate. The `offline_access` scope ensures you receive a **refresh token**.


2. ## Store session tokens securely (after authentication)

    After successful user authentication using any of the auth methods (passwordless, social, enterprise SSO), your application receives session tokens.
   ...
   <Tabs syncKey="tech-stack">
     <TabItem value="node" label="Node.js">
     ```javascript title="Express.js" showLineNumbers=true  collapse={1-4} { "1": 5-6 } { "2": 8-10 } { "3": 12-19 }
     ...
     <TabItem value="py" label="Python">
     ```python title="Flask" collapse={1-4} { "1": 5-9 } { "2": 12-13 } { "3": 15-24 }
...
3. ## Check the access token before handling requests
   ...
   <Tabs syncKey="tech-stack">
     ...
     <TabItem value="py" label="Python">
     ```python title="middleware/auth.py" wrap collapse={1-2}
...
5. ## Handling post-login redirects​ <Badge text="Optional" />
   When users visit specific routes like `/dashboard/analytics` without a valid session, your application detects the missing session and initiates the OAuth flow. After successful authentication, Scalekit redirects users to your callback URL, but they need to be redirected back to their original destination.

   Implement the **post-login redirect pattern** using cookies to preserve the user's intended destination through the authentication flow. This approach is more secure than using the `state` parameter, which should be reserved for CSRF protection.
   ...
   <Tabs syncKey="tech-stack">
     ...
     async function verifyToken(req, res, next) {
       ...
       if (!accessToken) {
         ...
         res.cookie('returnUrl', req.originalUrl, { /* secure cookie config */ });
         const authorizationUrl = scalekit.getAuthorizationUrl(redirectUri, options);
       ...
       try {
         const isValid = await scalekit.validateAccessToken(accessToken);
         if (!isValid) {
           // Same redirect logic for invalid tokens
           res.cookie('returnUrl', req.originalUrl, { /* secure cookie config */ });
           const authorizationUrl = scalekit.getAuthorizationUrl(redirectUri, options);
           return res.redirect(authorizationUrl);
         }
         ...
         // Same redirect logic for validation errors
         res.cookie('returnUrl', req.originalUrl, { /* secure cookie config */ });
         const authorizationUrl = scalekit.getAuthorizationUrl(redirectUri, options);
         return res.redirect(authorizationUrl);
       }
     ...
     def verify_token(f):
         ...
         def decorated_function(*args, **kwargs):
             ...
             if not access_token:
                 # Store current URL for post-login redirect
                 response.set_cookie('returnUrl', request.url, { /* secure cookie config */ })
                 authorization_url = scalekit.get_authorization_url(redirect_uri, options)
                 return redirect(authorization_url)
     ...
     func VerifyToken() gin.HandlerFunc {
       return func(c *gin.Context) {
         ...
         if err != nil || accessToken == "" {
           // Store current URL for post-login redirect
           c.SetCookie("returnUrl", c.Request.URL.String(), { /* secure cookie config */ })
     ...
     <TabItem value="java" label="Java">
     ```java title="middleware/AuthInterceptor.java"
     @Override
     public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
       ...
       if (accessToken == null) {
         // Store current URL for post-login redirect
         Cookie returnUrlCookie = new Cookie("returnUrl", request.getRequestURL().toString());
   ...
   Handle the `returnUrl` cookie in your callback endpoint to redirect users back to their original destination:

   <Tabs syncKey="tech-stack">
     <TabItem value="node" label="Node.js">
     ```javascript title="routes/auth.js" {3,7-9,11}
     // Handle OAuth callback and redirect to original destination
     app.get('/auth/callback', async (req, res) => {
       ...
       try {
         ...
         // ... store tokens securely (same as step 2)

         // Redirect to original destination or default page
         const returnUrl = req.cookies.returnUrl || '/dashboard';
         res.clearCookie('returnUrl'); // Clean up the cookie
     ...
     <TabItem value="py" label="Python">
     ```python title="routes/auth.py" {3,7-9,11}
     # Handle OAuth callback and redirect to original destination
     ...
     def callback():
         ...
         try:
             # Exchange authorization code for tokens
             auth_result = scalekit.authenticate_with_code(code=code, redirect_uri=redirect_uri)
             # ... store tokens securely (same as step 2)

             # Redirect to original destination or default page
             return_url = request.cookies.get('returnUrl', '/dashboard')
             response.set_cookie('returnUrl', '', expires=0)  # Clean up the cookie
     ...
     <TabItem value="go" label="Go">
     ```go title="routes/auth.go" {3,7-9,11}
     // Handle OAuth callback and redirect to original destination
     r.GET("/auth/callback", func(c *gin.Context) {
       code := c.Query("code")

       // Exchange authorization code for tokens
       authResult, err := scalekitClient.AuthenticateWithCode(code, redirectUri)
       if err != nil {
       ...
       // Redirect to original destination or default page
       returnUrl := "/dashboard"
       if cookie, err := c.Cookie("returnUrl"); err == nil && cookie != "" {
         returnUrl = cookie
       }
       c.SetCookie("returnUrl", "", -1, "/", "", false, true) // Clean up the cookie
       c.Redirect(http.StatusFound, returnUrl)
     })
     ```
     </TabItem>
     <TabItem value="java" label="Java">
     ```java title="AuthController.java" {3,7-9,11}
     // Handle OAuth callback and redirect to original destination
     @GetMapping("/auth/callback")
     public String callback(@RequestParam String code, HttpServletRequest request, HttpServletResponse response) {
         try {
             // Exchange authorization code for tokens
             AuthResult authResult = scalekitClient.authentication().authenticateWithCode(code, redirectUri);
             // ... store tokens securely (same as step 2)

             // Redirect to original destination or default page
             String returnUrl = getCookieValue(request, "returnUrl");
             if (returnUrl == null || returnUrl.isEmpty()) {
                 returnUrl = "/dashboard";
             }

             // Clean up the returnUrl cookie
             Cookie clearCookie = new Cookie("returnUrl", "");
   ...
   <Aside type="tip" title="Post-login redirect pattern">
   The `returnUrl` cookie preserves the user's intended destination through the OAuth flow. When users visit `/dashboard/analytics` without a session, they're redirected to Scalekit for authentication, then automatically returned to `/dashboard/analytics` after successful login. This approach is more secure than using the `state` parameter, which should be reserved for CSRF protection.
   </Aside>
...
## Handling indirect auth entry points

Sometimes users access your application through indirect entry points rather than the standard authentication flow. These scenarios require your application to detect the lack of authentication and initiate the OAuth flow appropriately.
...
For these scenarios, implement a dedicated login endpoint that initiates the OAuth flow. This endpoint should be registered as one of the initiate login redirects in your Scalekit dashboard > Authentication > Redirects.
...
<Aside type="note" title="Dashboard configuration">
Register `https://www.yourapp.com/login` as one of the [initiate login redirects](/guides/dashboard/redirects/#initiate-login-url) in your Scalekit dashboard. This ensures Scalekit recognizes this endpoint as a valid entry point for authentication flows when users access it directly.
</Aside>
