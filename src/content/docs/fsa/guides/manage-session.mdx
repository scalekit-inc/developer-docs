...
<Steps>
1. ### Verify user identity through auth methods
    ...
    **Create an authorization URL with offline_access scope:**
    ...
    This redirects users to Scalekit's managed sign-in page where they can authenticate. The `offline_access` scope ensures you receive a refresh token for seamless session management.


2. ### Store session tokens securely (after authentication)

    After successful user authentication using any of the auth methods (passwordless, social, enterprise SSO), your application receives session tokens in the response object.
