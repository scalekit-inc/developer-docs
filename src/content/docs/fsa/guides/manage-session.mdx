...
1. ## Store session tokens securely
   The response object contains an `accessToken`, `idToken`, and an optional `refreshToken`. These should be stored securely.
    ...
    You can configure session lifetimes, idle timeouts, and other security settings from the [Session Configuration](/fsa/reference/session-configs/) section in your Scalekit dashboard.
