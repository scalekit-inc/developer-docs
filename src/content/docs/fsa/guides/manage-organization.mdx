...
Organizations in Scalekit work for different product architectures:

**Multi-tenant identity**: Users can belong to multiple organizations with the same identity. This is common in products like Notion, where users collaborate across multiple workspaces.

**Isolated identity**: Users maintain separate identities for each organization. This approach is typical for employee survey tools or other applications where strict tenant isolation is required.

<Aside type="note">
  You can customize the terminology to match your product. Organizations can be relabeled as "Workspaces," "Teams," or any term that makes sense for your users.
</Aside>
...
For detailed configuration guides with specific providers, see [SCIM integrations](/guides/integrations/scim-integrations/).
...
- [Set up SCIM provisioning](/guides/directory/scim/) to automate user and group management