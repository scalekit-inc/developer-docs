...
browseCentral:
  label: "Build with Scalekit hosted signup page"
  filterType: ["tutorial"]
...
<Steps>
 1. ### Install the Scalekit SDK
  ...
  2. ### Redirect the user to the sign up page
       Generate the authorization URL by passing a [registered](/guides/dashboard/redirects/#allowed-callback-urls) _callback URL_ and _scopes_ to the Scalekit SDK.
       <RedirectAuthPageSection />
     This will redirect the user to Scalekit's managed sign-in page.

3. ### Allow users to sign up
      Users can now sign in or sign up using their preferred authentication methods. Each authentication method is managed by Scalekit to successfully verify the user's identity.
...
After the user signs up, they are redirected back to your application, and you can proceed to [create a session](/authenticate/fsa/manage-session/).