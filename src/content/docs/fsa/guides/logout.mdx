...
3. #### Invalidate the Scalekit session
   ...
   - `{postLogoutRedirectUri}` is the URL to redirect to after logout (must be registered in Scalekit).
    ...
    The post logout redirect URI is the destination to which the user will be redirected after logging out. For example, take them to website landing page or a login page again. This must be [registered post-login URL](/guides/dashboard/redirects/) in the Scalekit dashboard.
