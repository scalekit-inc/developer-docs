...
After implementing session management for your authenticated users, you'll need a secure logout process to properly terminate user sessions. This completes the authentication journey by ensuring both your application and Scalekit properly clear session data.

A complete logout process involves two main actions:
...
## IdP-initiated logout <Badge text="Coming Soon" variant="note" />
...
<Aside>
  Your application must expose a webhook endpoint to receive logout notifications from Scalekit.This feature is currently available upon request. [Contact our support team](/support/contact-us) to have this feature enabled for your account.
</Aside>