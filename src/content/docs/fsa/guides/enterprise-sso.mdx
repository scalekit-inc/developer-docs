...
3. ### Understanding the authorization flow
    ...
    **Scalekit hosted UI**: If you don't provide these identifiers, users will see Scalekit's hosted authentication page. By default, this shows Google OAuth and passwordless login (magic link + verification code) options. Enterprise SSO connection is enabled and functions only if the organization is configured for the user's email domain.
...
Scalekit automatically redirects users to their organization's identity provider for SSO authentication.

You can enable this capability in two ways:

1. **Organization admins** can add domains through the admin portal for users who will sign in with those domains.
2. **You** can add domains directly from the Scalekit dashboard for specific organizations as requested by your customer's IT admin.
