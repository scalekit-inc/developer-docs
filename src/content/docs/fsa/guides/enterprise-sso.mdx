...
When organization admins verify organization domains, users can sign in directly through their organization's identity provider from the sign-in page. Scalekit uses the domain part of their email address (for example, `@megacorp.org`) to automatically redirect them to the correct identity provider for authentication. Also called Home Realm Discovery. You can add a verified domain manually on behalf of your customer from the Scalekit Dashboard -> Organizations -> Select an Organization -> Single Sign On
...
#### Pre-check SSO by domain (when using discovery)

If your login form uses discovery (`domain_hint` or `login_hint`), run a lightweight check to confirm the user’s email domain maps to an active SSO connection before redirecting. This prevents dead-end redirects and helps you present the right path (SSO vs non‑SSO).
...
curl 'https://<SCALEKIT_ENVIRONMENT_URL>/api/v1/connections?domain=megacorp.org'
```

```js title="Example (pseudocode)" showLineNumbers=false
const domain = email.split('@')[1].toLowerCase();
const connections = await scalekit.connections.listConnectionsByDomain({ domain });

if (connections.length > 0) {
  // Proceed with SSO (domain_hint/login_hint or connectionId)
} else {
  // Fallback: passwordless, magic link, or other methods
}
```

<Aside>
This check returns results only when the domain is configured for an organization with an enabled SSO connection. See the <a href="https://docs.scalekit.com/apis/#tag/connections/get/api/v1/connections" target="_blank" rel="noopener">connections API reference</a>.
</Aside>
