---
title: "Invite users to application workspaces"
description: "Invite from UI or API. Scalekit sends, verifies, and creates accounts so orgs add users fast"
sidebar:
  label: "Build user invite access"
head:
  - tag: style
    content: |
...
<Steps>
 ...
 4. ### Handle user invitation acceptance
    ...
    When invited users click the invitation link in their email, Scalekit redirects them to your application's [registered initiate login endpoint](/guides/dashboard/redirects/#redirect-endpoint-types). Your application then completes the authentication flow.
    ...
    **Set up the initiate login endpoint:**
    ...
    2. **Handle the redirect** by [constructing an authorization URL](/fsa/quickstart) and redirecting the user to Scalekit's hosted login page
