...
User profiles in Scalekit are automatically generated when users sign up for an app. However, managing these profiles can be complex because users may be members of multiple organizations (also referred to as workspaces) and may hold distinct roles and permissions within each workspace. This setup requires differentiated access controls to various features of the application, based on the specific organization and associated user role.

<details>
<summary><IconMaterialSymbolsArticlePersonRounded style="display: inline; width: 1rem; height: 1rem; vertical-align: middle; margin-right: 0.5rem;" /> View user data model</summary>
