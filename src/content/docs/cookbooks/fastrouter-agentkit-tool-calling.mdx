...
const { connectedAccount } = await scalekit.actions.getOrCreateConnectedAccount({
  connectionName: process.env.SCALEKIT_CONNECTION_NAME!,
  identifier: 'user_123',
  userVerifyUrl,
});
...
connection_name = os.environ.get("SCALEKIT_CONNECTION_NAME")
...
const { tools } = await scalekit.tools.listScopedTools('user_123', {
  filter: { connectionNames: [process.env.SCALEKIT_CONNECTION_NAME!] },
  pageSize: 100,
});
...
for (let turn = 0; turn < 8; turn++) {
  ...
  for (const call of message.tool_calls) {
    const result = await scalekit.actions.executeTool({
      ...
      identifier: 'user_123',
      connector: process.env.SCALEKIT_CONNECTION_NAME!,
