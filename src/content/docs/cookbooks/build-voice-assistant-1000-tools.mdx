---
title: 'Build a voice assistant with Vapi + Scalekit (1000+ tools)'
...
sidebar:
  label: 'Vapi + Scalekit: 1000+ tools'
...
Voice interfaces shine for hands-free, conversational access to data and actions. But giving a voice agent access to real tools (Gmail, Calendar, Slack, GitHub, Drive, CRM, Notion, and 1000+ connectors) introduces two hard problems: per-user authentication without leaking tokens to the LLM, and managing a massive tool surface without overwhelming context windows or exposing everything to every user.
...
This cookbook walks through building a [Vapi](https://vapi.ai) voice assistant that safely surfaces thousands of tools using Scalekit's tool registration + Virtual MCP. You can also call specific registered tools directly via Vapi Function tools if you don't need dynamic discovery of many.
...
**MCP-specific vars** (`SCALEKIT_MCP_CONFIG_ID` + `NEXT_PUBLIC_SCALEKIT_MCP_SERVER_URL`) are only needed when using the dynamic discovery MCP tool path (recommended for 1000+ tools). They are not required for pure Function tool + `executeTool` mode.
...
3. Add connection tool mappings for the connectors you want. Example for calendar + email (expand later with Slack, GitHub, Drive, etc.):
...
The assistant should use the discovered tools, execute them via Scalekit (with your identity), and speak the results. Mix and match connectors that you've included in your Virtual MCP config.
...
A sales rep might get "Gmail + Calendar + Salesforce + Slack" (8-12 tools). An engineer might get "GitHub + Drive + Slack + Gmail". Both use the *same* underlying 1000+ tool catalog, but each voice session only sees what that person is allowed to do.
