...
Scalekit AgentKit with Virtual MCP solves both. You define one scoped MCP server per agent role that exposes only the tools that role needs. At runtime you mint a short-lived, user-scoped token. Vapi (or other MCP clients) connects directly, discovers the allowed tools, and calls them with proper identity — no custom webhook bridge, no per-connector OAuth code in your app.

This cookbook walks through building exactly that: a Vapi voice assistant backed by a Scalekit Virtual MCP that safely surfaces thousands of tools.
...
- **Dynamic tool discovery** via a Scalekit Virtual MCP — the assistant only sees the tools you explicitly scoped for that role.
...
## Step 1: Create a scoped Virtual MCP in Scalekit
