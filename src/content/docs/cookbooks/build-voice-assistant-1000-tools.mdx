...
3. Configure it with the Scalekit Virtual MCP details:
   ...
   - **HTTP Headers** (in the Headers / Custom Headers / server.headers section of the tool form):
     - Key: `Authorization`
...
You now have a voice agent that can discover tools dynamically from the scoped MCP.

**Alternative for specific tools**: Instead of (or in addition to) an MCP tool, you can create Vapi **Function** tools that call your webhook, which then uses `scalekit.actions.executeTool` for individual registered tools (no vMCP needed). This is great for a small number of tools.
