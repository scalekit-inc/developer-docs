...
3. Go to **APIs & Services → OAuth consent screen**. Set the **User type** to **External** (not Internal). Internal restricts access to your Google Workspace organization — personal Gmail accounts and users outside your org will see `Error 403: org_internal` during authorization. After selecting External, add the Google accounts you want to test with under **Test users** (required while the app is in "Testing" publishing status).
...
2. Copy the **Redirect URI** from the connection panel (it looks like `https://<ENV_URL>/sso/v1/oauth/<CONNECTION_ID>/callback`).
...
<details>
<summary>Tool calls fail with <code>permission_denied</code> even though the account is ACTIVE</summary>
...
<details>
<summary>Authorization fails with <code>Error 403: org_internal</code></summary>

- **Symptom**: Google shows "Access blocked: [App name] can only be used within its organization" when a user tries to authorize.
- **Cause**: The OAuth consent screen in Google Cloud Console is set to **Internal** user type, which restricts access to accounts within your Google Workspace organization. Personal Gmail accounts and external users are blocked.
- **Fix**: Go to [Google Cloud Console → APIs & Services → OAuth consent screen](https://console.cloud.google.com/apis/credentials/consent) and change the user type to **External**. Add the accounts you want to test with under **Test users** — this is required while the app is in "Testing" publishing status.

</details>

<details>
<summary>Tool calls fail with <code>permission_denied</code> after adding scopes</summary>
...
<details>
<summary>Notion page creation fails with "no parent_page_id/database_id provided"</summary>
