...
- [Build a Vapi voice assistant that can use any tool with Scalekit](/cookbooks/build-voice-assistant-1000-tools) — the Virtual MCP approach to the same problem, for large tool catalogs where dynamic discovery beats hardcoding one function tool per connector.
...
| Vapi + Virtual MCP cookbook | [Build a Vapi voice assistant that can use any tool](/cookbooks/build-voice-assistant-1000-tools) |
