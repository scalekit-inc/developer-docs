...
<Tabs syncKey="coding-agent">
  <TabItem label="Recommended: one command" icon="sparkle">
    ...
    The CLI installs the authstack plugin for your editor. Then load the shipped skill `integrating-agentkit`. Complete any browser OAuth prompt for the Scalekit MCP server.

    Done: the command exits 0 and `integrating-agentkit` is available.
