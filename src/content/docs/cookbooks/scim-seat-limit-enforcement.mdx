  ...
  <TabItem label="Node.js">
    ...
    app.post('/webhooks/scalekit', async (req, res) => {
      ...
      if (event.type === 'organization.directory.user_created') {
        ...
        // Fetch current usage. Lock the row to prevent race conditions.
        ...
        // Room available — create the user and increment the counter.
...
<Tabs syncKey="tech-stack">
  ...
  <TabItem label="Python">
    ...
    if event.get('type') == 'organization.directory.user_deleted':
        ...
        with db.transaction():
            # Remove the user and decrement the counter atomically.
            db.execute('DELETE FROM users WHERE id = %s', (directory_user['id'],))
            db.execute(
...
- [SCIM provisioning quickstart](/directory/scim/quickstart/) — set up webhooks and the Directory API
