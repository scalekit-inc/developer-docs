---
title: 'Build a multi-user GitHub PR summarizer with Render Workflows and Scalekit'
description: "Deploy a durable Render Workflow that reads open pull requests via Scalekit's GitHub connector and generates plain-language summaries for each team member using their own GitHub token."
date: 2026-04-11
tags: ['Agent auth', 'GitHub', 'Render']
sidebar:
  label: 'GitHub PR summarizer (Render)'
excerpt: >
...
The complete source is available in the [render-ai-agent-deploykit](https://github.com/scalekit-inc/render-ai-agent-deploykit) repository.
...
![](@/assets/docs/render-github-pr-summarizer/render-dash.png)
...
- **Add more connectors** — The same `getOrCreateConnectedAccount` + `executeTool` pattern works for any Scalekit-supported connector. See [all supported agent connectors](/guides/integrations/agent-connectors/).
