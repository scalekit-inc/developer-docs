...
3. Set `PUBLIC_BASE_URL` to `http://localhost:3000` while developing locally.
4. Set `PUBLIC_BASE_URL` to your Render service URL after deployment.
5. The app sends `${PUBLIC_BASE_URL}/user/verify` as `userVerifyUrl` when it generates the GitHub auth link.
</Steps>
...
`PUBLIC_BASE_URL` must match the public origin where the app is running because the server uses it to construct `userVerifyUrl`.
...
function getPublicBaseUrl(): string {
  ...
  if (!value) throw new Error("PUBLIC_BASE_URL is required");
...
export function startServer(): void {
  ...
  app.post("/api/auth", async (req, res) => {
    ...
    const result = await setupGitHubAuthTask({
      identifier,
      state,
      userVerifyUrl: `${getPublicBaseUrl()}/user/verify`,
    });
...
Set `PUBLIC_BASE_URL` to the deployed service origin, for example `https://your-service.onrender.com`.

After deployment, set `PUBLIC_BASE_URL` to the exact service origin so the app generates `https://your-service.onrender.com/user/verify` as `userVerifyUrl`.
