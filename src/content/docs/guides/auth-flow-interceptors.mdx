...
```json title="PRE_SIGNUP — request body" showLineNumbers=true collapse={9-14, 27-31,}
{
  ...
  "interceptor_context": {
    ...
    "auth_request": { // Optional: Contains below parameters from the /oauth/authorize request.
...
```json title="PRE_SIGNUP — response body" wrap showLineNumbers=true
{
  ...
  // Optional. Include when the decision is ALLOW and user is to be provisioned in an existing organization.
   "response": {
...
<Aside type='note' title='Modify token claims in the response'>
 The `claims` field lets you add custom information that will be included in access tokens issued by Scalekit.
</Aside>
