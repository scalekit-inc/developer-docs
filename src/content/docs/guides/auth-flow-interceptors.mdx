...
```json title="PRE_SIGNUP — request body" showLineNumbers=true wrap collapse={9-14,29-33}
{
  "display_name": "Block signup from spam emails",
  "trigger_point": "PRE_SIGNUP",
  "interceptor_context": {
    "environment_id": "env_92561807204567213",
    "user_id": "usr_93418238346728951",  // Present if user already exists (other org)
  ...
  "data": {
   // Present if user already exists (other org)
   "user": {
...
```json title="PRE_SESSION_CREATE — request body" wrap showLineNumbers=true collapse={11-16,35-39}
...
```json title="PRE_SESSION_CREATE — response body" wrap showLineNumbers=true
{
  // Required
  "decision": "ALLOW | DENY",
  // Optional (with ALLOW)
  "response": { "claims": { "claim1": "value1", "claim2": "value2" } },
  // Optional (with DENY)
  "error": { "message": "string" }
}
```
...
```json title="PRE_USER_INVITE — request body" wrap showLineNumbers=true collapse={12-15}
{
  "display_name": "Prevent invites to spam emails",
  "trigger_point": "PRE_USER_INVITATION",
  "interceptor_context": {
    "environment_id": "env_92561807204567213",
    "user_id": "usr_93418238346728951", // Present if user exists (other org)
    "user_email": "john.doe@example.com",
...
```json title="PRE_USER_INVITE — response body" wrap showLineNumbers=true
{
  // Required
  "decision": "ALLOW | DENY",
  // Optional (with DENY)
...
```json title="PRE_M2M_TOKEN_CREATE — request body" wrap showLineNumbers=true collapse={15-18,22-24}
{
...
```json title="PRE_M2M_TOKEN_CREATE — response body" wrap showLineNumbers=true
{
  // Required
  "decision": "ALLOW | DENY",
  // Optional (with ALLOW)
...
Open the Test tab on the Interceptors page. The left panel shows the request body sent to your endpoint, and the right panel shows your service’s response.

Based on your application's response, you can decide to allow or deny the user auth flow to continue to sign up or log into your application.

![Interceptors test tab example](@/assets/docs/auth-flow-interceptors/test-example.png)

To try it quickly, use a request bin service such as <a href="https://beeceptor.com/" target="_blank" rel="noopener">Beeceptor</a>.
