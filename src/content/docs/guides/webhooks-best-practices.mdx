...
Scalekit can post events directly to a Slack channel, with no receiver code to build or host. When a webhook endpoint's URL is a Slack incoming webhook (`https://hooks.slack.com/services/...`), Scalekit detects it and reshapes each event into Slack's message format before delivery, so notifications post straight to the channel.

To route events to a Slack channel:

1. In Slack, create an **incoming webhook** for the target channel and copy its URL. Follow Slack's [incoming webhooks guide](https://docs.slack.dev/messaging/sending-messages-using-incoming-webhooks/).
2. In the Scalekit dashboard, open **Webhooks**, add an endpoint, and paste the Slack incoming webhook URL.
3. Subscribe the endpoint to only the event types you want in the channel. A Slack channel is high-visibility, so a focused subscription keeps it useful and avoids noise.

<Aside type="note" title="Automatic Slack formatting">
Scalekit reshapes events into Slack's message format only for endpoints whose host is `hooks.slack.com`. Every other endpoint continues to receive the standard Scalekit event payload, unchanged.
</Aside>
