...
public String webhook(
  ...
  // Spring may keep title-case keys. The Java SDK reads lowercase names only.
...
The key is the base64-decoded text after the first `_` in the `whsec_…` secret. The `webhook-signature` header is a space-separated list of `v1,<base64>` values. Reject timestamps more than 5 minutes in the past or future. Compare digests with a timing-safe function.
