...
Scalekit can post events directly to a Slack channel, with no receiver code to build or host. **Slack notifications are configured separately from webhook endpoints**: a webhook endpoint delivers the raw Scalekit event payload (which Slack does not accept), whereas Slack notifications reshape each matching event into Slack's message format before delivery.

To route events to a Slack channel:

1. In Slack, create an **incoming webhook** for the target channel and copy its URL (`https://hooks.slack.com/services/...`). Follow Slack's [incoming webhooks guide](https://docs.slack.dev/messaging/sending-messages-using-incoming-webhooks/).
2. In the Scalekit dashboard, open **Webhooks > Slack notifications**, paste the Slack incoming webhook URL, and enable delivery.
3. Select the event types to forward. A Slack channel is high-visibility, so a focused selection keeps it useful and avoids noise. Leaving the selection empty forwards all event types.

<Aside type="note" title="Slack notifications vs webhook endpoints">
Slack notifications are a separate delivery path. Your existing webhook endpoints are unaffected and continue to receive the standard Scalekit event payload. The Slack webhook URL is a secret — Scalekit stores it encrypted and never returns it.
</Aside>

A delivered Slack message uses the event's display name as the header, lists key event metadata, and attaches the event data as a formatted block:

```json title="Payload delivered to Slack"
{
  "text": "User created",
  "blocks": [
    { "type": "header", "text": { "type": "plain_text", "text": "User created" } },
    { "type": "section", "text": { "type": "mrkdwn", "text": "*Event:* scalekit.dir.user.create" } }
  ]
}
```
