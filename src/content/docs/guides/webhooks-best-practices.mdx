...
```javascript title="Webhook signature verification - Node.js"
...
```go title="Webhook signature verification - Go"
...
```java title="Webhook signature verification - Java"
@PostMapping("/webhook")
...
```javascript title="Event type handling - Node.js"
app.post('/webhook', async (req, res) => {
...
```python title="Event type handling - Python"
from fastapi import FastAPI, Request
...
```go title="Event type handling - Go"
mux.HandleFunc("POST /webhook", func(w http.ResponseWriter, r *http.Request) {
...
```java title="Event type handling - Java"
@PostMapping("/webhook")
public String webhook(@RequestBody String body, @RequestHeader Map<String, String> headers) {
