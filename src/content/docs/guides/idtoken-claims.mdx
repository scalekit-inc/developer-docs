...
next:
  link: '/authenticate/fsa/complete-login/#decoding-token-claims'
...
| JWKS endpoint | `https://<YOUR_ENVIRONMENT_URL>/keys` |
...
1. Fetch the OpenID configuration from `https://<YOUR_ENVIRONMENT_URL>/.well-known/openid-configuration` to discover `issuer` and `jwks_uri`.
2. Fetch the public signing keys from the `jwks_uri` (e.g., `https://<YOUR_ENVIRONMENT_URL>/keys`).
