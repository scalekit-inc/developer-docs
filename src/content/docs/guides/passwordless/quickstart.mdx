...
This guide shows you how to implement passwordless authentication using Scalekit's APIs to send either one-time passcodes or magic links to your user's email address and verify their identity.
...
2. ### Send verification email
   ...
   **Example implementation**
   ...
   <Tabs syncKey='tech-stack'>
    <TabItem label="Curl">
    ```sh ins={6-11} "/api/v1/passwordless/email/send" title="Request" wrap { "Only if using magic link": 10}
    ...
    --data-raw '{
        ...
        "magiclink_auth_uri": "http://yourapp.com/passwordless/verify"
    }'
    ...
    <details>
      ...
      | `magiclink_auth_uri` | No | App callback URL to handle magic link callback and verify the user. Required only for magic link authentication. <Badge variant="note" text="string" /> |
    </details>
    ...
    ```
     <details>
      ...
      | `magiclinkAuthUri` | No | App URL to handle magic link callback and verify the user. Required only for magic link authentication. <Badge variant="note" text="string" /> |
    </details>
