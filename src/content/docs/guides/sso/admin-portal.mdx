...
Call the Generate Portal Link API to create a unique, embeddable Admin Portal link for an organization. Generated links expire after **1 minute**. Once an IT admin accesses the link, the session remains active for **6 hours** until setup is complete.
