...
tags: [external-ids, integration, stripe, salesforce, workday, billing, crm, hr]
tableOfContents:
  maxHeadingLevel: 2
sidebar:
...
app.post('/stripe/webhook', async (req, res) => {
  ...
  if (event.type === 'customer.subscription.updated') {
    ...
    const { organization: org } = await scalekit.organization.getOrganizationByExternalId(customerId);

    if (org) {
      // Update subscription metadata
      await scalekit.organization.updateOrganization(org.id, {
        metadata: {
          ...org.metadata,
...
def stripe_webhook():
    ...
    if event['type'] == 'customer.subscription.updated':
        ...
        if org:
            ...
            update_organization = UpdateOrganization(metadata=updated_metadata)
            scalekit_client.organization.update_organization(
                organization_id=org.id, organization=update_organization
            )
...
async function syncOrganizationWithCRM(organizationId, salesforceAccountId) {
  try {
    ...
    await scalekit.organization.updateOrganization(organizationId, {
      metadata: {
...
async function updateCRMFromScalekit(organizationId) {
  const { organization: org } = await scalekit.organization.getOrganization(organizationId);
...
async def sync_organization_with_crm(organization_id, salesforce_account_id):
    try:
        ...
        update_organization = UpdateOrganization(metadata=metadata)
        scalekit_client.organization.update_organization(
...
async def update_crm_from_scalekit(organization_id):
    response = scalekit_client.organization.get_organization(organization_id)
    org = response[0].organization
...
async function handleEmployeeStatusChange(employee_id, status) {
  try {
    ...
    const { user } = await scalekit.user.getUserByExternalId(employee_id);
    ...
    if (user) {
      if (status === 'terminated') {
        ...
        await scalekit.user.deleteMembership(organization.id, user.id);
...
async function onboardNewCustomer(customerData) {
  ...
  try {
    ...
    const { organization } = await scalekit.organization.createOrganization(company_name, {
      externalId: stripe_customer_id, // Use billing system ID as primary external ID
    });
...
async function findOrganizationWithRetry(externalId) {
  return retryWithBackoff(async () => {
    const { organization: org } = await scalekit.organization.getOrganizationByExternalId(externalId);
    if (!org) {
