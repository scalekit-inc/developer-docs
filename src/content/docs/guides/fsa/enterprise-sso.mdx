...
3. ## Initiate SSO login
    ...
    try {
      String url = Auth.scalekit.authentication().getAuthorizationUrl(redirectUrl, options).toString();
      // Redirect the user to this authorization URL
    } catch (Exception e) {
...
4. ## Retrieve user profile
    ...
    <Tabs syncKey="tech-stack">
        ...
        try {
            String redirectUrl = "http://localhost:3000/api/callback";
            AuthenticationResponse result = Auth.scalekit.authentication().authenticateWithCode(code, redirectUrl);
...
- [Customize the login page](/guides/fsa/login-page-branding/) to match your brand's design and style.