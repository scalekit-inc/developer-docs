...
If signed-in users in your application need a broader self-serve portal (members, profiles, security, domains, session policy, and enterprise setup), see [Hosted UI widgets](/authenticate/manage-users-orgs/hosted-widgets/) and redirect them to `/ui/`. Use the Admin portal when you need a shareable link or an iframe embed for enterprise SSO and SCIM setup. The Admin portal is the only hosted surface that supports iframe embed.
