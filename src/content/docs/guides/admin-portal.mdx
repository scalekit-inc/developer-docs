...
For a **logged-in SaaSKit** self-serve portal (members, profiles, security, domains, session policy, plus SSO and SCIM) reached by redirecting users to `/ui/`, see [Hosted widgets](/authenticate/manage-users-orgs/hosted-widgets/). Use Admin portal when you need a **shareable magic link** or **iframe embed** for enterprise IT setup.
