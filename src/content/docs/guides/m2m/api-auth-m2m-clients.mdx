...
description: "Learn how to enable OAuth-based API authentication for your customers' applications using Scalekit, from token issuance to request validation with JWKS"
browseCentral:
...
seeAlso:
  ...
  items:
    ...
    - title: "M2M authentication examples"
      icon: "github"
...
This guide explains how you enable API authentication for your customers' applications using Scalekit's OAuth 2.0 client credentials flow. When your customers build applications that need to access your API, they use client credentials registered through Scalekit to obtain access tokens. Your API validates these tokens to authorize their requests.

This guide covers the complete authentication flow: how your customers' applications obtain access tokens and how your API validates requests using JSON Web Key Sets (JWKS).
...
The [Python SDK](https://github.com/scalekit-inc/scalekit-sdk-python) includes built-in methods for token validation (`validate_access_token_and_get_claims`). For Node.js, Go, and Java, you can use standard JWT libraries with the JWKS endpoint as shown in the examples above. Native SDK support for token validation is coming soon to these SDKs.
