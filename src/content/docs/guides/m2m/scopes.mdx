...
The API returns a JSON object [containing comprehensive information](/m2m/quickstart/#register-an-m2m-client) about the newly created M2M client. Your application (acting as the M2M server) should securely provide only the `client_id` and `client_secret` from the response to the M2M client application that needs to authenticate.
