...
```javascript showLineNumbers=false title="ExpressJS" wrap
...
```python showLineNumbers=false title="FastAPI" collapse={1-5} wrap
...
// Resource identifier configured in your Scalekit dashboard - must match the 'resource' field in metadata
const RESOURCE_ID = `https://your-mcp-server.com`;
// Resource metadata endpoint URL for WWW-Authenticate header - copy from your Scalekit dashboard
...
export async function authMiddleware(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        // MCP clients need to discover authorization server information without authentication
        if (req.path.includes('.well-known')) {
            return next();
        }

        // Extract Bearer token from Authorization header for all MCP requests
        // MCP clients send tokens in the format: Authorization: Bearer <token>
        ...
        // Validate token against configured resource audience to ensure it was issued for this MCP server
...
def validate_token_with_issuer_audience(token: str) -> bool:
    ...
    This method catches all exceptions and returns False on any validation failure.
    Use this for simple authorization checks where detailed error information is not needed.
...
def validate_token_and_get_claims(token: str) -> dict:
    ...
    This method raises exceptions on validation failures, providing detailed error information.
    Use this when you need to access token claims or implement finer authorization logic.
