...
def validate_token_with_issuer_audience(token: str) -> bool:
    """
    Python equivalent of the .NET ValidateTokenWithIssuerAudienceAsync method
...
public async Task<bool> ValidateTokenWithIssuerAudienceAsync(string token)
{
    var options = new TokenValidationOptions
    {
        Issuer = "https://your-domain.scalekit.com",
        Audience = new[] { "your-api-audience" }
    };

    try
    {
        bool isValid = await scalekitClient.ValidateAccessTokenAsync(token, options);
        return isValid;
    }
    catch (Exception ex)
    {
...
try
{
    await scalekitClient.ValidateAccessTokenAsync(token, new TokenValidationOptions
    {
        Audience = new[] { RESOURCE_ID },
