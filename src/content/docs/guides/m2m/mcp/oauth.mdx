...
# Use this when you only need to know if the token is valid or not
...
# Use this when you need the actual token claims or detailed error information
def validate_token_and_get_claims(token: str) -> dict:
    """
    Validates a token and returns the token claims/payload.
    This method raises exceptions on validation failures, providing detailed error information.
    
    :param token: The token to validate
    :return: Token claims/payload as a dictionary
    :raises: ScalekitValidateTokenFailureException if validation fails
    """
    options = TokenValidationOptions(
        issuer="https://your-domain.scalekit.com",
        audience=["your-api-audience"],
        required_scopes=["read:users", "write:users"]  # Optional: validate specific scopes
    )

    try:
        claims = scalekit_client.validate_token(token, options=options)
        return claims
    except Exception as ex:
        print(f"Token validation failed: {ex}")
        raise
