...
description: "Provide automated user provisioning to your customers via directory providers"
head:
  - tag: style
    content: |
...
One of the easiest ways to see if SCIM setup for a organization is working successfully is to create a new user in the directory provider and see if it is automatically created in the Scalekit organization > users.

Often, you'd want to list the connected directories in your app.
...
```json title="List connected directories response" showLineNumbers=false frame="terminal" collapse={26-34, 37-40, 19-22}
{
  "directories": [
    {
      "attribute_mappings": {
