...
prev: false
next: false
description: "Learn how to set up SAML-based Single Sign-On (SSO) using Okta as your Identity Provider, with step-by-step instructions for enterprise application configuration."
tags: [sso, saml, okta, identityprovider, enterprise, integration, guide]
sidebar:
  order: 2
  label: "Okta"
  badge:
...
1. Login to your _Okta Admin Console_. Go to _Applications→ Applications_.
   ...
   ![Okta Admin Console with the Applications menu item highlighted](@/assets/docs/guides/sso-integrations/okta-saml/0.png)

2. In the **Applications** tab, click on **Create App Integration.**

   ![Okta Applications page with the Create App Integration button highlighted](@/assets/docs/guides/sso-integrations/okta-saml/1.png)

3. Choose **SAML 2.0**, and click on **Next.**

   ![Create a new app integration dialog with SAML 2.0 selected as the sign-in method](@/assets/docs/guides/sso-integrations/okta-saml/2.png)

4. Give your app a name, choose your app visibility settings, and click on **Next.**

   ![General Settings step of the Create SAML Integration wizard with app name and logo fields](@/assets/docs/guides/sso-integrations/okta-saml/3.png)
...
1. Copy the **SSO URL** from the **SSO Configuration Portal**. Paste this link in the space for **SSO URL** on the **Okta Admin Console**.
   ...
   ![SSO Configuration Portal showing the Single Sign-On URL to copy into Okta](@/assets/docs/guides/sso-integrations/okta-saml/4.png)
   ![Configure SAML step in Okta with the Single sign-on URL field filled in](@/assets/docs/guides/sso-integrations/okta-saml/5.png)
...
2. Copy the **Audience URI (SP Entity ID)** from the SSO Configuration Portal, and paste it in your **Okta Admin Console** in the space for **Audience URI.**
   ...
   ![SSO Configuration Portal showing the Audience URI (SP Entity ID) to copy into Okta](@/assets/docs/guides/sso-integrations/okta-saml/6.png)
   ![Configure SAML step in Okta with the Audience URI (SP Entity ID) field highlighted](@/assets/docs/guides/sso-integrations/okta-saml/7.png)
...
3. You can leave the Default Relay State as blank. Similarly, select your preferences for the Name ID format, Application Username, and Update application username on fields.
   ...
   ![Okta SAML settings for Default RelayState, Name ID format, and application username options](@/assets/docs/guides/sso-integrations/okta-saml/8.png)
...
<Steps>
1. The following user profile attributes should be mapped in Okta so that Scalekit receives user details during SSO login:

   - **Email Address** of the user (required)
   - **First Name** of the user (optional)
   - **Last Name** of the user (optional)

   Check the **Attribute Statements** section in the **SSO Configuration Portal** for the exact attribute names and values to use, then map the same attributes on your Okta Admin Console. You can either use the **Add expression** buttons or the legacy configuration method.

   ![Attribute mapping on SSO Configuration Portal](@/assets/docs/guides/sso-integrations/okta-saml/20.png)

2. You will have to enter each attribute one by one as shown below. click on **Save** once you have added the name and value for the attribute,

   ![Attribute mapping on Okta Admin Console](@/assets/docs/guides/sso-integrations/okta-saml/21.png)

3. Ensure that you map all the required attributes as shown on the SSO Configuration Portal.

   ![Attribute mapping completed on Okta Admin Console](@/assets/docs/guides/sso-integrations/okta-saml/22.png)
...
With this, you are done configuring your Okta application for an SSO login setup.
