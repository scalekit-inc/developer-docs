...
on implementing the IdP-initiated SSO flow, refer to [Implement IdP-Initiated SSO](/guides/product/idp-init-sso/) guide. This guide walks through the necessary steps to set up and handle SSO requests originating from the identity provider.
