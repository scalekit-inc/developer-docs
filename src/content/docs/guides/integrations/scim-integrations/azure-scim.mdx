...
4. ## Map custom attributes (optional)
   ...
   - **Source attribute**: Select the Entra ID attribute that contains the value you want to sync (for example, `employeeId` or a custom extension attribute like `extension_<guid>_customAttribute`).
   ...
   <Aside type="note">
   Custom extension attributes in Entra ID follow the naming pattern `extension_<application-object-id>_<attribute-name>`. You can find the application object ID in the Entra ID portal under **App registrations > Your app > Application (client) ID**.
