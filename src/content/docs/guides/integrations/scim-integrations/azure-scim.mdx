...
2. ## Add a new application in Entra ID
   ...
   In the "Manage > Enterprise Application" tab, click "+ New application".
...
3. ## Configure provisioning settings
   ...
   In the created application go to "Manage → Provisioning"
   
   ![Open a provisioning tab from created application](`@/assets/docs/guides/scim-integrations/entra-id-scim/04.png`)
   ...
   Once the credentials are configured, Test your connection and click "Save".
...
4. ## Map custom attributes (optional)
   ...
   By default, Entra ID syncs standard attributes such as email, first name, last name, and display name. To sync a custom attribute (for example, a department code or employee ID), you must map it explicitly in the provisioning configuration.
    ...
    <Aside type="note">
    Custom extension attributes in Entra ID follow the naming pattern `extension_<appId>_<attributeName>`, where `<appId>` is the Application (client) ID from your Entra app registration. You can find the Application (client) ID in the Entra ID portal under **App registrations > Your app > Application (client) ID**.
...
5. ## Assign User and Group

   In the Created Application, go to "Users and groups". And click "+ Add users/groups"

   ![Go to Users and Groups tab for assign a user](@/assets/docs/guides/scim-integrations/entra-id-scim/022.png)

   Click the button  under the "Users and Groups". In the menu, select the users and groups that you want to add to the SCIM application, and click "Select".

   ![Assgin users to application](@/assets/docs/guides/scim-integrations/entra-id-scim/023.png)

   Once the users are selected, the "Assign" button is automatically enabled. Then, click the "Assign".
