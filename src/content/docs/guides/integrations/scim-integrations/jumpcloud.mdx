...
6. ### Verify successful connection
   ...
   <Aside type="note">
   When an group is disassociated from an app in JumpCloud ("Provisioning via Scalekit" app), JumpCloud sends an group update event to Scalekit that unassigns all the group users to your app. However, the group association is not removed in Scalekit automatically.
   </Aside>
