...
1. ## Create an endpoint and API token
   ...
   Open the Admin Portal from the app being onboarded and select the "SCIM Provisioning" tab. Choose "OneLogin" as your Directory Provider.
...
2. ## Add a new application in OneLogin
   ...
   Search for "SCIM with SAML (SCIM v2 Enterprise)"
   ...
   Go to the "Configuration" tab in the left navigation panel. Configure the "Endpoint URL" and "Bearer Token", then click the "Enable" button.
   
   ![Configure credentials in the OneLogin dashboard.](@/assets/docs/guides/scim-integrations/onelogin-scim/19.png)
