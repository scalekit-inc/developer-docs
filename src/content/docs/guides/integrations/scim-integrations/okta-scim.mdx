...
1. ## Create an endpoint and API token
   ...
   Open the Admin Portal from the app being onboarded and select the "SCIM Provisioning" tab. A list of Directory Providers will be displayed. Choose "Okta" as your Directory Provider. If the Admin Portal is not accessible from the app, request instructions from the app owner.
...
6. ## Push groups and sync group membership
   ...
   <Aside type="caution" title="Deactivated users are skipped by Group Push">
   If a user is removed from an **assigning group** first, Okta deactivates them for the app and sends that event correctly. But Okta excludes deactivated users from Group Push updates, so a subsequent removal from a **push group** is not pushed to the application — the user keeps showing up in that group's membership downstream. To fix a stuck membership, reactivate the user in Okta and run Group Push again for each group the user belongs to.<br />
   See Okta's <a href="https://support.okta.com/help/s/article/Group-Push-Common-Issues?language=en_US" target="_blank" rel="noopener">Group Push Common Issues</a> article for more on this behavior.
   </Aside>
