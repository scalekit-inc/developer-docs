...
next:
  link: '/guides/dashboard/template-redirect-uris/'
...
If you need a different callback URL for each customer organization — for example, `https://auth.megasoft.com/oauth/callback` for Megasoft and `https://auth.betacorp.com/oauth/callback` for Beta Corp — register a single URL pattern with variables instead of registering one URL per organization. See [Organization-specific redirect URLs](/guides/dashboard/template-redirect-uris/) for the full setup.
