---
title: Auth Tester
description: Test your configuration and authentication flows without writing a single line of code
...
Use Auth Tester to verify your Scalekit authentication configuration **before** writing application code. This confirms your Scalekit environment is set up correctly and ready to handle login flows for Scalekit's Full Stack Authentication or Modular SSO.

Run it from your terminal with following command to test your authentication setup without integrating the SDK into your application.

The CLI creates a complete authentication flow, starts a local server, opens your browser, and displays a dashboard with authenticated user information and tokens. This mirrors the flow your application will implement, so you can confirm that your environment is configured correctly before writing any code.

Before you run Auth Tester, make sure you have:
...
Auth Tester is designed for **local testing only**:
