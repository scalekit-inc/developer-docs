...
def wait_for_auth(instance_id: str, poll_interval: int = 5):
    while True:
        state = my_mcp.get_instance_auth_state(instance_id=instance_id)
        connections = getattr(state, "connections", [])
        if all(c.connected_account_status == "ACTIVE" for c in connections):
            print("All connections authorized — MCP URL is ready.")
            return
        pending = [c.connection_name for c in connections if c.connected_account_status != "ACTIVE"]
