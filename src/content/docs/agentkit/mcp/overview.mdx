...
A session token is short-lived. The default expiry is about one hour, and `expiry` is a parameter on `create_session_token`.
...
Where you call it depends on the agent's lifetime:
