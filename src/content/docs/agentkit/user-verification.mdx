...
<details>
<summary>Error: <code>failed_to_exchange ... user_verify_url not configured for verification redirect</code></summary>

This error means the connection is set to **Custom user verification**, but the authorization flow started without a `user_verify_url`. Scalekit completes the OAuth exchange but has nowhere to redirect the user for verification.
