...
Use one of the JSON payloads from [Auth patterns](/agentkit/bring-your-own-connector/auth-types-and-patterns) as the request body. For MCP providers, use the payloads from the [MCP provider payloads](/agentkit/bring-your-own-connector/auth-types-and-patterns#mcp-provider-payloads) section — the API endpoint is the same.
