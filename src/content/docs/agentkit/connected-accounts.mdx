...
Connected accounts in Agent Auth represent individual user or organization connections to third-party connectors. They contain the authentication state, tokens, and permissions needed to execute tools on behalf of a specific identifier (user_id, org_id, or custom identifier).
...
3. **User completes OAuth flow** with the third-party connector
4. **Connector redirects back** with authorization code
...
Next, learn how to [authorize a user](/agentkit/tools/authorize) so connected accounts can complete authentication before tool execution.
