...
agent = client.beta.agents.create(
    ...
    tools=[
        {"type": "mcp_toolset", "mcp_server_name": "scalekit"},
    ],
)
```

<Aside type="note" title="Tool confirmation policy">
  MCP tools default to an `always_ask` permission policy, which pauses the agent to request approval before each tool call. For autonomous operation, set the policy to `always_allow`. See [Permission policies](https://platform.claude.com/docs/en/managed-agents/permission-policies) in the Anthropic docs.
</Aside>
