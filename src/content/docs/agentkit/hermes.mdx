...
There are three patterns. Pick the one that matches your host.

**Pattern A — Delegated skill.** The default for Hermes. The host holds Scalekit client credentials, and each turn names an identifier. Nothing on the host expires, because there is no token on the host.
...
**Pattern C — Virtual MCP in your app.** Your application mints a token before each agent run and passes it to the agent framework. Claude Managed Agents, Mastra, and CrewAI use this pattern.

| | A — Delegated skill | B — Virtual MCP on the host | C — Virtual MCP in your app |
|---|---|---|---|
...
The rest of this page covers Pattern A. For Pattern B, see [Can I use Virtual MCP with Hermes?](#common-scenarios) below. For Pattern C, see [Set up and connect a Virtual MCP server](/agentkit/mcp/configure-mcp-server/).
...
**The skill is the default.** `hermes-delegated-auth` calls `execute_tool` with Scalekit client credentials and an identifier. No token sits on the host, so nothing expires. Use this for a host that serves more than one user, and for cron.

**Virtual MCP is the scoped alternative.** A Virtual MCP server exposes only the tools you list, which is the reason to choose it. Hermes reads the bearer from `config.yaml` as `${SCALEKIT_MCP_SESSION_TOKEN}` and resolves it when it connects, or when you run `/reload-mcp`.
