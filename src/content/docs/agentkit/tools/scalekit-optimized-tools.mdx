---
title: Scalekit optimized tools
...
For LangChain and Google ADK, Scalekit returns native tool objects in Python — no schema reshaping needed.
...
<Aside type="note" title="MCP-compatible frameworks">
  Prefer a single interface any MCP client can consume? See [Tool execution via MCP](/agentkit/mcp/tool-access-via-mcp/).
</Aside>
