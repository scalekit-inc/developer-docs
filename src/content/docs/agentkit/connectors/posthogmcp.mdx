---
title: PostHog MCP
description: Connect to PostHog MCP to query analytics, manage feature flags, run experiments, and interact with your product data using OAuth 2.1.
...
This connector uses **OAuth 2.1 with PKCE**. Scalekit acts as the OAuth client: it redirects your user to PostHog MCP, obtains an access token, and automatically refreshes it before it expires. Your agent code never handles tokens directly — you only pass a `connectionName` and a user `identifier`.
