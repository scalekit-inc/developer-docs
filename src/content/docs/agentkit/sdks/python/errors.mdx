...
try:
    ...
    # Security: log on the server only. Do not return tool_error_message to the end user.
...
| `ScalekitToolException` | Base class for any upstream provider error during tool execution | Log `tool_error_code`, `tool_error_message`, and `execution_id` on the server; surface a generic error to the end user |
