...
<Aside type="note" title="Client timeouts">
The client sets two gRPC call deadlines you can override when you construct `ScalekitClient`:

- `timeoutMs` (default `20000`) applies to control-plane RPCs, such as organizations, users, and connections.
- `toolTimeoutMs` (default `60000`) applies to tool-execution RPCs (`tools.*` and `actions.*`), which proxy to third-party provider APIs and can run longer.

Starting in v2.8.0, control-plane calls use a 20-second deadline by default. A call that runs longer fails with a `DeadlineExceeded` error, so raise `timeoutMs` if your workload needs more time.
</Aside>
...
<MethodParams label="Input schema" params={[
  ...
  { name: 'timeoutMs', type: 'number', required: false, description: 'Per-call request timeout in ms. Defaults to the client toolTimeoutMs (60000).' },
]} />
