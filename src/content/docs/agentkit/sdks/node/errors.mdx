...
try {
  ...
  if (err instanceof ScalekitToolUnauthorizedException) {
    ...
    // Security: log on the server only. Do not return toolErrorMessage to the end user.
...
| `ScalekitToolException` | Any other upstream provider error during tool execution | Log `toolErrorCode`, `toolErrorMessage`, and `executionId` on the server; surface a generic error to the end user |
