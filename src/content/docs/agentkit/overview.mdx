...
banner:
  content: |
    Build this faster with AI coding agents. <a href="/cookbooks/set-up-agentkit-with-your-coding-agent/">Build with AI →</a>
---

import { Card, CardGrid, LinkCard, Aside } from '@astrojs/starlight/components';

Agent Auth is Scalekit's authentication solution that enables your applications to securely connect to third-party services on behalf of your users. It handles the complexity of OAuth flows, token management, and multi-connector authentication for popular business applications like Gmail, Slack, Jira, and more.

## What is Agent Auth?

Agent Auth simplifies third-party authentication by providing:

- **Multi-connector OAuth**: Support for OAuth 2.0 flows across all major connectors
- **Unified authentication API**: Single interface for managing connections to any connector
...
Connectors are third-party applications that your users can authenticate with. Agent Auth supports OAuth 2.0 flows for popular platforms including Gmail, Slack, GitHub, Jira, and many more.

### Connections

Connections define the authentication configuration for a specific connector. Each connection contains:

- **Authentication credentials** (OAuth client ID/secret, API keys)
- **OAuth configuration** (authorization URLs, token endpoints, scopes)
- **Connector-specific settings** (rate limits, API versions)

### Connected accounts

Connected accounts represent the authenticated link between a user in your application and their account on a third-party connector. Each connected account maintains:
...
### 1. Configure connector connections

Set up OAuth credentials for each connector you want to support:
...
// Redirect user to authUrl to authenticate with the connector
```

### 3. Handle OAuth callback

After the user authorizes your application, the connector redirects back with an authorization code. Agent Auth automatically exchanges this code for access and refresh tokens:
...
### Multi-connector authentication
...
// Generate OAuth URLs for each connector
...
- **Unified authentication API**: Single interface for OAuth across all connectors
- **Automatic token refresh**: No manual token lifecycle management required
- **Pre-built OAuth flows**: Skip complex OAuth implementation for each connector
- **Multi-language SDKs**: Support for popular programming languages
- **Standardized error handling**: Consistent error responses across connectors
...
- **Shared credentials**: Pre-configured OAuth credentials for all connectors
...
- **Direct relationships**: Establish direct OAuth connections with connectors
...
<CardGrid>
  <LinkCard title="Quickstart Guide" href="/agentkit/quickstart">
    Get up and running with Agent Auth in minutes
  </LinkCard>
  <LinkCard title="Authentication Flows" href="/agentkit/authentication/auth-flows-comparison">
    Compare different authentication flow patterns
  </LinkCard>
  <LinkCard title="Connectors" href="/agentkit/connectors">
    View supported connectors and their requirements
  </LinkCard>
</CardGrid>
...
<Aside type="note">
**Ready to get started?** Check out our [Quickstart Guide](/agentkit/quickstart) to implement your first OAuth integration with Agent Auth in minutes.
</Aside>
