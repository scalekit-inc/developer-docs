...
The connection authorizes for users in the provider workspace that created the OAuth app, but fails when your customers try to connect. Airtable returns "OAuth app can't be used outside development," and ZoomInfo rejects a custom (internal) app. This happens when you registered your own OAuth app (Bring Your Own Credentials) and the provider still has that app in an internal or development mode that authorizes only the account that created it.
