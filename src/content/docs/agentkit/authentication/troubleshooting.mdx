...
description: Diagnose OAuth failures, connected account status issues, and tool execution errors in AgentKit.
tags: [agentauth, authentication, troubleshooting, oauth, guide]
sidebar:
  order: 6
...
Use this guide when a user cannot finish OAuth, a connected account shows an unexpected status, or tool execution fails with an auth error. Start with the diagnostics below, then open the matching scenario.
