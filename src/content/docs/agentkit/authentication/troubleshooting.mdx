...
A new connection returns a connected account id (for example `ca_...`) but the access token and refresh token fields come back empty. The connection is valid; the tokens are withheld on purpose.
...
<Aside type="caution" title="Handle raw tokens as secrets">
