...
Token management is a critical aspect of AgentKit that ensures your users' connections remain active and secure. Scalekit automatically handles the complete token lifecycle, including storage, refresh, and expiration, so you can focus on building your application.
...
elif account.status == "PENDING_AUTH":
...
} else if (account.status === 'PENDING_AUTH') {
...
case "PENDING_AUTH":
    fmt.Println("✗ Re-authentication needed")
}
```
...
switch (account.getStatus()) {
    ...
    case "EXPIRED":
        ...
        break;
    case "PENDING_AUTH":
        System.out.println("✗ Re-authentication needed");
        break;
}
```
...
**Scalekit behavior:**
- Account status changes to `EXPIRED` (refresh needed) or `PENDING_AUTH` (re-authentication needed)
...
try:
    ...
    if 'EXPIRED' in str(e) or 'PENDING_AUTH' in str(e):
...
try {
  ...
  // Check if error is due to token issues
  if (error.message.includes('EXPIRED') || error.message.includes('PENDING_AUTH')) {
    // Get fresh authorization link for user
...
if err != nil {
    // Check if error is due to token issues
    if strings.Contains(err.Error(), "EXPIRED") || strings.Contains(err.Error(), "PENDING_AUTH") {
        // Get fresh authorization link for user
...
try {
    ...
    // Check if error is due to token issues
    if (e.getMessage().contains("EXPIRED") || e.getMessage().contains("PENDING_AUTH")) {
        // Get fresh authorization link for user
...
- [Configure connections](/agentkit/connections/) — OAuth scopes and connection setup
- [Troubleshoot connection errors](/agentkit/authentication/troubleshooting) — Debug connection and token issues
- [Manage connected accounts](/agentkit/connected-accounts/) — Per-user tokens and connection state
