...
description: Set up connectors and get OAuth tokens and call tools on the fly using Scalekit's AgentKit
...
>
  ...
  [Continue building with AI →](/cookbooks/set-up-agentkit-with-your-coding-agent/)
</FoldCard>
...
2. ## Create connected account for the user
   ...
   Connected account is a user's 3rd party account that is registered in your Scalekit environment. This represents the user's connection to their Gmail account in this case.
    ...
    <Aside title="Set up a connection">
      ...
      To set up your app's credentials for a connector, check the **Agent connectors** section in the sidebar.
    </Aside>
3. ## Authenticate the user

    In order to execute any tools on behalf of the user, the user first needs to grant authorization to access their gmail account. Scalekit automatically handles the entire OAuth workflow with the Gmail connector, gets the access token, refreshes the access token periodically based on the refresh token etc.
...
Congratulations! You've built an agent that authenticates users and executes API calls on their behalf.

## Next steps

Now that your agent can connect and call APIs on behalf of users, these guides help you go deeper:
...
- [Tool calling](/agentkit/tools/agent-tools-quickstart/) — Use Scalekit's optimized tools to call APIs without managing endpoints yourself.
