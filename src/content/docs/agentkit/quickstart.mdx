...
<Tabs syncKey="build-mode">
  <TabItem label="Using a coding agent">
    ...
    Install the authstack plugin for your coding agent with `npx @scalekit-inc/cli setup` (or install globally with `npm install -g @scalekit-inc/cli` then `scalekit setup`). Then load the shipped skill `integrating-agentkit`. Complete any browser authorization when prompted, then paste the implementation prompt.

    ```bash title="Terminal" frame="terminal" showLineNumbers=false
    npx @scalekit-inc/cli setup
    ```

    The CLI sets up the authstack plugin for your editors. Then load `integrating-agentkit`. Complete any browser authorization for the Scalekit MCP server when prompted. Then use the prompt below (or describe your goal in natural language).
