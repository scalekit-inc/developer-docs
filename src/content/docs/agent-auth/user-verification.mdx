...
description: On each authorization, verification binds OAuth to the user who started the flow. Configure modes under Agent Actions > User Verification.
...
Whenever a connected account is authorized, Scalekit runs **user verification** so the user who finishes OAuth matches the user your app tied to the flow. That alignment blocks common phishing and social-engineering paths where consent would otherwise land on the wrong account.

Choose a mode in **Agent Actions > User Verification**:

- **Verify with your app** — Your server checks the user against your session. **Production** and users who only sign in to your product.
- **Verify with Scalekit** — Scalekit checks users signed in to the Scalekit dashboard. **Development and internal testing** only.
