...
description: Confirm the teammate who authorized a connector is the one your app intended, then configure verification in the dashboard.
...
Your users connect Gmail, GitHub, and other providers so agents can act on their behalf. **You need to know that the person who finished the provider's consent screen is the same person your app invited to connect**—not someone who received a forwarded magic link or followed a fake “fix your connection” message.

Agent Auth runs OAuth for connectors, but **Scalekit does not see your product's sign-in sessions, workspaces, or user directory**. **User verification** closes that gap: after OAuth completes, your application confirms which end-user should own the connection using an **identifier** you set when the flow started. Scalekit activates the connected account only when that identifier matches the user who authorized.
...
<Tabs syncKey="user-verification-mode">
  <TabItem label="Verify with your app">
...
Your app -> End user: Deliver link (email, in-app, …)
...
4. #### Confirm the user's identity
  ...
  <TabItem label="Verify with Scalekit">
