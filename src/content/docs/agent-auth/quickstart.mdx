...
<Tabs syncKey="tech-stack">
  ...
  <TabItem label="Node.js">
    ```sh showLineNumbers=false frame="none"
    npm install @scalekit-sdk/node@2.2.0-beta.0
    ```
  </TabItem>
</Tabs>
<Tabs syncKey="tech-stack">
...
<Tabs syncKey="tech-stack">
  ...
  <TabItem label="Node.js">
    ```typescript
    // Create a connected account for user if it doesn't exist already
    const response = await connectedAccounts.createConnectedAccount({
      connector: 'gmail',
      identifier: 'user_123', // unique identifier for your connected account. can be replaced with your system's user ID
      connectedAccount: {},
    });
...
<Tabs syncKey="tech-stack">
  ...
  <TabItem label="Node.js">
    ...
    const accessToken = accountResponse.connectedAccount?.authorizationDetails?.oauthToken?.accessToken;
    const refreshToken = accountResponse.connectedAccount?.authorizationDetails?.oauthToken?.refreshToken;
