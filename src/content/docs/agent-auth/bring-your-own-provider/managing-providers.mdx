...
```bash
curl --location "$SCALEKIT_ENVIRONMENT_URL/api/v1/providers" \
...
```bash
curl --location --request PUT "$SCALEKIT_ENVIRONMENT_URL/api/v1/providers/$PROVIDER_IDENTIFIER" \
  --header "Authorization: Bearer $env_access_token" \
  --header "Content-Type: application/json" \
  --data '{...}'
```
...
```bash
curl --location --request DELETE "$SCALEKIT_ENVIRONMENT_URL/api/v1/providers/$PROVIDER_IDENTIFIER" \
  --header "Authorization: Bearer $env_access_token"
```
