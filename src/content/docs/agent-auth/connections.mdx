...
- **OAuth 2.0 connections** — Most modern APIs use OAuth 2.0 for secure authentication. Configure client ID, client secret, redirect URI, scopes, and token settings.
- **API key connections** — Simple authentication using static API keys. Provide your API key, base URL, and optional settings such as rate limit and timeout.
- **Custom authentication** — For providers with unique authentication requirements. Configure credentials (e.g., username, password, token) and custom auth or refresh endpoints.
