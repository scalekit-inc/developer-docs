...
- **A values file** (`values-<env>-<timestamp>.yaml`) — ready to pass directly to `helm install`
...
Option `4` is a shortcut for a local or throwaway environment. The script asks only for a webhook JWT secret, a webhook API token, and a registry access token, then exits. It generates a minimal `values-eval-<timestamp>.yaml` and prints a single `helm install` command. No databases or Redis instances are needed — the chart provides bundled ones.
