...
- **A secrets script** (`scalekit-secrets-<timestamp>.sh`) — runs `kubectl` commands to create all six required Kubernetes secrets
- **A values file** (`values-<timestamp>.yaml`) — ready to pass directly to `helm install`

At the end it prints the exact `helm install` command to run.
...
| `python3` | 3.6 or later | Generate Svix JWT and OIDC client secret |
...
When prompted to choose an environment, enter `2` for a Kubernetes cluster deployment.
...
| **PostgreSQL** | Host, port, credentials, and database names |
| **Redis** | Host, port, password, and db indexes for app, background jobs, and Svix |
...
| **Gateway** | GatewayClass name and GCP certificate map |
| **App settings** | Domain, region |
| **Admin user** | First name, last name, email for the initial dashboard login |

All cryptographic values (OIDC keys, cookie keys, Svix JWT, etc.) are auto-generated — you do not supply these.

## After the script completes

The script prints the exact commands to run:

```bash
# 1. Apply all Kubernetes secrets
bash scalekit-secrets-<timestamp>.sh

# 2. Install the Helm chart
helm install scalekit oci://ar.scalekit.cloud/scalekit/charts/scalekit \
  --version <chart-version> \
  -n <namespace> \
  --values values-<timestamp>.yaml
```

<Aside type="caution" title="Check databases exist first">
  The script reminds you to verify that all three PostgreSQL databases exist before running `helm install`. The migration hook will fail if any database is missing.
</Aside>
...
echo "  2) Kubernetes cluster (GKE / cloud)"
read -rp "${YELLOW}Enter 1 or 2: ${RESET}" ENV_CHOICE
ENV_LABEL=$([[ "$ENV_CHOICE" == "1" ]] && echo "minikube" || echo "gke")
...
SVIX_REDIS_DSN="redis://${REDIS_HOST}:${REDIS_PORT}/${SVIX_REDIS_DB}#insecure"
echo
...
if [[ "$ENV_CHOICE" == "1" ]]; then
  ask APP_DOMAIN    "  Application base domain" "scalekit.local"
  APP_PROTOCOL="http"
