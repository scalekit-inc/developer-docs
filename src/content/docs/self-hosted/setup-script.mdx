...
EOF
  ...
  echo -e "${BOLD}${CYAN}└─────────────────────────────────────────────────────────────┘${RESET}"
  echo
  echo "  helm install scalekit oci://${REGISTRY_SERVER}/scalekit/charts/scalekit \\"
...
dim "  Scalekit images are hosted on a private container registry."
...
ask        REGISTRY_SERVER   "  Container registry server URL (provided during onboarding)"
