...
Scalekit is configured through a `values.yaml` file passed to Helm at install and upgrade time. This page shows two complete examples followed by a field-by-field reference.

The full schema is in `scalekit/values.schema.json` inside the chart package.

## Complete examples

Pick the example that matches your setup. Replace placeholder values before running `helm install`.
...
| `artifact-registry-secret` | Docker registry credentials for `ar.scalekit.cloud` |
