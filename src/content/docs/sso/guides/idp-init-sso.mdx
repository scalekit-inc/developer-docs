...
browseCentral:
  label: "Add safe IdP-driven SSO authentication"
  filterType:
    - "tutorial"
  category:
...
prev:
  ...
  label: 'Mapping user attributes'
next:
  link: '/sso/guides/test-sso/'
  label: 'Test SSO implementation'
...
<Aside type="note" title="Modular SSO requirement">
With Full Stack Auth enabled, Scalekit handles all authentication flows automatically. IdP-initiated SSO needs to be handled manually when using Modular SSO. Enable/Disable Full Stack Auth in **Dashboard > Authentication > General**
</Aside>
...
Implement this as a backend route, not a frontend page. Scalekit redirects the browser to the Initiate Login URL with a `GET` request; your server reads the `idp_initiated_login` parameter, validates it, and issues its own redirect to continue the flow — the route never renders a page.
