...
1. **Set up an initiate login endpoint**

   Create an endpoint in your application that starts the authentication process. When Scalekit receives an IdP-initiated SSO request, it redirects the user to this endpoint with an `idp_initiated_login` parameter.
   ...
   [Register this endpoint in Scalekit Dashboard](/guides/dashboard/redirects/#initiate-login-url)

2. **Extract information from the JWT token**
...
3. **Convert to SP-initiated flow**

   Use the extracted parameters to initiate a new SSO request. Here are implementation examples:

   <Tabs syncKey="tech-stack">
   <TabItem value="nodejs" label="Node.js">

   ```javascript showLineNumbers=false "idp_initiated_login"
   // 1. Your Initiate Login Endpoint receives a JWT
   ...
   ```py showLineNumbers=false "idp_initiated_login"
   # 1. Your Initiate Login Endpoint receives a JWT
   ...
   ```go showLineNumbers=false "idp_initiated_login"
   // 1. Your Initiate Login Endpoint receives a JWT
...
4. **Handle errors**
   ...
   If errors occur, the redirect URI will receive a callback with error information. See the <a href="/reference/errors/" target="_blank">error handling</a> guide for more details.
...
graph LR;
    subgraph "Part 1: IdP-initiated"
        User-->IdP-->Scalekit-->YourApp["Your App (Initiate login endpoint)"];
    end
