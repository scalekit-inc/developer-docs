...
IdP-initiated SSO is only available when using standalone SSO. To enable this, disable Full Stack Auth in **Dashboard > Authentication > General**. With Full Stack Auth enabled, Scalekit handles all authentication flows automatically.
</Aside>
...
## Benefits of Scalekit's approach

The advantages of using Scalekit's conversion approach are:

- **Enhanced security**: Converts risky IdP-initiated flows to secure SP-initiated flows
- **Seamless experience**: The additional redirect is almost instantaneous, providing a smooth user experience
- **Consistent handling**: Your application uses the same authentication flow for both SP and IdP-initiated requests
- **Better error handling**: Standard SP-initiated flow provides clearer error messages and debugging
