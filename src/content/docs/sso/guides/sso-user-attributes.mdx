...
browseCentral:
  label: "Map SSO user attributes from IdP"
  filterType:
    - "tutorial"
  category:
  ...
  icon: "book"
sidebar:
  label: Mapping user attributes to IdP
prev:
...
## Set up IdP attributes <Badge text="Okta example" variant="tip" />
...
```typescript title="src/auth/validateToken.ts"
import type { IdTokenClaim } from '@scalekit-sdk/node';

try {
  // Validate the ID token and cast to include your custom attributes
  // Security: Prevents replay attacks, forged tokens, and ensures token integrity
...
```python title="src/auth/validate_token.py"
...
```
<TabItem label="Go">
```go title="cmd/auth/validate_token.go"
...
```
<TabItem label="Java">
```java title="src/auth/ValidateToken.java"
