...
browseCentral:
  label: "Implement standalone enterprise SSO"
  filterType: ["tutorial"]
...
2. ## Redirect the users to their enterprise identity provider login page
   ...
   To receive a refresh token, include the `offline_access` scope in your authorization request. Note that refresh tokens are only available when "Full-Stack Auth" is enabled in your Scalekit dashboard.
...
6. ## Set up SSO with your existing authentication system
    ...
    If you do not have an authentication provider, refer to the [this guide](/fsa/quickstart/) to add authentication to your application
...
7. ## Onboard enterprise customers
   ...
   **Pre-check SSO availability** before redirecting users. This prevents failed redirects when a user's domain doesn't have SSO configured:
   ...
   ```javascript showLineNumbers=true title="check-sso-availability.js" {5}
