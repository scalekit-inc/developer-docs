---
title: Build standalone enterprise SSO
...
seeAlso:
  ...
  label: "Next steps"
  items:
    - title: "SSO connection admin guides"
      icon: "book"
...
4. ## Get user details from the callback
   ...
   # Validate and decode the ID token from the authentication result
   id_token_claims = scalekit_client.validate_access_token_and_get_claims(result.id_token)

   # Validate and decode the access token
   access_token_claims = scalekit_client.validate_access_token_and_get_claims(result.access_token)
   ```
   ...
   // Validate the access token from the authentication result
   isValid, err := scalekitClient.ValidateAccessToken(result.AccessToken)
   if err != nil {
   ...
   if !isValid {
       log.Printf("Access token was invalid")
       return
   }

   // Use the ID token claims from the authentication response as needed
   ...
   // Access the decoded ID token claims from the authentication result
   IdTokenClaims idTokenClaims = result.getIdTokenClaims();

   // Validate the access token
   try {
       boolean isValid = scalekitAuthClient.validateAccessToken(result.getAccessToken());
       if (isValid) {
...
6. ## Set up SSO with your existing authentication system
    ...
    If you do not have an authentication provider, refer to the [this guide](/authenticate/fsa/quickstart/) to add authentication to your application
