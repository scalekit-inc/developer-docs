...
2. ### Authorize users
    ...
    <Aside type="caution" title="Include your registered redirect_uri">
    Scalekit only accepts authorization requests that specify a `redirect_uri`  matching one of the **Initiate Login URL** configured in **Dashboard → API&nbsp;Config**. [Learn more](/fsa/reference/redirects)

    Omitting this parameter, or sending a value that is not whitelisted, results in an error.
    </Aside>
