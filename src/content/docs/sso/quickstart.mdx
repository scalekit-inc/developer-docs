...
codeSamples:
  ...
  label: "Next steps"
  items:
    - title: "See OIDC and SAML integrations"
      icon: "ripple"
      url: "/guides/integrations/"
    - title: "Explore SSO demo app"
      icon: "expressjs"
...
This guide explains how to implement enterprise-grade SSO support, enabling compatibility with major identity providers from the outset.
...
This quickstart guide walks you through the SSO implementation process. You will learn how to deliver enterprise authentication features without managing complex protocol details.
   ...
   <Aside type="note" title="SSO standalone mode">
    Standalone SSO enables direct integration with your customer's identity provider without creating or storing users in Scalekit.


    <details>
    <summary>Full details</summary>

    - Post identity verficiation auth flows from the customer's identity provider directly to your application.
    - No user records are created in Scalekit's database.
    - Session management and user management features are not available
    - Scalekit hosted auth pages are not available
    </details>

    To configure standalone SSO mode:
...
2. ## Redirect the users to their enterprise identity provider login page
   ...
   Generate an authorization URL and redirect users to their identity provider's login page.
...
3. ## Handle IdP-initiated SSO <Badge type="tip" text="Optional" />
   ...
   **Configure your initiate login endpoint** in [Dashboard > Authentication > Redirects](/guides/dashboard/redirects/#initiate-login-url)
   ...
   ```javascript showLineNumbers wrap title="handle-idp-initiated.js" {7-8,19-23}
   ...
   ```python showLineNumbers wrap title="handle_idp_initiated.py" {7-8,19-23}
   # Your initiate login endpoint receives the IdP-initiated login token
   ...
   ```go showLineNumbers title="handle_idp_initiated.go" {9-10,20-28}
   // Your initiate login endpoint receives the IdP-initiated login token
   ...
   ```java showLineNumbers title="HandleIdpInitiated.java" {11-12,27-35}
   // Your initiate login endpoint receives the IdP-initiated login token
   @GetMapping("/login")
...
5. ## Test your SSO integration
   ...
   Your environment includes a pre-configured test organization (found in **Dashboard > Organizations**) with domains like `@example.com` for testing.

   **Test scenarios:**

   <Tabs syncKey="test-scenario">
   <TabItem value="sp-initiated" label="SP-initiated SSO">

   Users start login from your application. Generate an authorization URL with the test organization ID:

   ```javascript showLineNumbers=false title="test-sp-initiated.js"
   // Use your test organization ID from the dashboard
   const options = {
     organizationId: 'org_32656XXXXXX0438'  // Test Organization ID
   };

   // Generate authorization URL and redirect to IdP Simulator
   const authUrl = scalekit.getAuthorizationUrl(
     'https://your-app.com/auth/callback',
     options
   );

   // User will see the IdP Simulator where they can enter test credentials
   res.redirect(authUrl);
   ```

   The IdP Simulator allows you to simulate user login with custom attributes and test the complete authentication flow.

   </TabItem>

   <TabItem value="idp-initiated" label="IdP-initiated SSO">

   Users start login from their identity provider portal. Your initiate login endpoint receives the request:

   ```javascript showLineNumbers=false title="test-idp-initiated.js"
   // Your initiate login endpoint receives IdP-initiated login token
   const { idp_initiated_login } = req.query;

   if (idp_initiated_login) {
     // Decode claims and convert to SP-initiated flow (see Step 3)
     const claims = await scalekit.getIdpInitiatedLoginClaims(idp_initiated_login);
     ...
     // Generate authorization URL to complete authentication
     const authUrl = scalekit.getAuthorizationUrl(redirectUri, options);
     return res.redirect(authUrl);
   }
   ```

   In the IdP Simulator, select "IdP initiated SSO" from the dropdown to test this flow.
   ...
   ```javascript showLineNumbers=false title="test-error-handling.js"
   // Extract error parameters from callback
   ...
   In the IdP Simulator, select the "Error" scenario to simulate authentication failures and verify your error handling.

   </TabItem>
   </Tabs>

   **Testing checklist:**
   - ✅ SP-initiated SSO with test organization
   - ✅ IdP-initiated SSO flow conversion
   - ✅ Error scenarios and user feedback

   Learn more: [Complete SSO testing guide](/sso/guides/test-sso/)

6. ## Onboard enterprise customers
   ...
   **Create an organization** for your customer in [Dashboard > Organizations](https://app.scalekit.com/organizations), then provide Admin Portal access using one of these methods:
   ...
   // Example: https://random-subdomain.scalekit.dev/magicLink/8930509d-68cf-4e2c-8c6d-94d2b5e2db43
   ...
   Learn more: [Embedded Admin Portal guide](/guides/sso/admin-portal/#embedded-admin-portal)
   ...
   **Pre-check SSO availability** before redirecting users. This prevents failed redirects when a user's domain doesn't have SSO configured:
   ...
   ```javascript showLineNumbers=false title="check-sso-availability.js" {1-2,7-14}
