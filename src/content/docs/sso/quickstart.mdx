...
sidebar:
  label: "Build standalone enterprise SSO"
head:
  - tag: style
    content: |
...
![Diagram showing the SSO authentication flow: User initiates login → Scalekit handles protocol translation → Identity Provider authenticates → User gains access to your application](@/assets/docs/sso-quickstart/1.png)
 ...
 SSO enables direct integration with your customer's identity provider without creating or storing users in Scalekit.
...
1. ## Set up Scalekit
   ...
   <InstallSDK />
    ...
    In case you want to use, standalone SSO:
