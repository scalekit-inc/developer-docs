---
title: Agent Connect Overview
description: Learn about Agent Connect, Scalekit's solution for connecting to third-party applications and executing tools through a unified API.
sidebar:
  order: 1
  label: "Overview"
tableOfContents:
...
Agent Connect is Scalekit's powerful solution that enables your applications to connect to third-party services and execute tools on behalf of your users. It provides a unified API interface for interacting with popular business applications like Gmail, Slack, Jira, and more.

## What is Agent Connect?

Agent Connect abstracts the complexity of integrating with multiple third-party APIs by providing:
...
```mermaid
graph TB
    A[Your Application] --> B[Agent Connect API]
    B --> C[Connected Account]
    C --> D[Provider API]
    
    subgraph "Agent Connect"
        B --> E[Authentication]
        B --> F[Tool Execution]
...
Agent Connect supports a wide range of popular business applications:
...
Agent Connect supports multiple authentication approaches:
...
Ready to start using Agent Connect? Here's what you need to do:

<CardGrid>
  <LinkCard title="Quickstart Guide" href="/connect/quickstart">
    Get up and running with Agent Connect in minutes
  </LinkCard>
...
- **Developer community**: Join other developers using Agent Connect
- **Support portal**: Get help with technical issues
- **Professional services**: Expert assistance for complex integrations
- **Training programs**: Learn advanced Agent Connect features

<Aside type="note">
**Ready to get started?** Check out our [Quickstart Guide](/connect/quickstart) to build your first Agent Connect integration in minutes.
</Aside>

Agent Connect makes it easy to build powerful integrations that connect your application to the tools your users already love. Start building today and unlock the full potential of connected applications.