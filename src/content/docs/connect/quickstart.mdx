...
Congratulations! You've successfully created implemented tool calls on behalf of a user.

In this example, we showed how to implement tool calls on behalf of a user. You can now use the same approach to implement tool calls for other providers and tools.

Let's make it more interesting by implementing a simple agent that can fetch the last 5 unread emails from the user's inbox and send an email to the user. Go through our Agentic Tool Calling guide to implement the same usecase using Langchain and Agent Connect.
