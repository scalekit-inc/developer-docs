---
title: Providers
description: Learn about third-party application providers supported by Agent Connect and how they enable tool execution across different platforms.
sidebar:
  order: 2
  label: "Providers"
tableOfContents:
...
Providers in Agent Connect represent third-party applications that your users can connect to and interact with through Scalekit's unified API. Each provider offers a set of tools and capabilities that can be executed on behalf of connected users.
...
Providers expose various tools that can be executed through Agent Connect:
...
<Steps>
1. **Navigate to providers** in your Agent Connect dashboard
...
Agent Connect abstracts provider-specific APIs into a unified interface:
...
Different providers may return different error formats. Agent Connect normalizes these into consistent error responses:
