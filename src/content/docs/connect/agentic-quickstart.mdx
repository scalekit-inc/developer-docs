...
tools =  connect.langchain.get_tools(
    identifier="user_123",
    filter =Filter(
        provider = "GMAIL" # all tools for provider used by default
    )
)

# create a langchain agent and supply the tools to the agent. We are using gpt-4o as the LLM for this example.
...
result = agent_executor.invoke(
    {
        "input":
            '''
            Fetch last 5 unread emails from my inbox and summarize them.
            ''',
    }
)
```
...
Congratulations! You've successfully built a langchain agent and gave it access to gmail tools to execute tool calls on behalf of a user.
