...
When an administrator deletes a group from the directory, Scalekit removes that group from every member's group list and recalculates each member's role from their remaining group memberships. This happens even for users who belong to multiple mapped groups, so deleting one group never leaves a stale role or membership behind for a user who is still a member of other groups.
