---
title: Add Modular SCIM provisioning
description: "Automate user provisioning with SCIM. Directory API and webhooks for real-time user data sync"
tags: [scim, quickstart, provisioning, directory-sync, automation, users, groups, enterprise]
head:
  - tag: style
    content: |
...
seeAlso:
  expanded: true
  items:
    - title: "Browse python example app"
      icon: "python"
      url: "https://github.com/scalekit-inc/python-scim-example"
    - title: "Browse nextjs example app"
      icon: "nextjs"
      url: "https://github.com/scalekit-inc/nextjs-example-apps/tree/main/webhook-events"
---

import { LinkButton, Aside, Steps, TabItem } from '@astrojs/starlight/components';
...
<ProductAgentBlock skill="implement-scim" />
...
1. ### Create a secure webhook endpoint
    ...
    public ResponseEntity<String> webhook(
        ...
        verifyHeaders.put("webhook-id", headers.get("webhook-id"));
        verifyHeaders.put("webhook-timestamp", headers.get("webhook-timestamp"));
        verifyHeaders.put("webhook-signature", headers.get("webhook-signature"));

        try {
            // Java throws WebHookException. It does not return false.
            scalekitClient.webhook()
                .verifyWebhookPayload(secret, verifyHeaders, body.getBytes(java.nio.charset.StandardCharsets.UTF_8));
    ...
    http.HandleFunc("/webhook", func(w http.ResponseWriter, r *http.Request) {
        ...
        valid, err := scalekitClient.VerifyWebhookPayload(webhookSecret, headers, body)
        if err != nil || !valid {
...
2. ### Register your webhook endpoint
    ...
    Use request bin services like Beeceptor or webhook.site for initial testing. Refer to our [webhook setup guide](/directory/reference/directory-events/) for detailed testing instructions.
    </Aside>
