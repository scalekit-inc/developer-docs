...
excerpt: >
  Login is not enough. A signed-in user can still call every tool unless each tool checks an OAuth scope. This recipe authenticates with Scalekit, grants todo:read and todo:write, and tests both the allow path and the deny path in the mcp-use Inspector.
featured: false
authors:
  - name: 'Saif'
