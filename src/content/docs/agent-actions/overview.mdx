...
Providers are third-party applications that your users can authenticate with. Agent Actions supports OAuth 2.0 flows for popular platforms including Gmail, Slack, GitHub, Jira, and many more.
...
After the user authorizes your application, the provider redirects back with an authorization code. Agent Actions automatically exchanges this code for access and refresh tokens:

```javascript
// Agent Actions handles the OAuth callback and token exchange
...
<CardGrid>
  <LinkCard title="Quickstart Guide" href="/agent-actions/quickstart">
    Get up and running with Agent Auth in minutes
  </LinkCard>
  <LinkCard title="Authentication Flows" href="/agent-actions/authentication/auth-flows-comparison">
    Compare different authentication flow patterns
  </LinkCard>
  <LinkCard title="Providers" href="/agent-actions/providers">
    View supported OAuth providers and their requirements
  </LinkCard>
  <LinkCard title="Token Management" href="/agent-actions/authentication/token-management">
...
<Aside type="note">
**Ready to get started?** Check out our [Quickstart Guide](/agent-actions/quickstart) to implement your first OAuth integration with Agent Auth in minutes.
</Aside>
