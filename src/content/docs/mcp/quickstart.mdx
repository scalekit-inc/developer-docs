...
For a deeper explanation of why an OAuth layer is essential for remote MCP servers, read the <a href="/mcp/overview" target="_blank" rel="noopener">MCP authorization overview</a>.
...
![MCP server registration](@/assets/docs/guides/mcp/mcp_create.png)

**Basic configuration:**

- **Server name**: A display name that users will see during authorization and in the Consent Screen(e.g., "My ToDo MCP Server")
...
**Advanced settings:**
These settings are optional and can be configured at a later time. 
- **Resource identifier**: Your MCP server's unique identifier, typically your server's URL (e.g., `https://mcp.yourapp.com`). This is an optional field. If not provided, Scalekit will use the generated resource_id as the resource identifier. If provided, access tokens minted by Scalekit will have the resource identifier as `aud` claim along with the Scalekit generated resource_id.

import { Aside } from '@astrojs/starlight/components';

<Aside type="caution" title="Using FastMCP?">
If you're using FastMCP, you must provide a resource identifier. For a server running at `https://mcp.example.com/mcp`, register `https://mcp.example.com/` as the resource identifier.
</Aside>


- **Access token lifetime**: Recommended 300-3600 seconds (5 minutes to 1 hour)

- **Scopes**: Define the permissions your MCP server needs, such as `usr:read` or `usr:write`
...
![Resource Metadata Information](@/assets/docs/guides/mcp/mcp_prm_configure.png)
...
app.get('/.well-known/oauth-protected-resource', (req, res) => {
  res.json({
    "authorization_servers": [
      "https://<SCALEKIT_ENVIRONMENT_URL>/resources/res_xxxxxx"
    ],
...
**Configure Auth Methods:** We have documented how you can configure different auth methods like [Social Logins](/mcp/auth-methods/social) or [Enterprise Identity Providers](/mcp/auth-methods/enterprise/) like Okta, Microsoft Entra ID etc. in our documentation.

**Bring your own Auth:** If you already have an authentication system, you can integrate your existing authentication system with Scalekit’s drop-in OAuth 2.1 authorization layer by following this comprehensive [guide](/mcp/auth-methods/custom-auth/).
