...
2. ## Register your MCP server
   ...
   **Access control settings:**
   ...
   - **Allow dynamic client registration** — Enables MCP clients to register automatically without manual approval. For security reasons, Scalekit ensures that any client that registers via DCR has to implement PKCE-based OAuth 2.1 flow to prevent authorization code interception attacks <Badge variant="note" text="boolean" />
