...
Once you're satisfied with the quickstart example, extend `server.py` with your own FastMCP tools or replace the in-memory store with your production data source. Scalekit's provider handles authentication for any toolset you add. Continue with the <a href="/mcp/quickstart/">MCP OAuth 2.1 implementation guide</a> to enforce additional grant types, rotate credentials, and automate client onboarding.
