---
title: 'FastMCP integration quickstart 🚀'
...
This guide shows you how Scalekit's provider secures any FastMCP server. You'll register a sample server as a protected resource, configure a local FastMCP runtime, and confirm that every request carries todo scopes before connecting an MCP client.

Use this quickstart when you want a working reference for authorizing any FastMCP project with Scalekit. We walk through a simple todo server so you can reuse the pattern in your own tools.
...
1. ## Register your MCP server in Scalekit
    ...
    <Aside type="note" title="Replace this GIF">
      Swap the placeholder with a GIF that shows navigating to <strong>Dashboard &gt; MCP Servers</strong>, creating the FastMCP entry, adding the <code>todo:read</code> and <code>todo:write</code> scopes, and saving the configuration.
    </Aside>
...
2. ## Create your FastMCP todo server
    ...
    <Aside type="tip" title="Alternate Python versions">
      Replace <code>python3.11</code> with <code>python3.12</code> if that's the interpreter you have available.
    </Aside>
...
4. ## Implement the FastMCP todo server
    ...
    <Aside type="tip" title="Swap in your own tools">
      Replace the todo handlers with your project-specific tools. The Scalekit provider continues to enforce scopes for any additional FastMCP tooling you register.
    </Aside>
...
5. ## Provide runtime secrets
   ...
   <table>
    ...
    <tbody>
        ...
        <td>Your Scalekit environment URL used for OAuth discovery.</td>
        </tr>
        <tr>
        <td><code>SCALEKIT_CLIENT_ID</code></td>
        <td>The client ID for the MCP server resource (from the dashboard step above).</td>
        </tr>
        <tr>
        <td><code>SCALEKIT_RESOURCE_ID</code></td>
        <td>The resource identifier assigned to your MCP server in Scalekit.</td>
        </tr>
        <tr>
        <td><code>MCP_URL</code></td>
        <td>The full public URL you registered (keep the trailing slash, e.g., <code>http://localhost:3002/</code>).</td>
        </tr>
        <tr>
...
6. ## Run the FastMCP server locally
    ...
    <Aside type="note" title="Replace this GIF">
      Insert a GIF that captures activating the virtual environment, running <code>python server.py</code>, and the log lines confirming the server is listening.
    </Aside>
...
7. ## Connect with an MCP client
    ...
    In the Inspector UI, point the client to `http://localhost:3002/`, provide a token that includes `todo:read` and `todo:write`, and run any tool (the example exposes `create_todo`, `list_todos`, `get_todo`, `update_todo`, `delete_todo`).

    ![MCP Inspector](@/assets/docs/guides/mcp/mcp-inspector-fastmcp.png)

    <Aside type="note" title="Replace this GIF">
      Record a short GIF that demonstrates launching the MCP Inspector, configuring the MCP URL, supplying a valid token, and running a todo tool successfully.
    </Aside>
...
You can get the full code example at <a href="https://github.com/scalekit-inc/mcp-demo/tree/main/todo-fastmcp" target="_blank" rel="noreferrer">github.com/scalekit-inc/mcp-demo/tree/main/todo-fastmcp</a>.
