...
In the Scalekit dashboard, open **[Dashboard](https://app.scalekit.com) -> Settings -> Authentication -> Auth Methods -> Social Login**. You should see **Google** listed as `Enabled`.
...
Create `.env` with the values from your Scalekit workspace (replace the placeholders):
...
Create `.env` with your workspace values:

```bash
cat <<'ENV' > .env
PORT=3002
SK_ENV_URL=https://<your-workspace>.scalekit.com
SK_CLIENT_ID=<client-id-for-google-connection>
SK_CLIENT_SECRET=<client-secret-for-google-connection>
MCP_SERVER_ID=<mcp-server-id-from-dashboard>
PROTECTED_RESOURCE_METADATA='<resource-metadata-json>'
EXPECTED_AUDIENCE=http://localhost:3002/
ENV
