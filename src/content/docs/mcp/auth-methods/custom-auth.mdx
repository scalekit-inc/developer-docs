...
Scalekit also offers the option to integrate your existing authentication infrastructure with Scalekit's OAuth layer for MCP servers. **Use this when you have an existing auth system and want to add MCP OAuth without migrating users.** This approach is ideal when you want to provide a unified authentication experience across your application.
...
- Sessions are shared between your application and MCP server, eliminating duplicate logins
...
MCP Client -> Scalekit Auth Server: Initiate /oauth/authorize (Dynamic Client Registration)
