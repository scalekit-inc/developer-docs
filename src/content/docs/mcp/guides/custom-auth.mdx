...
description: Layer Scalekit OAuth 2.1 on top of your login system​
sidebar:
  label: 'Add OAuth 2.1 over existing auth'
tableOfContents:
  maxHeadingLevel: 3
head:
  - tag: style
...
prev:
  label: "Add auth to MCP server"
  link: "/mcp/oauth"
---
import Subtitle from '@/components/ui/Subtitle.astro'
import { CardGrid, Badge, Tabs, TabItem, Steps, Aside } from '@astrojs/starlight/components';

<Subtitle>Integrate your existing authentication system with Scalekit's OAuth 2.1 authorization layer for MCP servers</Subtitle>
...
## How Scalekit bridges MCP clients with your authentication system
...
"MCP Client" -> "Scalekit": Initiate /oauth/authorize
"Scalekit" -> "Your App": Redirect to /login\n?login_request_id & state
"Existing Auth System" <- "Your App": provider/app login box
"Your App" <- "Existing Auth System": Return user details
"Your App" -> "Scalekit": POST user details to /auth-requests/login_request_id/user
"Scalekit" -> "Your App": 200 Success response
"Your App" -> "Scalekit": Redirect to /partner:callback\n?state
"Scalekit" -> "MCP Client": Continue consent,\ntoken exchange, etc.
...
1. ## Initiate authentication flow
   ...
   **Example redirect URL:**
   ...
   ```http showLineNumbers=false frame="none"
   ...
   <Aside type="caution" title="Security requirement">
     Always validate the `login_request_id` before processing the authentication request. This prevents unauthorized authentication attempts and ensures the request originates from Scalekit.
   </Aside>
...
2. ## Authenticate the user in your system
   ...
   **Optional attributes:**
   ...
   <Aside type="note">
     ...
     - `{{envurl}}` — Your Scalekit environment URL
...
3. ## Redirect back to Scalekit
   ...
   **Callback URL format:**
   ...
   <Aside type="caution" title="State validation">
     Always verify that the `state` value you send back matches exactly what you received initially. Mismatched state values indicate a potential security issue and should be rejected.
   </Aside>
