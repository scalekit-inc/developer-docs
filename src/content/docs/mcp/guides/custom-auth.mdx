...
MCP Client -> Scalekit : "Initiate /oauth/authorize (dynamic client)"
Scalekit -> Customer App: "Redirect to /login?login_request_id & state"
Scalekit <- Customer App: "POST user details (machine to machine)"
Scalekit -> Customer App: "200 Success Response"
Scalekit <- Customer App: "Redirect to /partner:callback?state"
MCP Client <-> Scalekit : "Continue consent, token exchange, etc"
```
...
```bash title="Example cURL" showLineNumbers=false wrap
...
```http title="Example Redirect URL" showLineNumbers=false wrap
