...
```sh title="Console output" showLineNumbers=false wrap
  ...
  Login process begins over the browser. The authorization endpoint that you've generated should be a GET request from your application from the browser. The above example will redirect the user to scalekit's login page.

  ![](@/assets/docs/manual/getting-started/authenticate-users/3-loginbox.png)


  Users can choose to continue with their email or social provider. Upon successful authentication, scalekit will callback the `redirect_uri` that you've provided (which is also pre-registered in the dashboard) in the authorization request with an authorization code.

  ## Follow redirects for authorization code

  Once the user is authenticated, the Scalekit environment will redirect the user back to your application with an authorization code. An authorization code is a temporary code (<u>and is valid for only 5 minutes</u>) that your application can exchange for user profile details. This step is security measure to ensure that the authorization code is only used once and is not intercepted by a malicious actor.
