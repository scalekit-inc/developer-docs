...
Both `access_token` and `id_token` are JWT tokens that you can [decode to see their contents](/guides/extended/decoding-jwts). You can use tools like [authplay.io](https://authplay.io/pg/token-explorer) or [jwt.ms](https://jwt.ms) to inspect them.
...
3. **Session tracking**: [Set a cookie](/guides/extended/cookies) in your application that contains an identifier linked to the user's tokens.
...
You can configure the following session-related timeouts in your environment settings or [via the API](/reference/api/environments#update-environment):
