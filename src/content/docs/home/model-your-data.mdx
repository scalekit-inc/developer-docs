...
Keep your existing database as the source of truth. Store your database's primary key in the `external_id` field to maintain the connection between Scalekit and your system. For extra data, use the `metadata` field to store custom key-value pairs.

You can add `external_id` and `metadata` fields when creating organizations and users. These fields are optional and available when you fetch the data later.
