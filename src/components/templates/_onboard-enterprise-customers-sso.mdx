...
4. ## Verify domain ownership
   ...
   After SSO is configured, verify the organization's email domains to enable automatic SSO routing. When domains are verified, users with matching email addresses are automatically redirected to their organization's SSO login. This is also called as **Home Realm Discovery**.
   ...
   ### Manual verification (no DNS required)

   Domains added via the Scalekit Dashboard or API are marked as **Admin-verified** — no DNS challenge is required. Use this when you have already confirmed domain ownership through another process (contract, business verification, etc.) and want to activate SSO routing immediately.
   ...
   **Via Scalekit Dashboard:**
   ...
   ![Organization domain verification in dashboard](@/assets/docs/_onboard-enterprise-customers-sso/org_domain.png)
   ...
   **Via API:**
   ...
   ```sh frame="terminal" showLineNumbers=false
   ...
   ```ts
   const domain = await scalekit.organization.createDomain(organizationId, {
     domain: 'megacorp.com',
   ...
   ```python
   domain = scalekit_client.organization.create_domain(
       organization_id,
       domain="megacorp.com",
       domain_type="ORGANIZATION_DOMAIN",
   )
   ...
   ```go
   domain, err := scalekitClient.Organization().CreateDomain(
       ctx,
       organizationID,
   ...
   ```java
   Domain domain = scalekitClient.organizations()
       .createDomain(organizationId, new CreateDomainOptions()
           .setDomain("megacorp.com")
   ...
   ### DNS verification (self-serve for org admins)

   DNS verification lets your customer's IT admin prove domain ownership by publishing a TXT record — without involving your team. Scalekit displays the TXT record to publish, then polls DNS automatically until verified.

   1. Enable the `Domain Verification` feature for the organization (Dashboard org **Overview** → toggle **Domain Verification**, or via the [API](/apis/#tag/organizations/PATCH/api/v1/organizations/{id}/settings)).
   2. Generate an Admin Portal link and share it with the org admin.
   3. The admin enters the domain in the portal, copies the displayed TXT record, and adds it to their DNS provider (e.g., Cloudflare, Route 53, GoDaddy).
   4. Scalekit detects the TXT record automatically and marks the domain as **Verified**.
