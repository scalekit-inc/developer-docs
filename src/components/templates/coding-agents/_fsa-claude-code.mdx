...
1. ## Install the authstack plugin
   ...
   Select Claude Code when prompted. The CLI installs the authstack plugin for you. Then ask the agent to run the shipped skill `implement-saaskit`.

   Done: the command exits 0 and `implement-saaskit` is available.
