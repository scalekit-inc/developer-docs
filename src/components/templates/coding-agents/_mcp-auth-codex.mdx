...
2. ## Enable the MCP OAuth skill

   Restart Codex, then open the Plugin Directory and enable the authstack plugin.

   Load the shipped skill `adding-mcp-oauth`. That skill includes the workflows, framework-specific guidance, and references Codex uses to generate OAuth 2.1 protection for remote MCP servers.

   Done: `adding-mcp-oauth` is available to Codex.
