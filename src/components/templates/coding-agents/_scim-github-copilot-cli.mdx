...
1. ## Install the authstack plugin (recommended)
   ...
   The CLI installs the authstack plugin for GitHub Copilot. Then ask the agent to run the shipped skill `implementing-scim-provisioning`.

   Done: the command exits 0 and `implementing-scim-provisioning` is available.
