...
1. ## Install the authstack plugin (recommended)
   ...
   The CLI installs the authstack plugin for GitHub Copilot. Then ask the agent to run the shipped skill `implement-sso`.

   Done: the command exits 0 and `implement-sso` is available.
