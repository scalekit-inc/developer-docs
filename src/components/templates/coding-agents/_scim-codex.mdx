...
2. ## Enable the SCIM skill

   Restart Codex, then open the Plugin Directory and enable the authstack plugin.

   Load the shipped skill `implementing-scim-provisioning`. That skill includes the workflows, references, and prompts Codex uses to generate SCIM provisioning and deprovisioning code for your application.

   Done: `implementing-scim-provisioning` is available to Codex.
