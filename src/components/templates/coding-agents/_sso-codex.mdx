...
2. ## Enable the SSO skill

   Restart Codex, then open the Plugin Directory and enable the authstack plugin.

   Load the shipped skill `implementing-modular-sso`. That skill includes the workflows, references, and prompts Codex uses to generate SAML and OIDC SSO code for your existing application.

   Done: `implementing-modular-sso` is available to Codex.
