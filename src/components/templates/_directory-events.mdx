...
<Aside type="note">
Deleting a directory group removes it from every affected member and recalculates each member's role from their remaining group memberships, even if the user belongs to multiple mapped groups. Scalekit sends an `organization.directory.user_updated` event for each affected member alongside this `organization.directory.group_deleted` event.
</Aside>
