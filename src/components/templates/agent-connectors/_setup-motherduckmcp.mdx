{/* TODO: stub cloned from _setup-atlassianmcp.mdx for MotherDuck MCP. Review and update connector-specific references (URLs, scopes, app-registration steps) before merging. */}
...
1. ### Copy the redirect URI from Scalekit
   ...
   In the [Scalekit dashboard](https://app.scalekit.com), go to **AgentKit** > **Connections** > **Create Connection**. Find **MotherDuck MCP** and click **Create**. Copy the redirect URI — it looks like `https://<SCALEKIT_ENVIRONMENT_URL>/sso/v1/oauth/<CONNECTION_ID>/callback`.

   {/* TODO: add screenshot — alt: "Copy redirect URI from Scalekit dashboard for MotherDuck MCP", original src: @/assets/docs/agent-connectors/motherduckmcp/copy-redirect-uri.png */}
...
3. ### Add the redirect URI as a domain
   ...
   {/* TODO: add screenshot — alt: "Your domains section in MotherDuck MCP server administration", original src: @/assets/docs/agent-connectors/motherduckmcp/add-domain-redirect-uri.png */}
   - In the **Add domain** dialog, paste the redirect URI from Scalekit into the **Domain** field.
   - Accept the terms and click **Add**.

   {/* TODO: add screenshot — alt: "Add domain dialog in MotherDuck MCP server", original src: @/assets/docs/agent-connectors/motherduckmcp/add-domain-modal.png */}
