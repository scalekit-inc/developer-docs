...
<Aside type="note" title="Default GitHub connection">
  New Scalekit environments include a pre-configured GitHub connection named `github-connect` that uses Scalekit's managed OAuth credentials with the `user:email`, `repo`, and `public_repo` scopes. Use it to make GitHub tool calls without any setup. Follow the steps below only if you want to use your own GitHub OAuth app instead.
</Aside>
