...
- Use `diarize_create_transcription_job` to submit a URL for transcription. Returns a `job_id` and an `estimatedTime` (in seconds) for how long processing will take.
...
<Tabs syncKey="tech-stack">
  <TabItem label="Python">
    ...
    for segment in transcript_result.result.get("segments", []):
        print(f"[{segment.get('start', 0):.1f}s] {segment.get('speaker', '?')}: {segment.get('text', '')}")
    ```
  </TabItem>
  <TabItem label="Node.js">
    ...
    for (const segment of transcriptResult.data.segments ?? []) {
...
Submit a YouTube, X, Instagram, or TikTok URL for transcription. Returns a `job_id` and `estimatedTime` to use in subsequent status and download calls.
...
estimated_seconds = result.result.get("estimatedTime", 120)
```
...
Download a completed transcript. `json` includes full speaker diarization, segment timestamps, and word-level data. `txt`, `srt`, and `vtt` are plain-text and subtitle formats.

```python title="examples/diarize_download_transcript.py"
result = scalekit_client.actions.execute_tool(
    tool_name="diarize_download_transcript",
    connected_account_id=connected_account.id,
    tool_input={"job_id": job_id, "format": "json"},  # json | txt | srt | vtt
)
for segment in result.result.get("segments", []):
