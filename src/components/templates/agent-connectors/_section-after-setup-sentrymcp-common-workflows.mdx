{/* TODO: stub cloned from _section-after-setup-atlassianmcp-common-workflows.mdx for Sentry MCP. Review and update connector-specific references (URLs, scopes, app-registration steps) before merging. */}
...
<Aside type="note" title="Call this tool first">
  Run `sentrymcp_getaccessibleatlassianresources` before calling any Jira or Confluence tool. The response lists every Atlassian site the user has access to. Use the `id` field as `cloudId`.
</Aside>
...
<Tabs syncKey="tech-stack">
  ...
  <TabItem label="Python">
    ...
    resources = actions.execute_tool(
        connection_name="sentrymcp",
        identifier="user_123",
        tool_name="sentrymcp_getaccessibleatlassianresources",
        tool_input={},
    )
    ...
    issue = actions.execute_tool(
        connection_name="sentrymcp",
        identifier="user_123",
        tool_name="sentrymcp_getjiraissue",
        tool_input={
            "cloudId": cloud_id,
            "issueIdOrKey": "KAN-1",
        },
