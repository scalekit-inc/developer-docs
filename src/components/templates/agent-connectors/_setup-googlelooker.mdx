...
2. ### Enable the Looker API
   ...
   - In [Google Cloud Console](https://console.cloud.google.com/), go to **APIs & Services** > **Library**. Search for "Looker (Google Cloud core) API" and click **Enable**.

     ![Looker (Google Cloud core) API product page showing the Enable button and API Enabled status](@/assets/docs/agent-connectors/googlelooker/enable-looker-api.png)
...
3. ### Create OAuth credentials in Google Cloud Console
   ...
   <Aside type="tip" title="Get the redirect URI first">
     Open the Scalekit dashboard (step 4) to copy the redirect URI before creating the OAuth client. You can always edit the OAuth client to add the URI after creation.
   </Aside>
