...
/** Shared 0–5 shell. Product-only fields stay in each export. */
function playbook(p: PlaybookFields) {
  return `Build with ${p.title}.

Load https://docs.scalekit.com/llms.txt, then the ${p.llmsSet} at ${p.llmsUrl}, before writing any Scalekit code. The installed ${p.skill} skill is the source of truth for ${p.truthFor}. Credentials live in the environment.
...
2. Skill — install ${p.skill}, non-interactive:
   ${skillInstall(p.skill)}
...
4. Implement — load ${p.skill} and run its steps. ${p.implement}
   Done: the skill's own checklist is complete.

5. Handoff — return proof plus ${p.handoff}.
   Done: both are in your reply.

   ${p.proof}`
}

export const agentkitPrompt = playbook({
  title: 'AgentKit',
  llmsSet: 'AgentKit set',
  llmsUrl: 'https://docs.scalekit.com/_llms-txt/agentkit.txt',
  skill: agentkitSkill,
...
export const saaskitPrompt = playbook({
  title: 'SaaSKit',
  llmsSet: 'SaaSKit Complete set',
  llmsUrl: 'https://docs.scalekit.com/_llms-txt/saaskit-complete.txt',
  skill: saaskitSkill,
  truthFor: 'APIs, SDK calls, and session names',
  intro:
    'This run is SaaSKit. Humans log into my app. Scalekit hosts login. My app stores the session.',
  step0: `0. Product — SaaSKit. Do not add AgentKit tool calls, MCP well-known metadata, or a Modular Auth switch.
  ...
  credentialsDone: 'Done: all four names are set in .env. Code reads them from the environment.',
  implement: `Send the user to Scalekit authorize. Exchange the code on the callback. Store the session. Add logout.
  ...
  handoff: 'https://app.scalekit.com → Authentication → Redirect URLs',
  proof: `Proof:
...
export const mcpAuthPrompt = playbook({
  title: 'MCP Authentication',
  llmsSet: 'MCP Authentication set',
  llmsUrl: 'https://docs.scalekit.com/_llms-txt/mcp-authentication.txt',
  skill: mcpSkill,
  truthFor: 'APIs, SDK calls, and metadata',
  intro:
  ...
  step0: `0. Product — MCP Authentication for a server I host. Transport is Streamable HTTP. Do not use stdio.
  ...
  implement:
    'Serve Protected Resource Metadata at /.well-known/oauth-protected-resource with no auth middleware. Copy Metadata JSON from Dashboard → MCP servers → your server. On a missing or bad token, return 401 with WWW-Authenticate that points at that well-known URL.',
  handoff: 'https://app.scalekit.com → MCP servers',
  proof: `Proof:
   - the MCP server URL
...
export const modularSsoPrompt = playbook({
  title: 'Modular SSO',
  llmsSet: 'Enterprise SSO and SCIM set',
  llmsUrl: 'https://docs.scalekit.com/_llms-txt/enterprise-sso--scim.txt',
  skill: ssoSkill,
  truthFor: 'APIs, SDK calls, and connection selectors',
  intro:
  ...
  step0: `0. Product — Modular SSO. Set Dashboard → Settings → Authentication Mode to Modular Auth. Do not disable a Full-Stack Auth toggle on Authentication → General. Do not add offline_access.
  ...
  credentialsDone: 'Done: all three names are set in .env. I have a real Test Organization id.',
  implement:
    'The authorize URL must pass one selector: organization_id, connection_id, or login_hint. Implement IdP-initiated login on the initiate-login URL. Create the app session in the callback. Modular SSO does not create Scalekit users or hosted sessions.',
  handoff: 'https://app.scalekit.com → Organizations → Test Organization',
  proof: `Proof — an authorization URL that opens the SSO simulator, using any one of:
...
export const modularScimPrompt = playbook({
  title: 'Modular SCIM',
  llmsSet: 'Enterprise SSO and SCIM set',
  llmsUrl: 'https://docs.scalekit.com/_llms-txt/enterprise-sso--scim.txt',
  skill: scimSkill,
  truthFor: 'APIs, SDK calls, and webhook events',
  intro:
  ...
  step0: `0. Product — Modular SCIM. Do not generate getAuthorizationUrl, a connected-account link, or the admin portal. The portal is implement-sso.
  ...
  credentialsDone: 'Done: all four names are set in .env. Code reads them from the environment.',
  implement:
    'Expose a public HTTPS webhook. Verify the signature. Upsert or deactivate local users. Subscribe at least to organization.directory.user_created, organization.directory.user_updated, and organization.directory.user_deleted. Local dev needs a tunnel.',
  handoff: 'https://app.scalekit.com → Webhooks',
  proof: `Proof:
