...
export const tools: Tool[] = [
  {
    ...
    params: [
      { name: 'runId', type: 'string', required: true, description: `The ID of the Actor run to abort.` },
      { name: 'gracefully', type: 'boolean', required: false, description: `If true, the Actor run will abort gracefully with a 30-second timeout.` },
    ],
  },
  {
...
- You don't know the Actor's input schema — use apifymcp_fetch_actor_details first`,
    params: [
      ...
      { name: 'waitSecs', type: 'integer', required: false, description: `Seconds to wait for completion (0–45, default 30). Returns with current run status if not terminal within waitSecs.` },
    ],
  },
  {
...
- You want the output data — use apifymcp_get_actor_output with the datasetId`,
    params: [
      ...
      { name: 'waitSecs', type: 'integer', required: false, description: `Maximum seconds to wait for the run to reach a terminal state (SUCCEEDED, FAILED, ABORTED, TIMED-OUT).
0 returns immediately with the current status. Cap: 45. Default: 30.` },
  ...
  },
  {
    ...
    params: [
      { name: 'datasetId', type: 'string', required: true, description: `Dataset ID or username~dataset-name.` },
      { name: 'clean', type: 'boolean', required: false, description: `If true, returns only non-empty items and skips hidden fields (those starting with #).` },
      { name: 'desc', type: 'boolean', required: false, description: `If true, returns results in reverse order (newest to oldest).` },
      { name: 'fields', type: 'string', required: false, description: `Comma-separated list of fields to include. Supports dot notation for nested fields, e.g. metadata.url.` },
      { name: 'flatten', type: 'string', required: false, description: `Comma-separated fields to flatten into dot-notation keys. Usually inferred automatically from dot notation in fields.` },
      { name: 'limit', type: 'integer', required: false, description: `Maximum number of items to return. Defaults to 20.` },
      { name: 'offset', type: 'number', required: false, description: `Number of items to skip for pagination. Default is 0.` },
      { name: 'omit', type: 'string', required: false, description: `Comma-separated list of fields to exclude from results.` },
    ],
  },
  {
    ...
    params: [
      { name: 'keyValueStoreId', type: 'string', required: true, description: `Key-value store ID or username~store-name.` },
      { name: 'recordKey', type: 'string', required: true, description: `Key of the record to retrieve.` },
    ],
  },
  {
...
- User needs repeated/scheduled scraping of a specific platform — search for a dedicated Actor using apifymcp_search_actors instead`,
    params: [
      ...
      { name: 'waitSecs', type: 'integer', required: false, description: `Max seconds (0–45, default 30) to cap the wait for the Actor run to reach terminal state. For long-running Actors the response returns at the cap with the current run status; follow \`nextStep\` to poll via get-actor-run. Set to 0 to fire-and-forget.` },
    ],
  },
  {
