...
- "2hrs on NVIDIA pitch deck" â blocks: [{project: "NVIDIA", description: "Pitch deck work", durationMin: 120}]
- "30min call with Acme about onboarding" â blocks: [{client: "Acme", description: "Onboarding call", durationMin: 30}]

When blocks are provided: defaults to preview mode â shows matched entries for confirmation. Call again with save=true to commit.
...
The tool fetches the user's clients, projects, tasks, existing time entries, app activity, and existing notes for the target date. It detects overlaps between blocks and existing entries.`,
  ...
  },
  {
    ...
    description: `Create a new time entry with optional client, project, and task assignment. Supports idempotency keys to prevent duplicate entries on retry. Times must be in ISO 8601 format â convert user-local times to their timezone (provided as _user_timezone in responses) before sending.`,
    params: [
      {
        name: 'end_time',
        type: 'string',
...
IMPORTANT: Always provide the \`blocks\` parameter. Parse the user's text yourself before calling this tool.`,
  ...
  },
  {
    ...
    description: `Generate AI time entries for a time range. Analyzes the user's actual activity â apps, websites, meetings â and uses clustering to create multiple entries based on natural activity groups. By default, skips time slots where previous entries were rejected. Rate limited: 15 per minute.`,
    params: [
      {
        name: 'end_time',
        type: 'string',
  ...
  },
  {
    ...
    description: `Invite a new member to a team by email. Sends an invitation email. Requires team admin permissions. Naturally idempotent â re-inviting an existing member returns the existing record.`,
    params: [
      {
        name: 'email',
        type: 'string',
  ...
  },
  {
    ...
    description: `List contracts for an organization. Contracts track billing arrangements with clients including hourly rates, retainers, and profitability metrics. Archived contracts are excluded by default â pass status to filter. Use contract IDs with get_contract_profitability.`,
    params: [
      { name: 'org_id', type: 'string', required: true, description: `Organization (billing) ID` },
      { name: 'cursor', type: 'string', required: false, description: `Pagination cursor` },
