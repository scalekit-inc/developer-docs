...
export const tools: Tool[] = [
  {
    ...
    params: [
      { name: 'description', type: 'string', required: true, description: `High-level description of what the project should do.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'flow_id', type: 'string', required: true, description: `Project ID (from \`\`list_projects\`\`).` },
      { name: 'cursor', type: 'string', required: false, description: `Opaque pagination token from a prior response's
\`\`pagination.next_cursor\`\`. Pass \`\`null\`\` to fetch
the first page.` },
      { name: 'include_flow_json', type: 'string', required: false, description: `When true, also include the raw upstream
...
instead.` },
      { name: 'page_size', type: 'string', required: false, description: `Maximum nodes per page. Defaults to 50, clamped
to [1, 200].` },
    ],
  },
  {
    ...
    params: [
      { name: 'flow_id', type: 'string', required: true, description: `Project (flow) ID to re-validate.` },
      { name: 'cursor', type: 'string', required: false, description: `Opaque pagination cursor from a prior response.` },
      { name: 'page_size', type: 'string', required: false, description: `Maximum corrections per page (clamped to 200).` },
    ],
  },
  {
    ...
    params: [
      { name: 'project_id', type: 'string', required: true, description: `Project (flow) ID the run belongs to.` },
      { name: 'run_id', type: 'string', required: true, description: `Run ID to fetch.` },
      { name: 'cursor', type: 'string', required: false, description: `Opaque pagination cursor from a previous response's
\`\`pagination.next_cursor\`\`.` },
      { name: 'include_output', type: 'string', required: false, description: `When true, include every node-in-page's
inputs/outputs. Use \`\`node_ids\`\` for finer grain.` },
      { name: 'node_ids', type: 'string', required: false, description: `Optional list of node ids whose \`\`inputs\`\` /
\`\`outputs\`\` should be populated. Up to 50 ids per call.
Ids not in the current page are surfaced in the summary.` },
      { name: 'page_size', type: 'string', required: false, description: `Maximum nodes per page (clamped to 200).` },
      { name: 'severity_filter', type: 'string', required: false, description: `Which nodes to return:
...
\`\`"slow"\`\`, or \`\`"all"\`\`.` },
  ...
  },
  {
    ...
    params: [
      { name: 'cursor', type: 'string', required: false, description: `Opaque pagination cursor from a previous response's
\`\`pagination.next_cursor\`\`.` },
      { name: 'page_size', type: 'string', required: false, description: `Maximum connections per page (clamped to 200).` },
      { name: 'provider_id', type: 'string', required: false, description: `Optional filter, e.g. \`\`"gmail"\`\`, \`\`"slack"\`\`.
Pass null for all.` },
    ],
  },
  {
    ...
    params: [
      ...
      { name: 'page_size', type: 'string', required: false, description: `Maximum results per page (clamped to 200).` },
      { name: 'verbose', type: 'string', required: false, description: `When true, return the full KB metadata for each
row instead of the compact projection.` },
  ...
  },
  {
    ...
    params: [
      { name: 'cursor', type: 'string', required: false, description: `Opaque pagination cursor from a previous response.
Pass \`\`null\`\` to fetch the first page.` },
      { name: 'owned_only', type: 'string', required: false, description: `When true, only return projects the user owns.` },
      { name: 'page_size', type: 'string', required: false, description: `Maximum number of projects per page. Defaults to
50, clamped to [1, 200].` },
    ],
  },
  {
    ...
    params: [
      { name: 'actions', type: 'string', required: false, description: `optional list of action / trigger ids to drill into;
only valid with a single \`\`provider_id\`\` string.` },
      { name: 'provider_id', type: 'string', required: false, description: `\`\`null\`\`, a single id (e.g. \`\`"gmail"\`\`), or a list
(e.g. \`\`["gmail", "slack"]\`\`).` },
    ],
  },
  {
    ...
    params: [
      { name: 'project_id', type: 'string', required: true, description: `Project (flow) ID to inspect.` },
      { name: 'verbose', type: 'string', required: false, description: `When true, return the full trigger metadata
instead of the compact projection.` },
  ...
  },
  {
    ...
    params: [
      { name: 'kb_id', type: 'string', required: true, description: `Knowledge base ID to search.` },
      { name: 'query', type: 'string', required: true, description: `Natural-language search query (must be non-empty).` },
      { name: 'expand_chunk_ids', type: 'string', required: false, description: `Optional list of \`\`chunk_id\`\` values whose
...
result set are surfaced in the summary.` },
      { name: 'query_strategy', type: 'string', required: false, description: `Retrieval mode: \`\`"semantic"\`\` (embedding
similarity, default), \`\`"keyword"\`\` (BM25), or \`\`"hybrid"\`\`
(rerank of both).` },
      { name: 'top_k', type: 'string', required: false, description: `Maximum chunks to return. Defaults to 5, clamped to
[1, 20] (the upstream cap).` },
  ...
  },
  {
    ...
    params: [
      { name: 'org_id', type: 'string', required: true, description: `The Stack AI organization id to activate. Find valid
ids by calling \`\`whoami\`\`.` },
  ...
  },
  {
    ...
    params: [
      { name: 'flow_id', type: 'string', required: true, description: `Project (flow) ID to validate. The validator
always reads the draft, not the published version.` },
      { name: 'cursor', type: 'string', required: false, description: `Opaque pagination cursor from a prior response.` },
      { name: 'page_size', type: 'string', required: false, description: `Maximum findings per page (clamped to 200).` },
      { name: 'severity_filter', type: 'string', required: false, description: `\`\`"all"\`\` (default) returns errors AND
...
\`\`pagination.next_cursor\`\`. Pass null to start.` },
      { name: 'page_size', type: 'string', required: false, description: `Maximum orgs per page. Defaults to 50, clamped
to [1, 200].` },
    ],
  },
]
