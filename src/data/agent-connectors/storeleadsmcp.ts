...
export const tools: Tool[] = [
  {
    ...
    params: [
      { name: 'domains', type: 'array', required: true, description: `Array of domain names to look up (max 100)` },
      { name: 'fields', type: 'string', required: false, description: `Comma-separated fields to return` },
      { name: 'provider', type: 'string', required: false, description: `Platform provider (default: "all")` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'name', type: 'string', required: true, description: `Domain name to detect (e.g. 'example.com')` },
      { name: 'provider', type: 'string', required: false, description: `Platform provider (default: "all")` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'label', type: 'string', required: true, description: `Historical snapshot label (from list_historical_datasets)` },
  ...
  },
  {
    ...
    params: [
      { name: 'name', type: 'string', required: true, description: `Technology name (e.g. 'Klaviyo')` },
      { name: 'fields', type: 'string', required: false, description: `Comma-separated fields to return` },
      { name: 'provider', type: 'string', required: false, description: `Platform provider (default: "all")` },
    ],
  },
  ...
  },
  {
    ...
    params: [
      { name: 'fields', type: 'string', required: false, description: `Comma-separated fields to return` },
      { name: 'filters', type: 'object', required: false, description: `Filter object using f: prefix keys. Common filters:
...
- "f:rcmin"/"f:rcmax": review count range` },
      ...
      { name: 'sort', type: 'string', required: false, description: `Sort fields (e.g. '-installs')` },
    ],
  },
  {
...
- App filters ("f:an") require the app ID in "platform.token" format (e.g. "shopify.klaviyo-email-marketing"). ALWAYS use search_apps first, then use the "id" field from the result as the "f:an" value. Do NOT use the app name or token alone.`,
    params: [
      ...
      { name: 'fields', type: 'string', required: false, description: `Comma-separated fields to return (e.g. 'name,platform,estimated_sales,country_code')` },
      { name: 'filters', type: 'object', required: false, description: `Filter object using f: prefix keys.
...
- "not": exclude matching values` },
      ...
      { name: 'page_size', type: 'number', required: false, description: `Results per page (default: 50, max: 5000)` },
      { name: 'provider', type: 'string', required: false, description: `Platform provider (e.g. "shopify", "woocommerce", "bigcommerce", "all"). Defaults to STORELIST_PROVIDER env var or "all".` },
      { name: 'q', type: 'string', required: false, description: `Keyword search query` },
      { name: 'sort', type: 'string', required: false, description: `Sort fields, comma-separated. Prefix with - for descending. Always include rank as secondary sort.
...
"-cratyyyymmddnco,rank" (newest first), "-mas,rank" (monthly app spend desc)` },
  ...
  },
  {
    ...
    params: [
      { name: 'fields', type: 'string', required: false, description: `Comma-separated fields to return` },
      { name: 'filters', type: 'object', required: false, description: `Filter object. Common filters:
- "f:icmin"/"f:icmax": install count range` },
      { name: 'page', type: 'number', required: false, description: `Page number` },
      { name: 'page_size', type: 'number', required: false, description: `Results per page` },
      { name: 'provider', type: 'string', required: false, description: `Platform provider (default: "all")` },
