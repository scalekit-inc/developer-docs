...
export const tools: Tool[] = [
  {
    ...
    params: [
      { name: 'description', type: 'string', required: false, description: `Human-readable description of what this action represents.` },
      { name: 'name', type: 'string', required: false, description: `Name of the action (must be unique within the project)` },
      { name: 'pinned_at', type: 'string', required: false, description: `ISO 8601 timestamp when the action was pinned, or null if not pinned. Set any value to pin, null to unpin.` },
      { name: 'post_to_slack', type: 'boolean', required: false, description: `Whether to post a notification to Slack when this action is triggered.` },
      { name: 'slack_message_format', type: 'string', required: false, description: `Custom Slack message format. Supports templates with event properties.` },
      { name: 'steps', type: 'array', required: false, description: `Action steps defining trigger conditions. Each step matches events by name, properties, URL, or element attributes. Multiple steps are OR-ed together.` },
  ...
  },
  {
    ...
    params: [
      { name: 'id', type: 'number', required: true, description: `A unique integer value identifying this action.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'limit', type: 'number', required: false, description: `Number of results to return per page.` },
      { name: 'offset', type: 'number', required: false, description: `The initial index from which to return the results.` },
    ],
  },
  {
    ...
    params: [
      { name: 'item_id', type: 'string', required: false, description: `Filter by the ID of the affected resource.` },
      { name: 'page', type: 'number', required: false, description: `Page number for pagination. When provided, uses page-based pagination ordered by most recent first.` },
      { name: 'page_size', type: 'number', required: false, description: `Number of results per page (default: 100, max: 1000). Only used with page-based pagination.` },
      { name: 'scope', type: 'string', required: false, description: `Filter by a single activity scope, e.g. "FeatureFlag", "Insight", "Dashboard", "Experiment".
...
* \`Ticket\` - Ticket` },
      { name: 'scopes', type: 'array', required: false, description: `Filter by multiple activity scopes, comma-separated. Values must be valid ActivityScope enum values. E.g. "FeatureFlag,Insight".` },
      { name: 'user', type: 'string', required: false, description: `Filter by user UUID who performed the action.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'insight', type: 'number', required: true, description: `Insight ID monitored by this alert. Note: Response returns full InsightBasicSerializer object.` },
      { name: 'subscribed_users', type: 'array', required: true, description: `User IDs to subscribe to this alert. Note: Response returns full UserBasicSerializer object.` },
      { name: 'threshold', type: 'object', required: true, description: `Threshold configuration with bounds and type for evaluating the alert.` },
      { name: 'calculation_interval', type: 'string', required: false, description: `How often the alert is checked: hourly, daily, weekly, or monthly.

* \`hourly\` - hourly
* \`daily\` - daily
* \`weekly\` - weekly
* \`monthly\` - monthly` },
      { name: 'condition', type: 'string', required: false, description: `Alert condition type. Determines how the value is evaluated: absolute_value, relative_increase, or relative_decrease.` },
      { name: 'config', type: 'string', required: false, description: `Trends-specific alert configuration. Includes series_index (which series to monitor) and check_ongoing_interval (whether to check the current incomplete interval).` },
      { name: 'detector_config', type: 'string', required: false, description: `No description.` },
      { name: 'enabled', type: 'boolean', required: false, description: `Whether the alert is actively being evaluated.` },
      { name: 'name', type: 'string', required: false, description: `Human-readable name for the alert.` },
      { name: 'schedule_restriction', type: 'string', required: false, description: `Blocked local time windows (HH:MM in the project timezone). Interval is half-open [start, end): start inclusive, end exclusive. Use blocked_windows array of {start, end}. Null disables.` },
      { name: 'skip_weekend', type: 'string', required: false, description: `Skip alert evaluation on weekends (Saturday and Sunday, local to project timezone).` },
      { name: 'snoozed_until', type: 'string', required: false, description: `Snooze the alert until this time. Pass a relative date string (e.g. '2h', '1d') or null to unsnooze.` },
    ],
  },
  {
    ...
    params: [
      { name: 'id', type: 'string', required: true, description: `A UUID string identifying this alert configuration.` },
    ],
  },
  {
    ...
    params: [
      { name: 'id', type: 'string', required: true, description: `A UUID string identifying this alert configuration.` },
      { name: 'checks_date_from', type: 'string', required: false, description: `Relative date string for the start of the check history window (e.g. '-24h', '-7d', '-14d'). Returns checks created after this time. Max retention is 14 days.` },
      { name: 'checks_date_to', type: 'string', required: false, description: `Relative date string for the end of the check history window (e.g. '-1h', '-1d'). Defaults to now if not specified.` },
      { name: 'checks_limit', type: 'number', required: false, description: `Maximum number of check results to return (default 5, max 500). Applied after date filtering.` },
    ],
  },
  {
    ...
    params: [
      { name: 'detector_config', type: 'string', required: true, description: `Detector configuration to simulate.` },
      { name: 'insight', type: 'number', required: true, description: `Insight ID to simulate the detector on.` },
      { name: 'date_from', type: 'string', required: false, description: `Relative date string for how far back to simulate (e.g. '-24h', '-30d', '-4w'). If not provided, uses the detector's minimum required samples.` },
      { name: 'series_index', type: 'number', required: false, description: `Zero-based index of the series to analyze.` },
    ],
  },
  {
...
* \`monthly\` - monthly` },
      ...
      { name: 'detector_config', type: 'string', required: false, description: `No description.` },
      { name: 'enabled', type: 'boolean', required: false, description: `Whether the alert is actively being evaluated.` },
      { name: 'insight', type: 'number', required: false, description: `Insight ID monitored by this alert. Note: Response returns full InsightBasicSerializer object.` },
      { name: 'name', type: 'string', required: false, description: `Human-readable name for the alert.` },
      { name: 'schedule_restriction', type: 'string', required: false, description: `Blocked local time windows (HH:MM in the project timezone). Interval is half-open [start, end): start inclusive, end exclusive. Use blocked_windows array of {start, end}. Null disables.` },
      { name: 'skip_weekend', type: 'string', required: false, description: `Skip alert evaluation on weekends (Saturday and Sunday, local to project timezone).` },
      { name: 'snoozed_until', type: 'string', required: false, description: `Snooze the alert until this time. Pass a relative date string (e.g. '2h', '1d') or null to unsnooze.` },
      { name: 'subscribed_users', type: 'array', required: false, description: `User IDs to subscribe to this alert. Note: Response returns full UserBasicSerializer object.` },
      { name: 'threshold', type: 'object', required: false, description: `Threshold configuration with bounds and type for evaluating the alert.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'content', type: 'string', required: false, description: `Annotation text shown on charts to describe the change, release, or incident.` },
      { name: 'date_marker', type: 'string', required: false, description: `When this annotation happened (ISO 8601 timestamp). Used to position it on charts.` },
      { name: 'scope', type: 'string', required: false, description: `Annotation visibility scope: \`project\`, \`organization\`, \`dashboard\`, or \`dashboard_item\`. \`recording\` is deprecated and rejected.
...
* \`recording\` - recording` },
  ...
  },
  {
    ...
    params: [
      { name: 'id', type: 'number', required: true, description: `A unique integer value identifying this annotation.` },
    ],
  },
  {
...
* \`recording\` - recording` },
  ...
  },
  {
    ...
    params: [
      { name: 'id', type: 'string', required: true, description: `A UUID string identifying this approval policy.` },
    ],
  },
  {
    ...
    params: [
      { name: 'limit', type: 'number', required: false, description: `Number of results to return per page.` },
      { name: 'offset', type: 'number', required: false, description: `The initial index from which to return the results.` },
      { name: 'template_id', type: 'string', required: false, description: `Filter to a specific template by its template_id. Deprecated templates are excluded from list results; use the retrieve endpoint to look up a template by ID regardless of status.` },
      { name: 'type', type: 'string', required: false, description: `Filter by template type (e.g. destination, email, sms_provider, broadcast). Defaults to destination if neither type nor types is provided.` },
      { name: 'types', type: 'string', required: false, description: `Comma-separated list of template types to include (e.g. destination,email,sms_provider).` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      ...
      { name: 'enabled', type: 'boolean', required: false, description: `Whether the function is active and processing events.` },
      { name: 'execution_order', type: 'string', required: false, description: `Execution priority for transformation functions (lower runs first). Only applies to type=transformation. If omitted, the function is appended at the end.` },
      { name: 'filters', type: 'object', required: false, description: `Event filters that control which events trigger this function.` },
      { name: 'hog', type: 'string', required: false, description: `Source code for the function. For most types this is Hog code; for site_destination and site_app types this is TypeScript. Required if no template_id is provided.` },
      { name: 'icon_url', type: 'string', required: false, description: `URL for the function's icon displayed in the UI.` },
      { name: 'inputs', type: 'object', required: false, description: `Values for each input defined in inputs_schema.` },
      { name: 'inputs_schema', type: 'array', required: false, description: `Schema defining the configurable input parameters for this function.` },
      { name: 'mappings', type: 'string', required: false, description: `Event-to-destination field mappings. Only for destination and site_destination types.` },
      { name: 'masking', type: 'string', required: false, description: `PII masking configuration with TTL, threshold, and hash expression.` },
      { name: 'name', type: 'string', required: false, description: `Display name for the function.` },
      { name: 'template_id', type: 'string', required: false, description: `ID of a HogFunctionTemplate to derive defaults from (code, inputs_schema, icon, name, description). Use the cdp-function-templates-list tool to find available templates.` },
      { name: 'type', type: 'string', required: false, description: `Function type. One of: destination, site_destination, internal_destination, source_webhook, warehouse_source_webhook, site_app, transformation.` },
    ],
  },
  {
    ...
    params: [
      { name: 'id', type: 'string', required: true, description: `A UUID string identifying this hog function.` },
    ],
  },
  {
    ...
    params: [
      { name: 'configuration', type: 'object', required: true, description: `Full function configuration to test.` },
      { name: 'id', type: 'string', required: true, description: `A UUID string identifying this hog function.` },
      { name: 'clickhouse_event', type: 'object', required: false, description: `Mock ClickHouse event data to test the function with.` },
      { name: 'globals', type: 'object', required: false, description: `Mock global variables available during test invocation.` },
      { name: 'invocation_id', type: 'string', required: false, description: `Optional invocation ID for correlation.` },
      { name: 'mock_async_functions', type: 'boolean', required: false, description: `When true (default), async functions like fetch() are simulated.` },
    ],
  },
  {
    ...
    params: [
      ...
      { name: 'offset', type: 'number', required: false, description: `The initial index from which to return the results.` },
      { name: 'search', type: 'string', required: false, description: `A search term.` },
      { name: 'type', type: 'array', required: false, description: `Multiple values may be separated by commas.` },
  ...
  },
  {
    ...
    params: [
      { name: 'id', type: 'string', required: true, description: `A UUID string identifying this hog function.` },
      { name: 'description', type: 'string', required: false, description: `Human-readable description of what this function does.` },
      { name: 'enabled', type: 'boolean', required: false, description: `Set to true to activate or false to deactivate the function.` },
      { name: 'execution_order', type: 'string', required: false, description: `Execution priority for transformations. Lower values run first.` },
      { name: 'filters', type: 'object', required: false, description: `Event filters that control which events trigger this function.` },
      { name: 'hog', type: 'string', required: false, description: `Source code. Hog language for most types; TypeScript for site_destination and site_app.` },
      { name: 'icon_url', type: 'string', required: false, description: `URL for the function's icon displayed in the UI.` },
      { name: 'inputs', type: 'object', required: false, description: `Values for each input defined in inputs_schema.` },
      { name: 'inputs_schema', type: 'array', required: false, description: `Schema defining the configurable input parameters for this function.` },
      { name: 'mappings', type: 'string', required: false, description: `Event-to-destination field mappings. Only for destination and site_destination types.` },
      { name: 'masking', type: 'string', required: false, description: `PII masking configuration with TTL, threshold, and hash expression.` },
      { name: 'name', type: 'string', required: false, description: `Display name for the function.` },
      { name: 'template_id', type: 'string', required: false, description: `ID of the template to create this function from.` },
      { name: 'type', type: 'string', required: false, description: `Function type: destination, site_destination, internal_destination, source_webhook, warehouse_source_webhook, site_app, or transformation.
...
* \`transformation\` - Transformation` },
  ...
  },
  {
    ...
    params: [
      { name: 'orders', type: 'object', required: false, description: `Map of hog function UUIDs to their new execution_order values.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'id', type: 'string', required: true, description: `A UUID string identifying this change request.` },
    ],
  },
  {
    ...
    params: [
      ...
      { name: 'state', type: 'array', required: false, description: `Multiple values may be separated by commas.` },
    ],
  },
  {
    ...
    params: [
      { name: 'id', type: 'number', required: true, description: `A unique integer value identifying this cohort.` },
      { name: 'person_ids', type: 'array', required: false, description: `List of person UUIDs to add to the cohort` },
    ],
  },
  {
    ...
    params: [
      { name: 'cohort_type', type: 'string', required: false, description: `Type of cohort based on filter complexity
...
* \`analytical\` - analytical` },
  ...
  },
  {
    ...
    params: [
      { name: 'id', type: 'number', required: true, description: `A unique integer value identifying this cohort.` },
      { name: 'person_id', type: 'string', required: false, description: `Person UUID to remove from the cohort` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'id', type: 'string', required: true, description: `A UUID string identifying this comment.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'cursor', type: 'string', required: false, description: `The pagination cursor value.` },
      { name: 'item_id', type: 'string', required: false, description: `Filter by the ID of the resource being commented on.` },
      { name: 'scope', type: 'string', required: false, description: `Filter by resource type (e.g. Dashboard, FeatureFlag, Insight, Replay).` },
      { name: 'search', type: 'string', required: false, description: `Full-text search within comment content.` },
      { name: 'source_comment', type: 'string', required: false, description: `Filter replies to a specific parent comment.` },
    ],
  },
  {
    ...
    params: [
      { name: 'assignee', type: 'string', required: false, description: `Filter by assignee. Use \`unassigned\` for tickets with no assignee, \`user:<user_id>\` for a specific user, or \`role:<role_uuid>\` for a role.` },
      { name: 'channel_detail', type: 'string', required: false, description: `Filter by the channel sub-type (e.g. \`widget_embedded\`, \`slack_bot_mention\`).` },
      { name: 'channel_source', type: 'string', required: false, description: `Filter by the channel the ticket originated from.` },
      { name: 'date_from', type: 'string', required: false, description: `Only include tickets updated on or after this date. Accepts absolute dates (\`2026-01-01\`) or relative ones (\`-7d\`, \`-1mStart\`). Pass \`all\` to disable the filter.` },
      { name: 'date_to', type: 'string', required: false, description: `Only include tickets updated on or before this date. Same format as \`date_from\`.` },
      { name: 'distinct_ids', type: 'string', required: false, description: `Comma-separated list of person \`distinct_id\`s to filter by (max 100).` },
      { name: 'limit', type: 'number', required: false, description: `Number of results to return per page.` },
      { name: 'offset', type: 'number', required: false, description: `The initial index from which to return the results.` },
      { name: 'order_by', type: 'string', required: false, description: `Sort order. Prefix with \`-\` for descending. Defaults to \`-updated_at\`.` },
      { name: 'priority', type: 'string', required: false, description: `Filter by priority. Accepts a single value or a comma-separated list (e.g. \`medium,high\`). Valid values: \`low\`, \`medium\`, \`high\`.` },
      { name: 'search', type: 'string', required: false, description: `Free-text search. A numeric value matches a ticket number exactly; otherwise matches against the customer's name or email (case-insensitive, partial match).` },
      { name: 'sla', type: 'string', required: false, description: `Filter by SLA state. \`breached\` = past \`sla_due_at\`, \`at-risk\` = due within the next hour, \`on-track\` = more than an hour remaining.` },
      { name: 'status', type: 'string', required: false, description: `Filter by status. Accepts a single value or a comma-separated list (e.g. \`new,open,pending\`). Valid values: \`new\`, \`open\`, \`pending\`, \`on_hold\`, \`resolved\`.` },
      { name: 'tags', type: 'string', required: false, description: `JSON-encoded array of tag names to filter by, e.g. \`["billing","urgent"]\`.` },
    ],
  },
  {
    ...
    params: [
      { name: 'id', type: 'string', required: true, description: `A UUID string identifying this ticket.` },
    ],
  },
  {
    ...
    params: [
      { name: 'id', type: 'string', required: true, description: `A UUID string identifying this ticket.` },
      { name: 'priority', type: 'string', required: false, description: `Ticket priority: low, medium, or high. Null if unset.

* \`low\` - Low
* \`medium\` - Medium
* \`high\` - High` },
      { name: 'sla_due_at', type: 'string', required: false, description: `SLA deadline set via workflows. Null means no SLA.` },
      { name: 'snoozed_until', type: 'string', required: false, description: `No description.` },
      { name: 'status', type: 'string', required: false, description: `Ticket status: new, open, pending, on_hold, or resolved
...
* \`resolved\` - Resolved` },
  ...
  },
  {
    ...
    params: [
      { name: 'active', type: 'boolean', required: false, description: `Whether the feature flag is active.` },
      { name: 'evaluation_contexts', type: 'array', required: false, description: `Evaluation contexts that control where this flag evaluates at runtime.` },
      { name: 'filters', type: 'object', required: false, description: `Feature flag targeting configuration.` },
      { name: 'key', type: 'string', required: false, description: `Feature flag key.` },
      { name: 'name', type: 'string', required: false, description: `Feature flag description (stored in the \`name\` field for backwards compatibility).` },
      { name: 'tags', type: 'array', required: false, description: `Organizational tags for this feature flag.` },
    ],
  },
  {
    ...
    params: [
      { name: 'breakdown_colors', type: 'string', required: false, description: `Custom color mapping for breakdown values.` },
      { name: 'data_color_theme_id', type: 'string', required: false, description: `ID of the color theme used for chart visualizations.` },
      { name: 'delete_insights', type: 'boolean', required: false, description: `When deleting, also delete insights that are only on this dashboard.` },
      { name: 'description', type: 'string', required: false, description: `No description.` },
      { name: 'name', type: 'string', required: false, description: `No description.` },
      { name: 'pinned', type: 'boolean', required: false, description: `No description.` },
      { name: 'quick_filter_ids', type: 'string', required: false, description: `List of quick filter IDs associated with this dashboard` },
      { name: 'restriction_level', type: 'string', required: false, description: `* \`21\` - Everyone in the project can edit
* \`37\` - Only those invited to this dashboard can edit` },
      { name: 'tags', type: 'array', required: false, description: `No description.` },
      { name: 'use_dashboard', type: 'string', required: false, description: `ID of an existing dashboard to duplicate.` },
      { name: 'use_template', type: 'string', required: false, description: `Template key to create the dashboard from a predefined template.` },
    ],
  },
  {
    ...
    params: [
      { name: 'id', type: 'number', required: true, description: `A unique integer value identifying this dashboard.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'id', type: 'number', required: true, description: `A unique integer value identifying this dashboard.` },
      { name: 'output_format', type: 'string', required: false, description: `'optimized' (default) returns LLM-friendly formatted text per insight. 'json' returns the raw query result objects.` },
      { name: 'refresh', type: 'string', required: false, description: `Cache behavior. 'force_cache' (default) serves from cache even if stale. 'blocking' uses cache if fresh, otherwise recalculates. 'force_blocking' always recalculates.` },
    ],
  },
  {
    ...
    params: [
      { name: 'id', type: 'number', required: true, description: `A unique integer value identifying this dashboard.` },
      { name: 'tile_order', type: 'array', required: true, description: `Array of tile IDs in the desired display order (top to bottom, left to right).` },
    ],
  },
  {
...
* \`37\` - Only those invited to this dashboard can edit` },
  ...
  },
  {
    ...
    params: [
      { name: 'message', type: 'string', required: false, description: `Optional message to include in the debug data` },
    ],
  },
  {
    ...
    params: [
      { name: 'id', type: 'number', required: true, description: `A unique integer value identifying this feature flag.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'name', type: 'string', required: true, description: `The name of the early access feature.` },
      { name: 'stage', type: 'string', required: true, description: `Lifecycle stage. Valid values: draft, concept, alpha, beta, general-availability, archived. Moving to an active stage (alpha/beta/general-availability) enables the feature flag for opted-in users.
...
* \`archived\` - archived` },
      { name: 'description', type: 'string', required: false, description: `A longer description of what this early access feature does, shown to users in the opt-in UI.` },
      { name: 'documentation_url', type: 'string', required: false, description: `URL to external documentation for this feature. Shown to users in the opt-in UI.` },
      { name: 'feature_flag_id', type: 'number', required: false, description: `Optional ID of an existing feature flag to link. If omitted, a new flag is auto-created from the feature name. The flag must not already be linked to another feature, must not be group-based, and must not be multivariate.` },
      { name: 'payload', type: 'string', required: false, description: `Arbitrary JSON metadata associated with this feature.` },
    ],
  },
  {
    ...
    params: [
      { name: 'id', type: 'string', required: true, description: `A UUID string identifying this early access feature.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      ...
      { name: 'description', type: 'string', required: false, description: `A longer description of what this early access feature does, shown to users in the opt-in UI.` },
      { name: 'documentation_url', type: 'string', required: false, description: `URL to external documentation for this feature. Shown to users in the opt-in UI.` },
      { name: 'name', type: 'string', required: false, description: `The name of the early access feature.` },
      { name: 'stage', type: 'string', required: false, description: `Lifecycle stage. Valid values: draft, concept, alpha, beta, general-availability, archived. Moving to an active stage (alpha/beta/general-availability) enables the feature flag for opted-in users.
...
* \`archived\` - archived` },
  ...
  },
  {
    ...
    params: [
      { name: 'cache_age_seconds', type: 'string', required: false, description: `Cache TTL in seconds (60–86400).` },
      { name: 'description', type: 'string', required: false, description: `Human-readable description of what this endpoint returns.` },
      { name: 'is_materialized', type: 'string', required: false, description: `Whether query results are materialized to S3.` },
      { name: 'name', type: 'string', required: false, description: `Unique URL-safe name. Must start with a letter, only letters/numbers/hyphens/underscores, max 128 chars.` },
      { name: 'query', type: 'string', required: false, description: `HogQL or insight query this endpoint executes. Changing this auto-creates a new version.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'name', type: 'string', required: true, description: `No description.` },
      { name: 'version', type: 'number', required: false, description: `Specific endpoint version to generate the spec for. Defaults to latest.` },
    ],
  },
  {
    ...
    params: [
      { name: 'name', type: 'string', required: true, description: `No description.` },
      { name: 'limit', type: 'string', required: false, description: `Maximum number of results to return. If not provided, returns all results.` },
      { name: 'offset', type: 'string', required: false, description: `Number of results to skip. Must be used together with limit. Only supported for HogQL endpoints.` },
      { name: 'refresh', type: 'string', required: false, description: `No description.` },
      { name: 'variables', type: 'string', required: false, description: `Key-value pairs to parameterize the query. For HogQL endpoints, keys match variable code_name (e.g. {"event_name": "$pageview"}). For insight endpoints with breakdowns, use the breakdown property name as key.` },
    ],
  },
  {
    ...
    params: [
      ...
      { name: 'is_materialized', type: 'string', required: false, description: `Whether query results are materialized to S3.` },
      { name: 'query', type: 'string', required: false, description: `HogQL or insight query this endpoint executes. Changing this auto-creates a new version.` },
      { name: 'version', type: 'string', required: false, description: `Target a specific version for updates (defaults to current version).` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'query', type: 'string', required: true, description: `Search query to find entities by name or description` },
      { name: 'entities', type: 'array', required: false, description: `Entity types to search. If not specified, searches all types. Available: insight, dashboard, experiment, feature_flag, notebook, action, cohort, event_definition, survey` },
    ],
  },
  {
    ...
    params: [
      { name: 'assignee', type: 'object', required: true, description: `User or role to assign matching issues to.` },
      { name: 'filters', type: 'object', required: true, description: `Property-group filters that define when this rule matches incoming error events.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'filters', type: 'object', required: true, description: `Property-group filters that define which exceptions should be grouped into the same issue.` },
      { name: 'assignee', type: 'string', required: false, description: `Optional user or role to assign to issues created by this grouping rule.` },
      { name: 'description', type: 'string', required: false, description: `Optional human-readable description of what this grouping rule is for.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'id', type: 'string', required: true, description: `A UUID string identifying this error tracking issue.` },
      { name: 'ids', type: 'array', required: true, description: `IDs of the issues to merge into the current issue.` },
    ],
  },
  {
    ...
    params: [
      ...
      { name: 'name', type: 'string', required: false, description: `No description.` },
      { name: 'status', type: 'string', required: false, description: `* \`archived\` - Archived
* \`active\` - Active
* \`resolved\` - Resolved
...
* \`suppressed\` - Suppressed` },
  ...
  },
  {
    ...
    params: [
      { name: 'id', type: 'string', required: true, description: `A UUID string identifying this error tracking issue.` },
      { name: 'fingerprints', type: 'array', required: false, description: `Fingerprints to split into new issues. Each fingerprint becomes its own new issue.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'evaluation_config', type: 'object', required: true, description: `Configuration for the evaluation. Provide "prompt" for llm_judge or "source" for hog type.` },
      { name: 'evaluation_type', type: 'string', required: true, description: `Type of evaluation. "llm_judge" uses an LLM to score generations against a prompt. "hog" runs deterministic Hog code.` },
      { name: 'name', type: 'string', required: true, description: `Name of the evaluation.` },
      { name: 'description', type: 'string', required: false, description: `Description of what this evaluation checks.` },
      { name: 'enabled', type: 'boolean', required: false, description: `Whether the evaluation runs automatically on new generations. Defaults to false.` },
      { name: 'model_configuration', type: 'object', required: false, description: `LLM model configuration (required for llm_judge evaluations).` },
      { name: 'output_config', type: 'object', required: false, description: `No description.` },
      { name: 'output_type', type: 'string', required: false, description: `Output type. Currently only "boolean" is supported.` },
    ],
  },
  {
    ...
    params: [
      { name: 'evaluationId', type: 'string', required: true, description: `The UUID of the evaluation to delete.` },
    ],
  },
  {
    ...
    params: [
      { name: 'evaluationId', type: 'string', required: true, description: `The UUID of the evaluation to retrieve.` },
    ],
  },
  {
    ...
    params: [
      { name: 'evaluationId', type: 'string', required: true, description: `The UUID of the evaluation to run.` },
      { name: 'target_event_id', type: 'string', required: true, description: `The UUID of the $ai_generation event to evaluate.` },
      { name: 'timestamp', type: 'string', required: true, description: `ISO 8601 timestamp of the target event (needed for efficient lookup).` },
      { name: 'distinct_id', type: 'string', required: false, description: `Distinct ID of the event (optional, improves lookup performance).` },
      { name: 'event', type: 'string', required: false, description: `Event name. Defaults to "$ai_generation".` },
    ],
  },
  {
    ...
    params: [
      { name: 'source', type: 'string', required: true, description: `Hog source code to test. Must return a boolean (true = pass, false = fail).` },
      { name: 'allows_na', type: 'boolean', required: false, description: `Whether the evaluation can return N/A for non-applicable generations.` },
      { name: 'conditions', type: 'array', required: false, description: `Optional trigger conditions to filter which events are sampled.` },
      { name: 'sample_count', type: 'integer', required: false, description: `Number of recent $ai_generation events to test against (1-10, default 5).` },
    ],
  },
  {
    ...
    params: [
      { name: 'evaluationId', type: 'string', required: true, description: `The UUID of the evaluation to update.` },
      { name: 'description', type: 'string', required: false, description: `Updated description.` },
      { name: 'enabled', type: 'boolean', required: false, description: `Enable or disable the evaluation.` },
      { name: 'evaluation_config', type: 'object', required: false, description: `Updated evaluation configuration.` },
      { name: 'name', type: 'string', required: false, description: `Updated name.` },
      { name: 'output_config', type: 'object', required: false, description: `Updated output configuration.` },
    ],
  },
  {
    ...
    params: [
      ...
      { name: 'search', type: 'string', required: false, description: `Search evaluations by name or description.` },
    ],
  },
  {
    ...
    params: [
      { name: 'data', type: 'object', required: true, description: `The event definition data to update` },
      { name: 'eventName', type: 'string', required: true, description: `The name of the event to update (e.g. "$pageview", "user_signed_up")` },
    ],
  },
  {
    ...
    params: [
      ...
      { name: 'q', type: 'string', required: false, description: `Search query to filter event names. Only use if there are lots of events.` },
    ],
  },
  {
    ...
    params: [
      { name: 'id', type: 'number', required: true, description: `A unique integer value identifying this experiment.` },
    ],
  },
  {
    ...
    params: [
      { name: 'feature_flag_key', type: 'string', required: true, description: `Unique key for the experiment's feature flag. Letters, numbers, hyphens, and underscores only. Search existing flags with the feature-flags-get-all tool first — reuse an existing flag when possible.` },
      { name: 'name', type: 'string', required: true, description: `Name of the experiment.` },
      { name: 'allow_unknown_events', type: 'boolean', required: false, description: `No description.` },
      { name: 'description', type: 'string', required: false, description: `Description of the experiment hypothesis and expected outcomes.` },
      { name: 'exposure_criteria', type: 'string', required: false, description: `Exposure configuration including filter test accounts and custom exposure events.` },
      { name: 'holdout_id', type: 'string', required: false, description: `ID of a holdout group to exclude from the experiment.` },
      { name: 'metrics', type: 'string', required: false, description: `Primary experiment metrics. Each metric must have kind='ExperimentMetric' and a metric_type: 'mean' (set source to an EventsNode with an event name), 'funnel' (set series to an array of EventsNode steps), 'ratio' (set numerator and denominator EventsNode entries), or 'retention' (set start_event and completion_event). Use the event-definitions-list tool to find available events in the project.` },
      { name: 'metrics_secondary', type: 'string', required: false, description: `Secondary metrics for additional measurements. Same format as primary metrics.` },
      { name: 'parameters', type: 'string', required: false, description: `Variant definitions and statistical configuration. Set feature_flag_variants to customize the split (default: 50/50 control/test). Each variant needs a key and rollout_percentage; percentages must sum to 100. Set minimum_detectable_effect (percentage, suggest 20-30) to control statistical power.` },
      { name: 'type', type: 'string', required: false, description: `Experiment type: web for frontend UI changes, product for backend/API changes.
...
* \`product\` - product` },
  ...
  },
  {
    ...
    params: [
      { name: 'id', type: 'number', required: true, description: `A unique integer value identifying this experiment.` },
      { name: 'conclusion', type: 'string', required: false, description: `The conclusion of the experiment.
...
* \`invalid\` - invalid` },
      { name: 'conclusion_comment', type: 'string', required: false, description: `Optional comment about the experiment conclusion.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'experimentId', type: 'number', required: true, description: `The ID of the experiment to get comprehensive results for` },
      { name: 'refresh', type: 'boolean', required: true, description: `Force refresh of results instead of using cached values` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'id', type: 'number', required: true, description: `A unique integer value identifying this experiment.` },
      { name: 'variant_key', type: 'string', required: true, description: `The key of the variant to ship to 100% of users.` },
      { name: 'conclusion', type: 'string', required: false, description: `The conclusion of the experiment.
...
* \`invalid\` - invalid` },
  ...
  },
  {
    ...
    params: [
      { name: 'id', type: 'number', required: true, description: `A unique integer value identifying this experiment.` },
      { name: 'archived', type: 'boolean', required: false, description: `Whether the experiment is archived.` },
      { name: 'conclusion', type: 'string', required: false, description: `Experiment conclusion: won, lost, inconclusive, stopped_early, or invalid.
...
* \`stopped_early\` - stopped_early
* \`invalid\` - invalid` },
      { name: 'conclusion_comment', type: 'string', required: false, description: `Comment about the experiment conclusion.` },
      { name: 'description', type: 'string', required: false, description: `Description of the experiment hypothesis and expected outcomes.` },
      { name: 'exposure_criteria', type: 'string', required: false, description: `Exposure configuration including filter test accounts and custom exposure events.` },
      { name: 'metrics', type: 'string', required: false, description: `Primary experiment metrics. Each metric must have kind='ExperimentMetric' and a metric_type: 'mean' (set source to an EventsNode with an event name), 'funnel' (set series to an array of EventsNode steps), 'ratio' (set numerator and denominator EventsNode entries), or 'retention' (set start_event and completion_event). Use the event-definitions-list tool to find available events in the project.` },
      { name: 'metrics_secondary', type: 'string', required: false, description: `Secondary metrics for additional measurements. Same format as primary metrics.` },
  ...
  },
  {
    ...
    params: [
      ...
      { name: 'created_by_id', type: 'string', required: false, description: `The User ID which initially created the feature flag.` },
      { name: 'evaluation_runtime', type: 'string', required: false, description: `Filter feature flags by their evaluation runtime.` },
      { name: 'excluded_properties', type: 'string', required: false, description: `JSON-encoded list of feature flag keys to exclude from the results.` },
      { name: 'has_evaluation_contexts', type: 'string', required: false, description: `Filter feature flags by presence of evaluation contexts. 'true' returns only flags with at least one evaluation context, 'false' returns only flags without.` },
      { name: 'limit', type: 'number', required: false, description: `Number of results to return per page.` },
      { name: 'offset', type: 'number', required: false, description: `The initial index from which to return the results.` },
      { name: 'search', type: 'string', required: false, description: `Search by feature flag key or name (case-insensitive). Use this to find the flag ID for get/update/delete tools.` },
      { name: 'tags', type: 'string', required: false, description: `JSON-encoded list of tag names to filter feature flags by.` },
  ...
  },
  {
    ...
    params: [
      { name: 'feature_flag_key', type: 'string', required: true, description: `Key of the feature flag to copy` },
      { name: 'from_project', type: 'number', required: true, description: `Source project ID to copy the flag from` },
      { name: 'target_project_ids', type: 'array', required: true, description: `List of target project IDs to copy the flag to` },
      { name: 'copy_schedule', type: 'boolean', required: false, description: `Whether to also copy scheduled changes for this flag` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      ...
      { name: 'groups', type: 'string', required: false, description: `Groups for feature flag evaluation (JSON object string)` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'condition', type: 'object', required: true, description: `The release condition to evaluate` },
      { name: 'group_type_index', type: 'string', required: false, description: `Group type index for group-based flags (null for person-based flags)` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'query', type: 'string', required: true, description: `No description.` },
      { name: 'dashboards', type: 'array', required: false, description: `Dashboard IDs this insight should belong to. This is a full replacement — always include all existing dashboard IDs when adding a new one.` },
      { name: 'description', type: 'string', required: false, description: `No description.` },
  ...
  },
  {
    ...
    params: [
      { name: 'id', type: 'string', required: true, description: `Numeric primary key or 8-character \`short_id\` (for example \`AaVQ8Ijw\`) identifying the insight.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'insightId', type: 'string', required: true, description: `The insight ID or short_id to run.` },
      { name: 'output_format', type: 'string', required: false, description: `Output format. "optimized" returns a human-readable summary from server-side formatters (recommended for analysis). "json" returns the raw query results as JSON.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'id', type: 'number', required: true, description: `A unique integer value identifying this integration.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'id', type: 'string', required: true, description: `A UUID string identifying this clustering job.` },
    ],
  },
  {
    ...
    params: [
      { name: 'evaluation_id', type: 'string', required: true, description: `UUID of the evaluation config to summarize` },
      { name: 'filter', type: 'string', required: false, description: `Filter type to apply ('all', 'pass', 'fail', or 'na')

* \`all\` - all
* \`pass\` - pass
* \`fail\` - fail
* \`na\` - na` },
      { name: 'force_refresh', type: 'boolean', required: false, description: `If true, bypass cache and generate a fresh summary` },
      { name: 'generation_ids', type: 'array', required: false, description: `Optional: specific generation IDs to include in summary (max 250)` },
    ],
  },
  {
    ...
    params: [
      ...
      { name: 'analysis_level', type: 'string', required: false, description: `* \`trace\` - trace
* \`generation\` - generation` },
  ...
  },
  {
    ...
    params: [
      { name: 'data', type: 'string', required: false, description: `Data to summarize. For traces: {trace, hierarchy}. For events: {event}. Not required when using trace_id or generation_id.` },
      { name: 'date_from', type: 'string', required: false, description: `Start of date range for ID-based lookup (e.g. '-7d' or '2026-01-01'). Defaults to -30d.` },
      { name: 'date_to', type: 'string', required: false, description: `End of date range for ID-based lookup. Defaults to now.` },
      { name: 'force_refresh', type: 'boolean', required: false, description: `Force regenerate summary, bypassing cache` },
      { name: 'generation_id', type: 'string', required: false, description: `Generation event UUID to summarize. The backend fetches the event data automatically. Requires date_from for efficient lookup.` },
      { name: 'mode', type: 'string', required: false, description: `Summary detail level: 'minimal' for 3-5 points, 'detailed' for 5-10 points

* \`minimal\` - minimal
* \`detailed\` - detailed` },
      { name: 'model', type: 'string', required: false, description: `LLM model to use (defaults based on provider)` },
      { name: 'summarize_type', type: 'string', required: false, description: `Type of entity to summarize. Inferred automatically when using trace_id or generation_id.

* \`trace\` - trace
* \`event\` - event` },
      { name: 'trace_id', type: 'string', required: false, description: `Trace ID to summarize. The backend fetches the trace data automatically. Requires date_from for efficient lookup.` },
    ],
  },
  {
    ...
    params: [
      { name: 'key', type: 'string', required: true, description: `The attribute key to get values for` },
      { name: 'attribute_type', type: 'string', required: false, description: `Type of attribute: "log" or "resource". Defaults to "log".

* \`log\` - log
* \`resource\` - resource` },
      { name: 'dateRange', type: 'object', required: false, description: `Date range to search within. Defaults to last hour.` },
      { name: 'filterGroup', type: 'array', required: false, description: `Property filters to narrow which logs are scanned for values.` },
      { name: 'serviceNames', type: 'array', required: false, description: `Filter values to those appearing in logs from these services.` },
      { name: 'value', type: 'string', required: false, description: `Search filter for attribute values` },
    ],
  },
  {
    ...
    params: [
      { name: 'attribute_type', type: 'string', required: false, description: `Type of attributes: "log" for log attributes, "resource" for resource attributes. Defaults to "log".

* \`log\` - log
* \`resource\` - resource` },
      { name: 'dateRange', type: 'object', required: false, description: `Date range to search within. Defaults to last hour.` },
      { name: 'filterGroup', type: 'array', required: false, description: `Property filters to narrow which logs are scanned for attributes.` },
      { name: 'limit', type: 'number', required: false, description: `Max results (default: 100)` },
      { name: 'offset', type: 'number', required: false, description: `Pagination offset (default: 0)` },
      { name: 'search', type: 'string', required: false, description: `Search filter for attribute names` },
      { name: 'serviceNames', type: 'array', required: false, description: `Filter attributes to those appearing in logs from these services.` },
    ],
  },
  {
...
\`\`\``,
    params: [
      { name: 'query', type: 'object', required: true, description: `The sparkline query to execute.` },
    ],
  },
  {
    ...
    params: [
      { name: 'content', type: 'string', required: false, description: `Notebook content as a ProseMirror JSON document structure.` },
      { name: 'deleted', type: 'boolean', required: false, description: `Whether the notebook has been soft-deleted.` },
      { name: 'text_content', type: 'string', required: false, description: `Plain text representation of the notebook content for search.` },
  ...
  },
  {
    ...
    params: [
      { name: 'contains', type: 'string', required: false, description: `Filter for notebooks that match a provided filter.
                Each match pair is separated by a colon,
                multiple match pairs can be sent separated by a space or a comma` },
      { name: 'created_by', type: 'string', required: false, description: `The UUID of the Notebook's creator` },
      { name: 'date_from', type: 'string', required: false, description: `Filter for notebooks created after this date & time` },
      { name: 'date_to', type: 'string', required: false, description: `Filter for notebooks created before this date & time` },
      { name: 'limit', type: 'number', required: false, description: `Number of results to return per page.` },
      { name: 'offset', type: 'number', required: false, description: `The initial index from which to return the results.` },
      { name: 'user', type: 'string', required: false, description: `If any value is provided for this parameter, return notebooks created by the logged in user.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'limit', type: 'number', required: false, description: `Number of results to return per page.` },
      { name: 'offset', type: 'number', required: false, description: `The initial index from which to return the results.` },
      { name: 'order', type: 'string', required: false, description: `Sort order. Defaults to \`-joined_at\`.` },
    ],
  },
  {
    ...
    params: [
      { name: 'id', type: 'string', required: false, description: `Organization ID. If omitted, uses the active organization.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'delete_events', type: 'boolean', required: false, description: `If true, queue deletion of all events associated with these persons.` },
      { name: 'delete_recordings', type: 'boolean', required: false, description: `If true, queue deletion of all recordings associated with these persons.` },
      { name: 'distinct_ids', type: 'array', required: false, description: `A list of distinct IDs whose associated persons will be deleted (max 1000).` },
      { name: 'ids', type: 'array', required: false, description: `A list of PostHog person UUIDs to delete (max 1000).` },
      { name: 'keep_person', type: 'boolean', required: false, description: `If true, keep the person records but delete their events and recordings.` },
    ],
  },
  {
    ...
    params: [
      { name: 'person_id', type: 'string', required: true, description: `The person ID or UUID to get cohorts for.` },
    ],
  },
  {
    ...
    params: [
      { name: 'distinct_id', type: 'string', required: false, description: `Filter list by distinct id.` },
      { name: 'email', type: 'string', required: false, description: `Filter persons by email (exact match)` },
      { name: 'limit', type: 'number', required: false, description: `Number of results to return per page.` },
      { name: 'offset', type: 'number', required: false, description: `The initial index from which to return the results.` },
      { name: 'search', type: 'string', required: false, description: `Search persons, either by email (full text search) or distinct_id (exact match).` },
    ],
  },
  {
    ...
    params: [
      { name: 'id', type: 'string', required: true, description: `A unique value identifying this person. Accepts both numeric ID and UUID.` },
      { name: 'unset', type: 'string', required: true, description: `The property key to remove from this person.` },
    ],
  },
  {
    ...
    params: [
      ...
      { name: 'value', type: 'string', required: true, description: `The property value. Can be a string, number, boolean, or object.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'key', type: 'string', required: true, description: `The person property key to get values for (e.g., 'email', 'plan', 'role').` },
      { name: 'value', type: 'string', required: false, description: `Optional search string to filter values (case-insensitive substring match).` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'name', type: 'string', required: true, description: `Unique prompt name using letters, numbers, hyphens, and underscores only.` },
      { name: 'prompt', type: 'string', required: true, description: `Prompt payload as JSON or string data.` },
    ],
  },
  {
    ...
    params: [
      { name: 'new_name', type: 'string', required: true, description: `Name for the duplicated prompt. Must be unique and use only letters, numbers, hyphens, and underscores.` },
...
or \`content=preview\` for a short \`prompt_preview\` snippet instead of the full prompt.`,
    params: [
      { name: 'prompt_name', type: 'string', required: true, description: `No description.` },
      { name: 'content', type: 'string', required: false, description: `Controls how much prompt content is included in the response. 'full' includes the full prompt, 'preview' includes a short prompt_preview, and 'none' omits prompt content entirely. The outline field is always included.

* \`full\` - full
* \`preview\` - preview
* \`none\` - none` },
      { name: 'version', type: 'number', required: false, description: `Specific prompt version to fetch. If omitted, the latest version is returned.` },
    ],
  },
  {
    ...
    params: [
      { name: 'content', type: 'string', required: false, description: `Controls how much prompt content is included in list results. 'full' includes the full prompt, 'preview' includes a short prompt_preview, and 'none' omits prompt content entirely.` },
      { name: 'search', type: 'string', required: false, description: `Optional substring filter applied to prompt names and prompt content.` },
    ],
  },
  {
...
may be provided.`,
    params: [
      { name: 'prompt_name', type: 'string', required: true, description: `No description.` },
      { name: 'base_version', type: 'number', required: false, description: `Latest version you are editing from. Used for optimistic concurrency checks.` },
      { name: 'edits', type: 'array', required: false, description: `List of find/replace operations to apply to the current prompt version. Each edit's 'old' text must match exactly once. Edits are applied sequentially. Mutually exclusive with prompt.` },
      { name: 'prompt', type: 'string', required: false, description: `Full prompt payload to publish as a new version. Mutually exclusive with edits.` },
    ],
  },
  {
    ...
    params: [
      ...
      { name: 'eventName', type: 'string', required: false, description: `Event name to filter properties by, required for event type` },
      { name: 'includePredefinedProperties', type: 'boolean', required: false, description: `Whether to include predefined properties` },
      { name: 'limit', type: 'integer', required: false, description: `No description.` },
      { name: 'offset', type: 'integer', required: false, description: `No description.` },
    ],
  },
  {
    ...
    params: [
      { name: 'domain', type: 'string', required: true, description: `The custom domain to proxy through, e.g. 'e.example.com'. Must be a valid subdomain you control.` },
    ],
  },
  {
    ...
    params: [
      { name: 'id', type: 'string', required: true, description: `A UUID string identifying this proxy record.` },
    ],
  },
  {
...
- Avoid overcomplicating the response with excessive property filters. Focus on the simplest solution.`,
    params: [
      ...
      { name: 'dateRange', type: 'object', required: false, description: `Date range to filter results.` },
      { name: 'filterGroup', type: 'array', required: false, description: `Property filters for the query` },
      { name: 'filterTestAccounts', type: 'boolean', required: false, description: `Whether to filter out test accounts.` },
      { name: 'issueId', type: 'string', required: false, description: `Filter to a specific error tracking issue by ID.` },
      { name: 'kind', type: 'string', required: false, description: `No description.` },
      { name: 'limit', type: 'integer', required: false, description: `No description.` },
      { name: 'offset', type: 'integer', required: false, description: `No description.` },
      { name: 'orderBy', type: 'string', required: false, description: `Field to sort results by.` },
      { name: 'orderDirection', type: 'string', required: false, description: `Sort direction.` },
      { name: 'searchQuery', type: 'string', required: false, description: `Free-text search across exception type, message, and stack frames.` },
      { name: 'status', type: 'string', required: false, description: `Filter by issue status.` },
      { name: 'volumeResolution', type: 'integer', required: false, description: `Controls volume chart granularity. Use 1 for sparklines, 0 for counts only.` },
    ],
  },
  {
    ...
    params: [
      { name: 'question', type: 'string', required: true, description: `Your natural language query describing the SQL insight (max 1000 characters).` },
    ],
  },
  {
...
- Prefer \`searchTerm\` for simple text matching; use \`filterGroup\` with type \`log\` and key \`message\` for regex or exact matching.`,
  ...
  },
  {
    ...
    params: [
      { name: 'id', type: 'string', required: true, description: `A UUID string identifying this role.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'model_name', type: 'string', required: true, description: `The type of record to modify. Currently only "FeatureFlag" is supported.

* \`FeatureFlag\` - feature flag` },
      { name: 'payload', type: 'string', required: true, description: `The change to apply. Must include an 'operation' key and a 'value' key. Supported operations: 'update_status' (value: true/false to enable/disable the flag), 'add_release_condition' (value: object with 'groups', 'payloads', and 'multivariate' keys), 'update_variants' (value: object with 'variants' and 'payloads' keys).` },
      { name: 'record_id', type: 'string', required: true, description: `The ID of the record to modify (e.g. the feature flag ID).` },
      { name: 'scheduled_at', type: 'string', required: true, description: `ISO 8601 datetime when the change should be applied (e.g. '2025-06-01T14:00:00Z').` },
      { name: 'cron_expression', type: 'string', required: false, description: `No description.` },
      { name: 'end_date', type: 'string', required: false, description: `Optional ISO 8601 datetime after which a recurring schedule stops executing.` },
      { name: 'is_recurring', type: 'boolean', required: false, description: `Whether this schedule repeats. Only the 'update_status' operation supports recurring schedules.` },
      { name: 'recurrence_interval', type: 'string', required: false, description: `How often the schedule repeats. Required when is_recurring is true. One of: daily, weekly, monthly, yearly.
...
* \`yearly\` - yearly` },
  ...
  },
  {
    ...
    params: [
      { name: 'id', type: 'number', required: true, description: `A unique integer value identifying this scheduled change.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'limit', type: 'number', required: false, description: `Number of results to return per page.` },
      { name: 'model_name', type: 'string', required: false, description: `Filter by model type. Use "FeatureFlag" to see feature flag schedules.` },
      { name: 'offset', type: 'number', required: false, description: `The initial index from which to return the results.` },
      { name: 'record_id', type: 'string', required: false, description: `Filter by the ID of a specific feature flag.` },
    ],
  },
  {
    ...
    params: [
      ...
      { name: 'end_date', type: 'string', required: false, description: `Optional ISO 8601 datetime after which a recurring schedule stops executing.` },
      { name: 'is_recurring', type: 'boolean', required: false, description: `Whether this schedule repeats. Only the 'update_status' operation supports recurring schedules.` },
      { name: 'model_name', type: 'string', required: false, description: `The type of record to modify. Currently only "FeatureFlag" is supported.

* \`FeatureFlag\` - feature flag` },
      { name: 'payload', type: 'string', required: false, description: `The change to apply. Must include an 'operation' key and a 'value' key. Supported operations: 'update_status' (value: true/false to enable/disable the flag), 'add_release_condition' (value: object with 'groups', 'payloads', and 'multivariate' keys), 'update_variants' (value: object with 'variants' and 'payloads' keys).` },
      { name: 'record_id', type: 'string', required: false, description: `The ID of the record to modify (e.g. the feature flag ID).` },
      { name: 'recurrence_interval', type: 'string', required: false, description: `How often the schedule repeats. Required when is_recurring is true. One of: daily, weekly, monthly, yearly.
...
* \`monthly\` - monthly
* \`yearly\` - yearly` },
      { name: 'scheduled_at', type: 'string', required: false, description: `ISO 8601 datetime when the change should be applied (e.g. '2025-06-01T14:00:00Z').` },
    ],
  },
  {
    ...
    params: [
      { name: 'id', type: 'string', required: true, description: `A UUID string identifying this session recording.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'deleted', type: 'boolean', required: false, description: `Set to true to soft-delete the playlist.` },
      { name: 'derived_name', type: 'string', required: false, description: `No description.` },
      { name: 'description', type: 'string', required: false, description: `Optional description of the playlist's purpose or contents.` },
      { name: 'filters', type: 'string', required: false, description: `JSON object with recording filter criteria. Only used when type is 'filters'. Defines which recordings match this saved filter view. When updating a filters-type playlist, you must include the existing filters alongside any other changes — omitting filters will be treated as removing them.` },
      { name: 'name', type: 'string', required: false, description: `Human-readable name for the playlist.` },
      { name: 'pinned', type: 'boolean', required: false, description: `Whether this playlist is pinned to the top of the list.` },
      { name: 'type', type: 'string', required: false, description: `Playlist type: 'collection' for manually curated recordings, 'filters' for saved filter views. Required on create, cannot be changed after.
...
* \`filters\` - Filters` },
  ...
  },
  {
    ...
    params: [
      { name: 'frequency', type: 'string', required: true, description: `How often to deliver: daily, weekly, monthly, or yearly.

* \`daily\` - Daily
* \`weekly\` - Weekly
* \`monthly\` - Monthly
* \`yearly\` - Yearly` },
      { name: 'start_date', type: 'string', required: true, description: `When to start delivering (ISO 8601 datetime).` },
      { name: 'target_type', type: 'string', required: true, description: `Delivery channel: email, slack, or webhook.

* \`email\` - Email
* \`slack\` - Slack
* \`webhook\` - Webhook` },
      { name: 'target_value', type: 'string', required: true, description: `Recipient(s): comma-separated email addresses for email, Slack channel name/ID for slack, or full URL for webhook.` },
      { name: 'bysetpos', type: 'string', required: false, description: `Position within byweekday set for monthly frequency (e.g. 1 for first, -1 for last).` },
      { name: 'byweekday', type: 'string', required: false, description: `Days of week for weekly subscriptions: monday, tuesday, wednesday, thursday, friday, saturday, sunday.` },
      { name: 'count', type: 'string', required: false, description: `Total number of deliveries before the subscription stops. Null for unlimited.` },
      { name: 'dashboard', type: 'string', required: false, description: `Dashboard ID to subscribe to (mutually exclusive with insight on create).` },
      { name: 'dashboard_export_insights', type: 'array', required: false, description: `List of insight IDs from the dashboard to include. Required for dashboard subscriptions, max 6.` },
      { name: 'deleted', type: 'boolean', required: false, description: `Set to true to soft-delete. Subscriptions cannot be hard-deleted.` },
      { name: 'insight', type: 'string', required: false, description: `Insight ID to subscribe to (mutually exclusive with dashboard on create).` },
      { name: 'integration_id', type: 'string', required: false, description: `ID of a connected Slack integration. Required when target_type is slack.` },
      { name: 'interval', type: 'number', required: false, description: `Interval multiplier (e.g. 2 with weekly frequency means every 2 weeks). Default 1.` },
      { name: 'invite_message', type: 'string', required: false, description: `Optional message included in the invitation email when adding new recipients.` },
      { name: 'summary_enabled', type: 'boolean', required: false, description: `No description.` },
      { name: 'summary_prompt_guide', type: 'string', required: false, description: `No description.` },
      { name: 'title', type: 'string', required: false, description: `Human-readable name for this subscription.` },
      { name: 'until_date', type: 'string', required: false, description: `When to stop delivering (ISO 8601 datetime). Null for indefinite.` },
    ],
  },
  {
    ...
    params: [
      { name: 'created_by', type: 'string', required: false, description: `Filter by creator user UUID.` },
      { name: 'dashboard', type: 'number', required: false, description: `Filter by dashboard ID.` },
      { name: 'insight', type: 'number', required: false, description: `Filter by insight ID.` },
      { name: 'limit', type: 'number', required: false, description: `Number of results to return per page.` },
      { name: 'offset', type: 'number', required: false, description: `The initial index from which to return the results.` },
      { name: 'ordering', type: 'string', required: false, description: `Which field to use when ordering the results.` },
      { name: 'resource_type', type: 'string', required: false, description: `Filter by subscription resource: insight vs dashboard export.` },
      { name: 'search', type: 'string', required: false, description: `A search term.` },
      { name: 'target_type', type: 'string', required: false, description: `Filter by delivery channel (email, Slack, or webhook).` },
    ],
  },
  {
    ...
    params: [
      { name: 'id', type: 'number', required: true, description: `A unique integer value identifying this subscription.` },
      { name: 'bysetpos', type: 'string', required: false, description: `Position within byweekday set for monthly frequency (e.g. 1 for first, -1 for last).` },
      { name: 'byweekday', type: 'string', required: false, description: `Days of week for weekly subscriptions: monday, tuesday, wednesday, thursday, friday, saturday, sunday.` },
      { name: 'count', type: 'string', required: false, description: `Total number of deliveries before the subscription stops. Null for unlimited.` },
      { name: 'dashboard', type: 'string', required: false, description: `Dashboard ID to subscribe to (mutually exclusive with insight on create).` },
      { name: 'dashboard_export_insights', type: 'array', required: false, description: `List of insight IDs from the dashboard to include. Required for dashboard subscriptions, max 6.` },
      { name: 'deleted', type: 'boolean', required: false, description: `Set to true to soft-delete. Subscriptions cannot be hard-deleted.` },
      { name: 'frequency', type: 'string', required: false, description: `How often to deliver: daily, weekly, monthly, or yearly.
...
* \`yearly\` - Yearly` },
      ...
      { name: 'interval', type: 'number', required: false, description: `Interval multiplier (e.g. 2 with weekly frequency means every 2 weeks). Default 1.` },
      { name: 'invite_message', type: 'string', required: false, description: `Optional message included in the invitation email when adding new recipients.` },
      { name: 'start_date', type: 'string', required: false, description: `When to start delivering (ISO 8601 datetime).` },
      { name: 'summary_enabled', type: 'boolean', required: false, description: `No description.` },
      { name: 'summary_prompt_guide', type: 'string', required: false, description: `No description.` },
      { name: 'target_type', type: 'string', required: false, description: `Delivery channel: email, slack, or webhook.

* \`email\` - Email
* \`slack\` - Slack
* \`webhook\` - Webhook` },
      { name: 'target_value', type: 'string', required: false, description: `Recipient(s): comma-separated email addresses for email, Slack channel name/ID for slack, or full URL for webhook.` },
      { name: 'title', type: 'string', required: false, description: `Human-readable name for this subscription.` },
      { name: 'until_date', type: 'string', required: false, description: `When to stop delivering (ISO 8601 datetime). Null for indefinite.` },
    ],
  },
  ...
  },
  {
    ...
    params: [
      ...
      { name: 'type', type: 'string', required: true, description: `Survey type.

* \`popover\` - popover
* \`widget\` - widget
* \`external_survey\` - external survey
* \`api\` - api` },
      { name: 'appearance', type: 'string', required: false, description: `Survey appearance customization.` },
      { name: 'description', type: 'string', required: false, description: `Survey description.` },
      { name: 'enable_partial_responses', type: 'string', required: false, description: `When at least one question is answered, the response is stored (true). The response is stored when all questions are answered (false).` },
      { name: 'iteration_count', type: 'string', required: false, description: `For a recurring schedule, this field specifies the number of times the survey should be shown to the user. Use 1 for 'once every X days', higher numbers for multiple repetitions. Works together with iteration_frequency_days to determine the overall survey schedule.` },
      { name: 'iteration_frequency_days', type: 'string', required: false, description: `For a recurring schedule, this field specifies the interval in days between each survey instance shown to the user, used alongside iteration_count for precise scheduling.` },
      { name: 'linked_flag_id', type: 'string', required: false, description: `The feature flag linked to this survey.` },
      { name: 'questions', type: 'string', required: false, description: `
        ...
        \`\`\`
        ` },
      { name: 'responses_limit', type: 'string', required: false, description: `The maximum number of responses before automatically stopping the survey.` },
      { name: 'start_date', type: 'string', required: false, description: `Setting this will launch the survey immediately. Don't add a start_date unless explicitly requested to do so.` },
      { name: 'targeting_flag_filters', type: 'string', required: false, description: `Target specific users based on their properties. Example: {groups: [{properties: [{key: 'email', value: ['@company.com'], operator: 'icontains'}], rollout_percentage: 100}]}` },
    ],
  },
  {
    ...
    params: [
      { name: 'id', type: 'string', required: true, description: `A UUID string identifying this survey.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'id', type: 'string', required: true, description: `A UUID string identifying this survey.` },
      { name: 'date_from', type: 'string', required: false, description: `Optional ISO timestamp for start date (e.g. 2024-01-01T00:00:00Z)` },
      { name: 'date_to', type: 'string', required: false, description: `Optional ISO timestamp for end date (e.g. 2024-01-31T23:59:59Z)` },
    ],
  },
  {
    ...
    params: [
      { name: 'id', type: 'string', required: true, description: `A UUID string identifying this survey.` },
      { name: 'appearance', type: 'string', required: false, description: `Survey appearance customization.` },
      { name: 'archived', type: 'boolean', required: false, description: `Archive state for the survey.` },
      { name: 'conditions', type: 'string', required: false, description: `Display and targeting conditions for the survey.` },
      { name: 'description', type: 'string', required: false, description: `Survey description.` },
      { name: 'enable_partial_responses', type: 'string', required: false, description: `When at least one question is answered, the response is stored (true). The response is stored when all questions are answered (false).` },
      { name: 'end_date', type: 'string', required: false, description: `When the survey stopped being shown to users. Setting this will complete the survey.` },
      { name: 'iteration_count', type: 'string', required: false, description: `For a recurring schedule, this field specifies the number of times the survey should be shown to the user. Use 1 for 'once every X days', higher numbers for multiple repetitions. Works together with iteration_frequency_days to determine the overall survey schedule.` },
      { name: 'iteration_frequency_days', type: 'string', required: false, description: `For a recurring schedule, this field specifies the interval in days between each survey instance shown to the user, used alongside iteration_count for precise scheduling.` },
      { name: 'linked_flag_id', type: 'string', required: false, description: `The feature flag linked to this survey.` },
      ...
      { name: 'questions', type: 'string', required: false, description: `
        ...
        \`\`\`
        ` },
      { name: 'remove_targeting_flag', type: 'string', required: false, description: `Set to true to completely remove all targeting filters from the survey, making it visible to all users (subject to other display conditions like URL matching).` },
      { name: 'responses_limit', type: 'string', required: false, description: `The maximum number of responses before automatically stopping the survey.` },
      { name: 'schedule', type: 'string', required: false, description: `Survey scheduling behavior: 'once' = show once per user (default), 'recurring' = repeat based on iteration_count and iteration_frequency_days settings, 'always' = show every time conditions are met (mainly for widget surveys)
...
* \`always\` - always` },
      { name: 'start_date', type: 'string', required: false, description: `Setting this will launch the survey immediately. Don't add a start_date unless explicitly requested to do so.` },
      { name: 'targeting_flag_filters', type: 'string', required: false, description: `Target specific users based on their properties. Example: {groups: [{properties: [{key: 'email', value: ['@company.com'], operator: 'icontains'}], rollout_percentage: 100}]}` },
      { name: 'targeting_flag_id', type: 'number', required: false, description: `An existing targeting flag to use for this survey.` },
      { name: 'type', type: 'string', required: false, description: `Survey type.
...
* \`api\` - api` },
  ...
  },
  {
    ...
    params: [
      { name: 'orgId', type: 'string', required: true, description: `No description.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'name', type: 'string', required: true, description: `Unique name for the view. Used as the table name in HogQL queries. Must not conflict with existing table names.` },
      { name: 'dag_id', type: 'string', required: false, description: `Optional DAG to place this view into` },
      { name: 'folder_id', type: 'string', required: false, description: `Optional folder ID used to organize this view in the SQL editor sidebar.` },
      { name: 'is_test', type: 'boolean', required: false, description: `Whether this view is for testing only and will auto-expire.` },
      { name: 'query', type: 'string', required: false, description: `HogQL query definition as a JSON object. Must contain a "query" key with the SQL string. Example: {"query": "SELECT * FROM events LIMIT 100"}` },
    ],
  },
  {
    ...
    params: [
      { name: 'id', type: 'string', required: true, description: `A UUID string identifying this data warehouse saved query.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'page', type: 'number', required: false, description: `A page number within the paginated result set.` },
      { name: 'search', type: 'string', required: false, description: `A search term.` },
    ],
  },
  {
    ...
    params: [
      { name: 'id', type: 'string', required: true, description: `A UUID string identifying this data warehouse saved query.` },
      { name: 'name', type: 'string', required: true, description: `Unique name for the view. Used as the table name in HogQL queries and the node name in the data modeling Node.` },
      { name: 'dag_id', type: 'string', required: false, description: `Optional DAG to place this view into` },
      { name: 'deleted', type: 'string', required: false, description: `No description.` },
      { name: 'edited_history_id', type: 'string', required: false, description: `Activity log ID from the last known edit. Used for conflict detection.` },
      { name: 'folder_id', type: 'string', required: false, description: `Optional folder ID used to organize this view in the SQL editor sidebar.` },
      { name: 'is_test', type: 'boolean', required: false, description: `Whether this view is for testing only and will auto-expire.` },
      { name: 'query', type: 'string', required: false, description: `HogQL query definition as a JSON object with a "query" key containing the SQL string and a "kind" key containing the query type. Example: {"query": "SELECT * FROM events LIMIT 100", "kind": "HogQLQuery"}` },
      { name: 'soft_update', type: 'string', required: false, description: `If true, skip column inference and validation. For saving drafts.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'id', type: 'string', required: true, description: `A UUID string identifying this data warehouse saved query.` },
      { name: 'dag_id', type: 'string', required: false, description: `Optional DAG to place this view into` },
      { name: 'edited_history_id', type: 'string', required: false, description: `Required when updating the query field. Get this from latest_history_id on the retrieve response. Used for optimistic concurrency control.` },
      { name: 'folder_id', type: 'string', required: false, description: `Optional folder ID used to organize this view in the SQL editor sidebar.` },
      { name: 'is_test', type: 'boolean', required: false, description: `Whether this view is for testing only and will auto-expire.` },
      { name: 'name', type: 'string', required: false, description: `Unique name for the view. Used as the table name in HogQL queries. Must not conflict with existing table names.` },
      { name: 'query', type: 'string', required: false, description: `HogQL query definition as a JSON object. Must contain a "query" key with the SQL string. Example: {"query": "SELECT * FROM events LIMIT 100"}` },
    ],
  },
  {
    ...
    params: [
      { name: 'id', type: 'string', required: true, description: `A UUID string identifying this hog flow.` },
    ],
  },
  {
