...
export const tools: Tool[] = [
  {
    ...
    params: [
      { name: 'name', type: 'string', required: true, description: `Name for the new app. Maximum 24 characters.` },
      { name: 'stack', type: 'string', required: true, description: `Platform/stack for the app. Accepted values: WEBFLOW, VANILLA, WORDPRESS.` },
      { name: 'templateId', type: 'string', required: false, description: `Webflow template ID to scaffold the app from. Only applicable for WEBFLOW stack.` },
      { name: 'wordpressPageBuilder', type: 'string', required: false, description: `WordPress page builder plugin. Accepted values: GUTENBERG, ELEMENTOR, DIVI, BEAVER_BUILDER, BRICKS, CORNERSTONE, OTHER.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'category', type: 'string', required: false, description: `Filter tools by category. Accepted values: core, members, plans, dataTables, gatedContent, teams, customFields, stripe. Omit to see all.` },
      { name: 'search', type: 'string', required: false, description: `Search term to filter tools by name or description.` },
    ],
  },
  {
    ...
    params: [
      { name: 'toolName', type: 'string', required: true, description: `Exact tool name returned by explore_tools, e.g. getMember.` },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      { name: 'appId', type: 'string', required: true, description: `Unique identifier of the app to switch to. Retrieve app IDs using listApps.` },
    ],
  },
  {
    ...
    params: [
      { name: 'environment', type: 'string', required: true, description: `Environment for member operations. Accepted values: LIVE (production), SANDBOX (test data).` },
    ],
  },
]
