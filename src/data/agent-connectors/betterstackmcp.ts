...
Dashboard chart queries run against the metrics collection: use \`sum(logs_count)\` (or the matching \`sum(<row_type>_count)\` for the source), \`avgMerge(value_avg)\`, \`label('tag')\`. Do NOT use \`JSONExtract(raw, …)\` — that is for live-tail charts and \`query\` only; dashboards have no \`raw\` column. Call \`metrics_query_help\` with the source ID **and \`context: 'chart_query'\`** for available metrics/labels and chart-query-style examples.`,
  ...
  },
  {
    ...
    params: [
      ...
      },
      {
        ...
        type: 'string',
        required: false,
        description: `Short description of the section, shown as a tooltip next to section name in the UI. Use rarely, when wanting to show more context for non-obvious sections.`,
      },
    ],
  },
  {
  ...
  {
    name: 'betterstackmcp_change_team_member_role',
    description: `Change a team member's role. Identify the member by email or user_id (from team_members) and pass the target role_id (from team_roles). The admin role cannot be assigned, and an existing admin's role cannot be changed, via the API. Pending invitations can't have their role changed — cancel and re-invite instead. If the token can reach more than one team, pass team_id (or team_name).`,
    params: [
      {
        name: 'role_id',
        type: 'integer',
        required: true,
        description: `ID of the role to assign (use team_roles to look up ids).`,
      },
      {
        name: 'email',
        type: 'string',
        required: false,
        description: `E-mail of the member whose role to change (alternative to user_id).`,
      },
      {
        name: 'team_id',
        type: 'integer',
        required: false,
        description: `ID of the team the member belongs to. Required when the token can access more than one team.`,
      },
      {
        name: 'team_name',
        type: 'string',
        required: false,
        description: `Name of the team (alternative to team_id).`,
      },
      {
        name: 'user_id',
        type: 'integer',
        required: false,
        description: `ID of the member whose role to change (alternative to email).`,
      },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      ...
      {
        name: 'anomaly_training_range_days',
        type: 'integer',
        required: false,
        description: `How many days of data the anomaly model is first trained on (1-30, default 7), shown as 'Initial training data range' in the UI. A full week (7) captures daily and weekly pattern shifts. Only for anomaly_rrcf alerts`,
      },
      {
  ...
  },
  {
    ...
    params: [
      ...
      },
      {
        ...
        type: 'boolean',
        required: false,
        description: `Send e-mail alerts when the heartbeat goes down.`,
      },
      {
        name: 'grace',
        type: 'integer',
        required: false,
        description: `Extra seconds to wait after the period elapses before reporting the heartbeat as down. Defaults to 300.`,
      },
      {
        name: 'heartbeat_group_id',
        type: 'integer',
        required: false,
        description: `ID of the heartbeat group to add this heartbeat to.`,
      },
      {
        name: 'maintenance_days',
        type: 'array',
        required: false,
        description: `Days the maintenance window applies to (e.g. ["mon", "tue", "wed"]).`,
      },
      {
        name: 'maintenance_from',
        type: 'string',
        required: false,
        description: `Start of a daily maintenance window during which alerts are paused (e.g. "01:00:00").`,
      },
      {
        name: 'maintenance_timezone',
        type: 'string',
        required: false,
        description: `Timezone for the maintenance window (e.g. "UTC", "Europe/Berlin"). Defaults to UTC.`,
      },
      {
        name: 'maintenance_to',
        type: 'string',
        required: false,
        description: `End of the daily maintenance window (e.g. "03:00:00").`,
      },
      {
        name: 'paused',
        type: 'boolean',
        required: false,
        description: `Create the heartbeat in a paused state, so it expects no requests and raises no incidents until resumed.`,
      },
      {
        name: 'period',
        type: 'integer',
        required: false,
        description: `How often you expect a request, in seconds. The heartbeat reports down if no request arrives within this window plus the grace period. Defaults to 86400 (one day).`,
      },
      {
        name: 'policy_id',
        type: 'integer',
      ...
      },
      {
        ...
        type: 'boolean',
        required: false,
        description: `Send push notification alerts when the heartbeat goes down.`,
      },
      {
        name: 'sms',
        type: 'boolean',
        required: false,
        description: `Send SMS alerts when the heartbeat goes down.`,
      },
      {
        name: 'sort_index',
        type: 'integer',
        required: false,
        description: `Position of the heartbeat in the list, set to NULL to sort by creation date.`,
      },
      {
        name: 'team_id',
        type: 'integer',
        required: false,
        description: `The ID of the team to create the heartbeat in (use list_heartbeats to see team IDs). Required only when the API token has access to multiple teams.`,
      },
      {
        name: 'team_wait',
        type: 'integer',
...
Provide the \`url\` to monitor. For ping, TCP, UDP, SMTP, POP, IMAP, and DNS monitors this is the host (e.g. \`example.com\`) rather than a full URL. The monitor starts checking immediately unless \`paused\` is set to true.`,
  ...
  },
  {
    ...
    params: [
      ...
      },
      {
        ...
        type: 'integer',
        required: false,
        description: `Optional team ID (use teams to get team IDs). Status pages belong to organizations, so this is only needed to narrow the lookup to one team's organization`,
      },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      ...
      },
      {
        ...
        type: 'string',
        required: false,
        description: `Optional description shown under the resource`,
      },
      {
        name: 'position',
        type: 'integer',
        required: false,
        description: `Zero-based position of the resource within its section`,
      },
      {
        name: 'status_page_section_id',
        type: 'integer',
        required: false,
        description: `The ID of the section to add the resource to (use status_page_sections to list them). Defaults to the first section.`,
      },
      {
        name: 'team_id',
        type: 'integer',
        required: false,
        description: `The ID of the team that owns the status page. Required only when the API token has access to multiple teams.`,
      },
      {
        name: 'widget_type',
        type: 'string',
        required: false,
        description: `How the resource is displayed. "history" shows the daily uptime bar, "plain" hides it.`,
      },
    ],
  },
  {
    name: 'betterstackmcp_create_status_page_section',
    description: `Create a section (resource group) on a status page to group resources under a heading.`,
    params: [
      {
        name: 'name',
        type: 'string',
        required: true,
        description: `The section heading shown to visitors`,
      },
      {
        name: 'status_page_id',
        type: 'integer',
      ...
      },
      {
        ...
        type: 'integer',
        required: false,
        description: `Zero-based position of the section on the status page`,
      },
      {
        name: 'team_id',
        type: 'integer',
  ...
  {
    name: 'betterstackmcp_edit_application',
    description: `Edit an existing application in Better Stack — rename it, pause or resume ingesting, or set its VRL transformations, including the exception grouping program. Only provide the fields you want to change. Use applications or application first to find the application ID.`,
    params: [
      {
        name: 'id',
        type: 'integer',
        required: true,
        description: `The ID of the application to edit`,
      },
      {
        name: 'ingesting_paused',
        type: 'boolean',
        required: false,
        description: `Set to true to temporarily pause data ingesting for this application, false to resume it`,
      },
      {
        name: 'name',
        type: 'string',
      ...
      {
        name: 'vrl_transformation_exceptions',
        type: 'string',
        required: false,
        description: `VRL transformation applied to exceptions during ingestion. This is what controls how exceptions are grouped into errors: it should set the ._pattern grouping key. Set to an empty string to remove a custom program, omit to leave unchanged.`,
      },
      {
        name: 'vrl_transformation_logs',
        type: 'string',
        required: false,
        description: `VRL transformation applied to logs during ingestion. Set to an empty string to remove, omit to leave unchanged.`,
      },
      {
        name: 'vrl_transformation_replays',
        type: 'string',
        required: false,
        description: `VRL transformation applied to session replays during ingestion. Set to an empty string to remove, omit to leave unchanged.`,
      },
      {
        name: 'vrl_transformation_spans',
        type: 'string',
        required: false,
        description: `VRL transformation applied to traces (spans) during ingestion. Set to an empty string to remove, omit to leave unchanged.`,
      },
      {
        name: 'vrl_transformation_web_events',
        type: 'string',
        required: false,
        description: `VRL transformation applied to web events (page views and web vitals) during ingestion. Set to an empty string to remove, omit to leave unchanged.`,
      },
    ],
  },
  {
  ...
  },
  {
    ...
    params: [
      ...
      },
      {
        ...
        type: 'integer',
        required: false,
        description: `How many days of data the anomaly model is first trained on (1-30, default 7), shown as 'Initial training data range' in the UI`,
      },
      {
        name: 'anomaly_trigger',
        type: 'string',
      ...
      },
      {
        ...
        type: 'boolean',
        required: false,
        description: `When true, each alerting series opens its own incident instead of one shared incident`,
      },
  ...
  },
  {
    ...
    params: [
      ...
      },
      {
        ...
        type: 'string',
        required: false,
        description: `Short description shown as a tooltip next to the section name. Pass an empty string to clear it.`,
      },
  ...
  {
    name: 'betterstackmcp_invite_team_member',
    description: `Invite someone to a Better Stack team by e-mail address. Optionally set their role by system-role name (role: responder, member, team_lead, billing_admin) or by role_id (use team_roles to look up ids). Defaults to responder. The admin role cannot be assigned via the API. Someone who already belongs to the organization is added to the team directly, with no invitation to accept; an organization-wide role they hold (Admin, Billing admin) is kept. If the token can reach more than one team, pass team_id (or team_name).`,
    params: [
      {
        name: 'email',
        type: 'string',
        required: true,
        description: `E-mail address of the person to invite.`,
      },
      {
        name: 'role',
        type: 'string',
        required: false,
        description: `System-role name: responder, member, team_lead, or billing_admin. Defaults to responder. 'admin' is not allowed.`,
      },
      {
        name: 'role_id',
        type: 'integer',
        required: false,
        description: `ID of the role to assign (alternative to role; supports custom roles). Use team_roles to look up ids.`,
      },
      {
        name: 'team_id',
        type: 'integer',
        required: false,
        description: `ID of the team to invite into. Required when the token can access more than one team.`,
      },
      {
        name: 'team_name',
        type: 'string',
        required: false,
        description: `Name of the team to invite into (alternative to team_id).`,
      },
    ],
  },
  {
...
- **IMPORANT**: Use \`metrics_query_help\` to get instructions on how to create the correct query for metrics`,
  ...
  {
    name: 'betterstackmcp_remove_dashboard_variable',
    description: `Remove a dashboard template variable by name. Cannot remove the automatic variables source, start_time, end_time, or time. A chart still referencing a removed variable as a required {{name}} errors until it is redefined (the next chart save auto-creates it again, empty). Use dashboard first to see the defined variables.`,
    params: [
      {
        name: 'dashboard_id',
        type: 'integer',
  ...
  {
    name: 'betterstackmcp_remove_status_page_resource',
    description: `Remove a resource from a status page.`,
    params: [
      {
        name: 'resource_id',
        type: 'integer',
        required: true,
        description: `The ID of the status page resource to remove (from status_page_resources)`,
      },
      {
        name: 'status_page_id',
        type: 'integer',
  ...
  {
    name: 'betterstackmcp_remove_status_page_section',
    description: `Remove a section from a status page. Resources in the section are removed with it.`,
    params: [
      {
        name: 'section_id',
        type: 'integer',
        required: true,
        description: `The ID of the section to remove (from status_page_sections)`,
      },
      {
        name: 'status_page_id',
        type: 'integer',
  ...
  {
    name: 'betterstackmcp_remove_team_member',
    ...
    params: [
      {
        ...
        type: 'string',
        required: false,
        description: `E-mail of the member to remove, or of a pending invitation to cancel (alternative to user_id).`,
      },
      {
        name: 'team_id',
        type: 'integer',
        required: false,
        description: `ID of the team to remove from. Required when the token can access more than one team.`,
      },
      {
        name: 'team_name',
        type: 'string',
      ...
      },
      {
        ...
        type: 'integer',
        required: false,
        description: `ID of the member to remove, or of a pending invitation to cancel — the id column from team_members (alternative to email).`,
      },
    ],
  },
  {
...
- \`pie\` — share of a total across categories. Alias a categorical column \`AS series\` (the slice label) and a numeric column \`AS value\` (the slice size).`,
  ...
  },
  {
    ...
    params: [
      ...
      },
      {
        ...
        type: 'string',
        required: true,
        description: `Variable name used as {{name}} in chart queries`,
      },
      { name: 'variable_type', type: 'string', required: true, description: `The variable type` },
      {
        name: 'allow_multiple_values',
        type: 'boolean',
        required: false,
        description: `select_value / select_with_sql: let users pick several options at once; the variable then interpolates as a quoted regex used with match(), never IN. Ignored for other types.`,
      },
      {
        name: 'default_values',
        type: 'array',
        required: false,
        description: `select_value: the options (first = default). string/number/boolean: [default]. sql_expression: [raw SQL]. date/datetime: [ISO date], e.g. ['2026-07-23T14:30:00Z'].`,
      },
      {
        name: 'filters',
        type: 'array',
        required: false,
        description: `select_predefined_sql: named SQL filters. Unlike sql_definition, each sql is a FULL query returning one column, whose rows are injected into the chart as an IN-list. Filter time with dt BETWEEN {{start_time}} AND {{end_time}}, never {{time}}.`,
      },
      {
        name: 'selected_label',
        type: 'string',
        required: false,
        description: `select_predefined_sql: the default filter label.`,
      },
      {
        name: 'source_ids',
        type: 'array',
        required: false,
        description: `source: the source IDs this variable resolves to (must be accessible to the dashboard's team). Ignored for other types.`,
      },
      {
        name: 'sql_definition',
        type: 'string',
        required: false,
        description: `select_with_sql / multi_select_with_sql: an expression (NOT a full query) yielding the option values — interpolated as SELECT DISTINCT <sql_definition> AS option FROM {{source}} WHERE dt BETWEEN {{start_time}} AND {{end_time}} LIMIT 50. Example: label('_host').`,
      },
    ],
  },
  {
  ...
  {
    name: 'betterstackmcp_status_page_sections',
    description: `List the sections (resource groups) of a status page.`,
    params: [
      {
        name: 'status_page_id',
        type: 'integer',
  ...
  {
    name: 'betterstackmcp_team_members',
    description: `List the members of a Better Stack team, including pending invitations. Returns each member's email, name, role and the mobile app platforms they have signed in on. Supports the same email filter and pagination as the REST team-members API. If the token can reach more than one team, pass team_id (or team_name) to pick one; call it without a team first to see the list of accessible teams. Useful for auditing who has access — e.g. offboarding.`,
    params: [
      {
        name: 'email',
        type: 'string',
        required: false,
        description: `Return only the member or pending invitation with this exact email address.`,
      },
      {
        name: 'page',
        type: 'integer',
      ...
      },
      {
        ...
        type: 'integer',
        required: false,
        description: `Results per page. Defaults to 50, maximum 250.`,
      },
      {
        name: 'team_id',
        type: 'integer',
        required: false,
        description: `ID of the team to list members for. Required when the token can access more than one team.`,
      },
      {
        name: 'team_name',
        type: 'string',
        required: false,
        description: `Name of the team to list members for (alternative to team_id).`,
      },
    ],
  },
  {
    name: 'betterstackmcp_team_roles',
    description: `List the roles defined in a Better Stack organization, including their role_id and system-role identifier (admin, billing_admin, team_lead, responder, member, or "custom"). Use this to discover valid role_id values for interpreting team member roles. If the token can reach more than one team/organization, pass team_id (or team_name) to choose one.`,
    params: [
      {
        name: 'team_id',
        type: 'integer',
        required: false,
        description: `ID of a team in the target organization. Required when the token can access more than one team.`,
      },
      {
        name: 'team_name',
        type: 'string',
        required: false,
        description: `Name of a team in the target organization (alternative to team_id).`,
      },
    ],
  },
  {
...
Prefer \`build_type: new_data\` (default). \`historical_logs\` re-runs the rule over every stored log — expensive; only when the user explicitly asks.`,
  ...
  {
    name: 'betterstackmcp_update_status_page',
    description: `Update the settings of a status page (company name, contact URL, theme, layout, and more).`,
    params: [
      { name: 'id', type: 'integer', required: true, description: `The ID of the status page` },
      {
        name: 'announcement',
      ...
      },
      {
        ...
        type: 'string',
        required: false,
        description: `Company name shown as the status page title`,
      },
      {
        name: 'company_url',
        type: 'string',
        required: false,
        description: `URL the company name links to`,
      },
      {
        name: 'contact_url',
        type: 'string',
        required: false,
        description: `URL or mailto link for the contact/support button`,
      },
      {
        name: 'custom_domain',
        type: 'string',
        required: false,
        description: `Custom CNAME domain for the status page`,
      },
      {
        name: 'hide_from_search_engines',
        type: 'boolean',
        required: false,
        description: `Hide the status page from search engines`,
      },
      { name: 'layout', type: 'string', required: false, description: `Resource layout` },
      {
        name: 'published',
        type: 'boolean',
        required: false,
        description: `Whether the status page is published (publicly visible)`,
      },
      {
        name: 'subdomain',
        type: 'string',
        required: false,
        description: `Subdomain the page is served on (e.g. "acme" for acme.betteruptime.com)`,
      },
      {
        name: 'subscribable',
        type: 'boolean',
      ...
      { name: 'theme', type: 'string', required: false, description: `Color theme` },
      {
        ...
        type: 'string',
        required: false,
        description: `Timezone used to display times (e.g. "Europe/Prague")`,
      },
    ],
  },
  {
    name: 'betterstackmcp_update_status_page_resource',
    description: `Update a resource on a status page. Change its public name, description, widget type (e.g. show or hide the uptime history), or move it by setting position (zero-based) and/or status_page_section_id. Use status_page_resources to find resource IDs.`,
    params: [
      {
        name: 'resource_id',
        type: 'integer',
        required: true,
        description: `The ID of the status page resource to update (from status_page_resources)`,
      },
      {
        name: 'status_page_id',
        type: 'integer',
      ...
      },
      {
        ...
        type: 'string',
        required: false,
        description: `Description shown under the resource`,
      },
      {
        name: 'position',
        type: 'integer',
      ...
      },
      {
        ...
        type: 'string',
        required: false,
        description: `Name shown to visitors`,
      },
      {
        name: 'status_page_section_id',
        type: 'integer',
        required: false,
        description: `Move the resource to this section (use status_page_sections to list them)`,
      },
      {
        name: 'widget_type',
        type: 'string',
  ...
  {
    name: 'betterstackmcp_update_status_page_section',
    description: `Rename a status page section or move it by setting its position.`,
    params: [
      {
        name: 'section_id',
        type: 'integer',
        required: true,
        description: `The ID of the section to update (from status_page_sections)`,
      },
      {
        name: 'status_page_id',
        type: 'integer',
