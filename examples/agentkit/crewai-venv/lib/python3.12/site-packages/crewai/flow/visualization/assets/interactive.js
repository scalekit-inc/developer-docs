...
class NodeRenderer {
  ...
  drawNodeShape(ctx, x, y, width, height, scale, nodeStyle, opacity = 1.0, node = null) {
    ...
    const isHighlighted = this.networkManager.triggeredByHighlighter?.highlightedNodes?.includes(
      node?.id,
    )
    const isActiveNode = this.networkManager.drawerManager?.activeNodeId === node?.id
    const hasHighlightedNodes =
      this.networkManager.triggeredByHighlighter?.highlightedNodes?.length > 0
...
class AnimationManager {
  ...
  animateEdgeWidth(edges, edgeId, targetWidth, duration = CONSTANTS.EDGE.ANIMATION_DURATION) {
    this.cancel(edgeId)
...
class TriggeredByHighlighter {
      ...
      if (directEdges.length > 0) {
        ...
        for (const [nodeName, nodeInfo] of Object.entries(nodeData)) {
          if (nodeInfo.router_paths && nodeInfo.router_paths.includes(triggerNodeId)) {
            ...
            const routerEdges = allEdges.filter((edge) => edge.from === routerNode && edge.dashes)
            let foundEdge = false
  ...
  highlightAllRouterPaths() {
    ...
    if (outgoingRouterEdges.length > 0) {
      ...
      activeMetadata &&
      activeMetadata.router_paths &&
      activeMetadata.router_paths.length > 0
    ) {
        ...
        for (const [nodeName, nodeInfo] of Object.entries(nodeData)) {
          if (nodeInfo.router_paths && nodeInfo.router_paths.includes(pathName)) {
            const edgeFromRouter = allEdges.filter(
              (edge) =>
                edge.from === nodeName && edge.to === this.activeDrawerNodeId && edge.dashes,
            )
  ...
  highlightTriggeredBy(triggerNodeId) {
    ...
    if (connectingEdges.length === 0) {
      const edgesWithLabel = allEdges.filter(
        (edge) =>
          edge.dashes && edge.label === triggerNodeId && edge.to === this.activeDrawerNodeId,
      )
  ...
  animateNodeOpacity() {
    ...
    const animate = () => {
      ...
      allNodesList.forEach((node) => {
        ...
        const targetOpacity = this.highlightedNodes.includes(node.id) ? 1.0 : inactiveOpacity
    ...
    const animate = () => {
      ...
      edgeIds.forEach((edgeId) => {
        ...
        if (this.highlightedEdges.includes(edge.id)) {
          ...
          const currentShadowSize = edge.shadow?.size || CONSTANTS.EDGE.DEFAULT_SHADOW_SIZE
...
class DrawerManager {
  ...
  getPathSignature(triggerNodeId, activeNodeId, allEdges, nodeData) {
    ...
    if (
      ...
      for (const [nodeName, nodeInfo] of Object.entries(nodeData)) {
        if (nodeInfo.router_paths && nodeInfo.router_paths.includes(triggerNodeId)) {
          const routerEdges = allEdges.filter((edge) => edge.from === nodeName && edge.dashes)
    ...
    if (condition.type === 'AND' || condition.type === 'OR') {
      ...
      if (nonStringChildren.length > 0) {
        children += nonStringChildren
          .map((sub) => this.renderConditionTree(sub, depth + 1))
          .join('')
      }
        ...
        children += grouped
          .map((group) => {
            if (group.items.length === 1) {
              ...
              const groupId = group.items.join(',')
              const groupColor =
                conditionType === 'AND' ? '{{ CREWAI_ORANGE }}' : 'var(--text-secondary)'
              const groupBgColor =
                conditionType === 'AND' ? 'rgba(255,90,80,0.08)' : 'rgba(102,102,102,0.06)'
              const groupHoverBg =
                conditionType === 'AND' ? 'rgba(255,90,80,0.15)' : 'rgba(102,102,102,0.12)'
              return `
  ...
  renderMetadata(metadata) {
    ...
    const typeBadgeColor =
      nodeType === 'start' || nodeType === 'router' ? '{{ CREWAI_ORANGE }}' : '{{ DARK_GRAY }}'
    metadataContent += `
    ...
    if (metadata.condition_type) {
      ...
      const { color, bg } = conditionColors[metadata.condition_type] || conditionColors.default
    ...
    if (
      metadata.trigger_condition ||
      (metadata.trigger_methods && metadata.trigger_methods.length > 0)
    ) {
    ...
    return metadataContent ? `<div class="drawer-metadata-grid">${metadataContent}</div>` : ''
  }
  ...
  attachContentEventListeners(nodeName) {
    const copyButtons = this.elements.content.querySelectorAll('.code-copy-button')
    ...
    const accordionHeaders = this.elements.content.querySelectorAll('.accordion-header')
...
class NetworkManager {
  ...
  createNetworkOptions() {
    ...
    const hasExplicitPositions = nodesArray.some(
  ...
  calculateNetworkPosition(isDrawerOpen, centerScreen = false) {
    ...
    const rightEdge = isDrawerOpen
      ? window.innerWidth - CONSTANTS.DRAWER.WIDTH - 40
    ...
    const canvas = this.network
      ? this.network.canvas.frame.canvas
      : document.getElementById('network')
    const canvasRect = canvas.getBoundingClientRect()
    const domCenterX = centerScreen
      ? canvasRect.left + canvas.clientWidth / 2
      : leftEdge + availableWidth / 2
  ...
  setupTheme() {
    ...
    const updateEdgeColors = () => {
      const orEdgeColor = getComputedStyle(document.documentElement)
        .getPropertyValue('--edge-or-color')
        .trim()
