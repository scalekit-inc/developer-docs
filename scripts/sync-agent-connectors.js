...
/**
 * CRM and Sales are paired on the connector catalog for discoverability.
 * If the API returns only one of the two, add the other so both sections list the same connectors.
 */
function normalizeConnectorCategories(categories) {
  const list = (categories || []).map((c) => String(c))
  const set = new Set(list)
  if (set.has('crm') || set.has('sales')) {
    set.add('crm')
    set.add('sales')
  }
...
function generateCatalogFile(providers) {
  ...
  for (const provider of providers) {
    ...
    const categories = normalizeConnectorCategories(provider.categories || [])
    lines.push(`  '${slug}': {`)
...
function generateMdxContent(provider, tools) {
  ...
  // Provider categories (API may or may not return this field); CRM/Sales mirrored for catalog parity
  const categories = normalizeConnectorCategories(
